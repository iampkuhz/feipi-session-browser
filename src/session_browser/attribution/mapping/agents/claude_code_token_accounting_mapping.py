"""Claude Code 专属 call mapping 与 token accounting mapper。"""
...
class ClaudeCodeCallMappingResolver:
    """解析 Claude Code call 的 API family/provider/billing 决策。"""
...
class ClaudeCodeTokenAccountingMapper:
    """把 Claude Code source_units 映射为 field-first accounting payload。"""
        ...
        return {
            ...
            "fresh_input_tokens": self._field_payload(
                ...
                notes=[
                    ...
                    "cache read/write 是 Anthropic accounting fields，不创建 provider cache 来源 candidate。",
                ],
            ),
            "cache_read_tokens": self._field_payload(
                ...
                cache_read,
                [],
                notes=["Anthropic cache_read_input_tokens 只作为 accounting field 展示，不做 per-candidate 拆分。"],
            ),
            "cache_write_tokens": self._field_payload(
                "cache_write_tokens",
                cache_write,
                [],
                notes=["Anthropic cache_creation_input_tokens 只作为 accounting field 展示，不变成 content candidate。"],
            ),
