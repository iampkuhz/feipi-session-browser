"""Qoder call mapping resolver and token accounting mapper."""
...
class QoderCallMappingResolver:
    """Resolve Qoder calls into broker, provider, and billing decisions."""
    ...
    def resolve(
        ...
        *,
        ...
        """Map one Qoder call to the attribution call-mapping contract.

        The resolver is invoked after a Qoder call row is normalized. Any known
        provider-like usage shape is treated as broker-reported token accounting;
        missing usage falls back to local reconstruction from visible source
        units. Raw request and response payloads are accepted to keep the common
        resolver interface stable, but this mapper does not inspect them.

        Args:
            usage: Optional broker or provider usage object from the call.
            model: Model name reported for the call, if available.
            raw_request: Raw request payload reserved for future broker checks.
            raw_response: Raw response payload reserved for future broker checks.

        Returns:
            A decision with Qoder as runtime and broker, token and credit billing
            units, usage source, confidence, reasons, and fallback warnings.
        """
        ...
        if usage_shape in {
            ...
            'openai_responses_like',
            'openai_chat_like',
            'token_reported_unknown_cache',
        }:
...
class QoderTokenAccountingMapper:
    """Build field-first accounting payloads from Qoder source units.

    Attribution assembly calls this mapper after Qoder request or response source
    ...
    fixed field order, keeps cache read/write as broker accounting fields instead
    of content candidates, and never mixes request allocation with response
    allocation.
    ...
    def build_request_accounting(
        ...
        *,
        ...
        """Build request-side token accounting for a Qoder call.

        The request attribution path calls this with request source units and the
        broker or reconstructed request accounting fields. Candidate allocation is
        limited to fresh input tokens, while cache read and cache write remain
        ...
        Args:
            ...
            cache_read: Broker cache-read token field to expose without splits.
            cache_write: Broker cache-write token field to expose without splits.
        ...
        return {
            ...
            'fresh_input_tokens': self._field_payload(
                ...
                notes=[
                    ...
                    'cache read/write 是 Qoder accounting fields, '
    ...
    def build_response_accounting(
        ...
        """Build response-side token accounting for a Qoder call.
...
def _num(value: object) -> float:
    ...
    Args:
        value: Provider, broker, or reconstructed value that should represent tokens.
