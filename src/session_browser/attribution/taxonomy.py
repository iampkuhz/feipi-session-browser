"""Define the canonical request-side token attribution taxonomy.
...
class RequestTokenCategory:
    """Describe one canonical request-side token attribution category.

    The taxonomy layer uses these categories after agent-specific builders emit raw
    bucket keys. They provide stable labels, grouping, ordering, and color keys for API
    payloads and UI charts.

    Attributes:
        key: Stable canonical category key.
        label: Human-readable category label.
        parent_key: Parent group key used by UI grouping.
        parent_label: Human-readable parent group label.
        order: Sort order used for request bucket display.
        color_key: Stable color key consumed by CSS and chart logic.
        description: Explanation shown in diagnostics or tooltip surfaces.
    """
...
def request_bucket_category(agent: str, bucket_key: str) -> RequestTokenCategory:
    ...
    Args:
        agent: Runtime name that produced the raw bucket.
        bucket_key: Runtime-specific request bucket key.

    Returns:
        Canonical request token category, falling back to unlocated residual.
    """
...
def normalize_request_bucket_payload(agent: str, bucket: dict) -> dict:
    """Attach canonical taxonomy metadata to a bucket payload.

    Args:
        agent: Runtime name that produced the raw bucket.
        bucket: Request bucket dictionary emitted by an attribution builder.

    Returns:
        Copy of the bucket with canonical keys, labels, ordering, and color metadata.
    """
...
def sort_request_buckets(agent: str, buckets: list[dict]) -> list[dict]:
    ...
    Args:
        agent: Runtime name used to resolve raw bucket keys.
        buckets: Request bucket dictionaries to sort.

    Returns:
        Buckets sorted by canonical display order with original order as a tiebreaker.
    """
    ...
    indexed.sort(
        key=lambda item: (
            int(
                item[1].get(
                    'display_order', request_bucket_category(agent, item[1].get('key', '')).order
                )
            ),
            item[0],
        )
...
def request_color_index(color_key: str) -> int:
    """Return a stable color slot for the current CSS palette.

    Args:
        color_key: Canonical color key from the request taxonomy.

    Returns:
        Numeric color slot used by existing chart styling.
    """
    order = [
        'current_user_input',
...
def _agent_bucket_map(agent: str) -> dict[str, str]:
    """Return runtime-specific bucket-to-category overrides.

    Args:
        agent: Runtime name that produced request attribution buckets.

    Returns:
        Mapping from runtime bucket key to canonical category key.
    """
