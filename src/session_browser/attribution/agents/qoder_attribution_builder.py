"""Qoder attribution builder backed by normalized source units.

This module is triggered after Qoder sessions have been scanned into a
...
class QoderAttributionBuilder(BaseAttributionBuilder):
    """Attribution implementation for Qoder normalized artifacts.

    The builder is selected for Qoder calls. It consumes provider usage fields,
    cache read/write fields, and normalized ``source_units`` generated by the
    scanner. It emits request and response attribution only for the selected
    call id. If normalized evidence is absent, the builder returns
    low-confidence unavailable or residual values and notes that the session
    should be rescanned.
    ...
    def build_request(self) -> LLMRequestAttribution:
        """Build Qoder request attribution from usage and source units.
    ...
    def build_response(self) -> LLMResponseAttribution:
        """Build Qoder response attribution from output source units.
...
def _session_key(builder: BaseAttributionBuilder) -> str:
    """Derive the Qoder normalized artifact session key.

    Args:
        builder: Builder with an optional session summary.

    Returns:
        Existing summary session key, derived ``qoder:<session_id>`` key, or an
        empty string when neither value exists.
    """
...
def _request_values_from_call(lc: LLMCall) -> tuple[int, int, int]:
    """Extract Qoder request usage values from a call.
...
def _request_availability_rows(has_usage: bool, has_units: bool) -> list[AvailabilityRow]:
    """Build availability rows for Qoder request attribution.
...
def _response_availability_rows(has_output: bool, has_units: bool) -> list[AvailabilityRow]:
    """Build availability rows for Qoder response attribution.
