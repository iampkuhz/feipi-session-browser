...
This module is triggered when Claude Code request attribution needs the
request-side tool schema bucket. Its inputs are local Claude Code SDK type
definitions, optional extracted binary descriptions, and a small on-disk schema
cache. Its outputs are Anthropic-compatible schema dictionaries and token
estimates; they are bounded to schema metadata and do not include tool call
...
# Cache file for extracted Claude Code tool schemas.
_SCHEMA_CACHE_PATH = Path(__file__).parent / '.tool_schemas_cache.json'

# Fallback npm package location when local package discovery fails.
_NPM_PACKAGE_PATHS = [
...
# They are significantly longer than TypeScript JSDoc for tools like Bash and Read.
#
# Extraction notes:
...
_BINARY_TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    'Agent': (
        ...
        'open-ended questions that span the codebase, or tasks that match an available '
        'agent type.\n\n'
        '## Usage notes\n\n'
    ...
    'ExitWorktree': ('Exits a git worktree. Cleans up the temporary worktree directory.'),
    'ListMcpResources': (
...
def _find_sdk_tools_file() -> Path | None:
    """Locate the Claude Code ``sdk-tools.d.ts`` file used as schema input.

    Returns:
        Existing SDK type definition path, or ``None`` when local discovery fails.
    """
    for p in _NPM_PACKAGE_PATHS:
        if p.exists():
            return p

    ext_dirs = Path.home().glob('.vscode/extensions/anthropic.claude-code-*/package/sdk-tools.d.ts')
    for p in ext_dirs:
        if p.exists():
            return p
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            candidate = npm_root / '@anthropic-ai' / 'claude-code' / 'package' / 'sdk-tools.d.ts'
            if candidate.exists():
                return candidate
...
def _parse_ts_interface(
    ...
    Args:
        content: Text content of the Claude Code SDK type definition file.

    Returns:
        Interface names mapped to property schema metadata.
    """
    ...
    for match in re.finditer(interface_pattern, content, re.DOTALL):
        ...
        # Match JSDoc blocks of this form:
        #   /**
        ...
        prop_pattern = (
            ...
            r'(?:\s*\*[^\n]*\n)*?'  # 匹配零行或多行中间的 * 行
            r'\s*\*\s+'  # 匹配最后一个后接空白的 *
            r'((?:(?!\*/).)*?)'  # description captured until */
            r'\s*\*/'  # 匹配 closing */
            r'\s*\n\s*'  # 匹配空白或换行
            r'("?\??-?[\w.-]+"?)'  # property name can include quotes or hyphens
            r'(\?)?:\s*'  # optional marker
            r'([^;]+);'  # type captured until ;
        )
        for jsdoc, raw_prop_name, optional, prop_type in re.findall(prop_pattern, body):
            desc = re.sub(r'\n\s*\*\s*', ' ', jsdoc).strip()
            prop_name = raw_prop_name.strip('"')
            props[prop_name] = {
...
def _map_ts_type(ts_type: str) -> str:
    """Map a TypeScript type into a JSON Schema primitive type string.

    Args:
        ts_type: Raw TypeScript property type extracted from the SDK file.

    Returns:
        JSON Schema primitive type used in the attribution schema boundary.
    """
...
def _build_json_schema(
    ...
    Args:
        tool_name: Claude Code tool name used in the request-side schema.
        description: Runtime or fallback description for the tool.
        properties: Parsed TypeScript property metadata for the tool input.

    Returns:
        Anthropic-compatible tool schema dictionary.
    """
    props: dict[str, Any] = {}
    required: list[str] = []
...
def extract_tool_schemas(
    *,
    ...
    """Extract Claude Code tool schemas from the npm package or fallback cache.
    ...
    try:
        ...
        # Ensure every known Claude Code tool has a schema entry.
...
def _build_fallback_schemas() -> dict[str, dict[str, Any]]:
    """Build fallback tool schemas when the npm package is unavailable.

    Returns:
        Schema dictionaries keyed by Claude Code tool name.
    """
    schemas: dict[str, dict[str, Any]] = {}
...
def get_tool_schema_text(tool_name: str, schemas: dict[str, dict] | None = None) -> str:
    """Get serialized JSON text for a Claude Code tool schema.

    Args:
        tool_name: Claude Code tool name whose schema should be serialized.
        schemas: Optional schema mapping; extracted schemas are used when omitted.

    Returns the actual JSON representation that approximates what gets sent to
    the API, suitable for accurate token counting.

    Returns:
        Serialized schema JSON, or an empty string when the tool is unavailable.
    """
    if schemas is None:
...
def get_tool_schema_tokens(
    ...
    """Estimate the token count for a single Claude Code tool schema.

    Args:
        tool_name: Claude Code tool name whose schema should be counted.
        schemas: Optional schema mapping; extracted schemas are used when omitted.
        model: Optional model hint forwarded to the token estimator.

    Uses the actual serialized JSON text for accurate counting.

    Returns:
        Estimated schema token count, or zero when no schema exists.
    """
...
def get_all_tool_schema_tokens(
    ...
    """Estimate the total token count for multiple Claude Code tool schemas.

    Args:
        tool_names: Claude Code tool names included in the request-side schema set.
        schemas: Optional schema mapping; extracted schemas are used when omitted.
        model: Optional model hint forwarded to the token estimator.

    Returns:
        Sum of estimated schema token counts.
    """
    return sum(get_tool_schema_tokens(name, schemas, model) for name in tool_names)
...
def get_cached_schemas() -> dict[str, dict]:
    """Get cached Claude Code tool schemas, extracting them when necessary.

    Returns:
        Cached or newly extracted schema dictionaries keyed by tool name.
    """
    return extract_tool_schemas()


def invalidate_schema_cache() -> None:
    """Invalidate the module-level Claude Code schema cache."""
    get_cached_schemas.cache_clear()
