...
def normalize_request_reconstruction_buckets(
    ...
    *,
    fresh_input: int,
    request_content_denominator: int | None = None,
    total_input: int | None = None,
) -> list["RequestAttributionBucket"]:
    """Normalize heuristic buckets against the Request Content Denominator.
    ...
    7. Recompute unlocated_residual = max(fresh_input - known_sum, 0).
    8. Recompute coverage = min(known_sum / fresh_input, 1.0).
    ...
    request_content_denominator = (
        fresh_input if request_content_denominator is None else request_content_denominator
    )
    if not buckets or request_content_denominator <= 0:
        return buckets
    ...
    # Step 6: Scale estimated if they exceed the Fresh request-content budget.
    remaining_for_estimated = max(0, request_content_denominator - measured_sum)
    if estimated_sum > remaining_for_estimated and remaining_for_estimated > 0:
        ...
        # When measured already fills/exceeds the request content denominator,
        # preserve estimated buckets — they represent real content.
