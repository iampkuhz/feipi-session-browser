...
- Single agent branch (Model Mix, Tool Distribution, Failure Signals,
  Model Efficiency Detail, View Sessions CTA)
"""
...
from session_browser.index.queries import list_model_stats, list_projects
...
_COMPACT_NUMBER_UNITS = (
    (1_000_000_000.0, 'B'),
    (1_000_000.0, 'M'),
    (1_000.0, 'K'),
)
_MIN_VISIBLE_PERCENT_SHARE = 0.1
_PREVIOUS_CURRENT_VALUE_COUNT = 2
_PERCENT_TOTAL = 100.0
_SECONDS_PER_MINUTE = 60
_SECONDS_PER_HOUR = 3_600
_SECONDS_PER_DAY = 86_400
_PRIMARY_MODEL_SESSION_THRESHOLD = 10
_LOW_SAMPLE_SESSION_THRESHOLD = 3
_HIGH_INPUT_TOKEN_THRESHOLD = 50_000
_LOW_CACHE_REUSE_PERCENT_THRESHOLD = 20.0
_HIGH_FAILURE_PER_SESSION_THRESHOLD = 0.5

_DASHBOARD_TEST_PATCH_SEAMS = (
    compute_aggregate_metrics,
    get_agent_distribution,
    get_token_breakdown,
    list_projects,
...
_KPI_BADGE_DESCRIPTIONS = {
    ...
    'Cache Read Ratio': (
        '当前可见时间窗口最后一个可计算 Cache Read Ratio,对比前一个可计算点,单位为百分点.'
    ),
    'Failed Tools': (
        '当前可见时间窗口最后一个时间点的 failed tool 数,对比前一个时间点;下降显示为正向颜色.'
    ),
}
...
_KPI_SECONDARY_DESCRIPTIONS = {
    ...
    'P50 Session Ratio': (
        'eligible sessions 中每个 session 的 Cache Read / Input-side Tokens 的中位数.'
    ),
...
def _build_chart_notes(agent_scope: str, grain: str) -> dict[str, str]:
    ...
    Args:
        agent_scope: Dashboard scope from the route query, where ``all`` keeps
            agent comparisons and single-agent scopes switch notes to that agent.
        grain: Trend bucket selected by the dashboard control; invalid values are
            normalized before this helper is called.

    Returns:
        Notes keyed by chart id. Text documents aggregation, stacking, and cache
        ratio semantics consumed by the dashboard template.
    """
    ...
    if agent_scope == 'all':
        ...
        token_note = (
            f'{grain_label}展示 total tokens,按照 Fresh、Cache Read、Cache Write、Output 组成展示.'
        )
        prompt_note = (
            f'{grain_label}展示 user prompts 总数,并用折线显示每个 session 的平均 prompts.'
        )
        ...
        prompt_note = (
            f'{grain_label}展示 {agent_label} 的 user prompts 数量,'
            '并用折线显示每个 session 的平均 prompts.'
        )
...
def _fmt(n: int | float) -> str:
    ...
    Args:
        n: Numeric counter from presenter metrics; ``None`` is treated as zero
            for legacy template compatibility.

    Returns:
        Integer display string with thousands separators.
    """
    if n is None:
        return '0'
    return f'{int(n):,}'
...
def _fmt_compact(n: int | float) -> str:
    ...
    Args:
        n: Token, session, or duration-like count rendered in compact KPI cells.

    Returns:
        Compact display string preserving the existing K/M/B units.
    """
    if n is None or n == 0:
        return '0'
    n = float(n)
    for threshold, suffix in _COMPACT_NUMBER_UNITS:
        if n >= threshold:
...
def _safe_ratio(
    num: int | float,
    den: int | float,
    fmt_func: Callable[[int | float], str] = _fmt,
) -> str:
    """计算 ratio 安全地, returning N/A,当 denominator is 0.

    Args:
        num: Numerator metric for the percentage.
        den: Denominator metric; zero or missing values produce ``N/A``.
        fmt_func: Compatibility hook retained for callers that pass a formatter.

    Returns:
        Percentage string with one decimal place, or ``N/A`` when the ratio is
        undefined.
    """
    if not den or den == 0:
        return 'N/A'
...
def _safe_ratio_float(num: int | float, den: int | float) -> float | None:
    ...
    Args:
        num: Numerator metric used by cache and failure calculations.
        den: Denominator metric; zero returns ``None`` so callers can render
            ``N/A`` without losing numeric sorting semantics.

    Returns:
        Raw ratio in the ``0.0`` to ``1.0`` range when defined.
    """
    if not den or den == 0:
        return None
    return num / den
...
def _fmt_percent_share(value: float) -> str:
    ...
    Args:
        value: Percent share value on a 0-100 scale.

    Returns:
        Display percentage, using ``<0.1%`` for non-zero tiny contributors so
        small agents or models remain visible.
    """
    if value > 0 and value < _MIN_VISIBLE_PERCENT_SHARE:
        return '<0.1%'
    return f'{value:.1f}%'
...
def build_dashboard_view_model(
    ...
    Args:
        ...
        page: One-based embedded sessions page used only by single-agent CTAs.
        page_size: Embedded sessions page size; controls pagination metadata only.
...
def _compute_kpis(
    ...
    Args:
        stats: Aggregate dashboard counters for the active agent scope.
        conn: SQLite connection used for supplemental window and percentile
            queries that are not present in ``stats``.
        agent_scope: Normalized dashboard agent scope.
        trend: Bucketed session/token rows used for KPI deltas.
        prompt_activity: Bucketed prompt rows used for prompt KPI deltas.

    Returns:
        KPI card dictionaries in template order with primary, badge, secondary,
        and tooltip fields attached.
    """
    ...
    kpis.append(
        {
            ...
            'secondary': [
                {
                    'label': 'Active 24h',
                    'value': _fmt(_count_active_projects(conn, agent_scope, 1)),
                },
    ...
    kpis.append(
        {
            ...
            'secondary': [
                ...
                {
                    'label': '7d Avg',
                    'value': _fmt_compact(_avg_daily_sessions(conn, agent_scope, 7)),
                },
    ...
    kpis.append(
        {
            ...
            'secondary': [
                ...
                {'label': 'Tool Calls', 'value': _fmt(total_tool_calls)},
                {
                    ...
                    'value': f'{prompts_per_session:.1f}' if total_sessions > 0 else 'N/A',
                },
            ],
        }
    )
    ...
    kpis.append(
        {
            ...
            'secondary': [
                ...
                {
                    'label': 'P50 Session Ratio',
                    'value': f'{p50_ratio * 100:.1f}%' if p50_ratio is not None else 'N/A',
                },
    ...
    kpis.append(
        {
            ...
            'secondary': [
                {
                    'label': 'Failure Rate',
                    'value': f'{failure_rate * 100:.1f}%' if failure_rate is not None else 'N/A',
                },
...
def _attach_kpi_descriptions(kpi: dict[str, Any]) -> dict[str, Any]:
    ...
    Args:
        kpi: KPI card assembled by ``_compute_kpis``.

    Returns:
        Copy of the KPI card enriched with primary, badge, and secondary metric
        descriptions; input order and values are preserved.
    """
    kpi = dict(kpi)
    label = kpi.get('label', '')
...
def _format_delta_badge(delta: int | float | None, suffix: str = '') -> str:
    ...
    Args:
        delta: Current-minus-previous value; ``None`` represents an unavailable
            comparison window.
        suffix: Unit suffix such as ``pp`` appended after compact formatting.

    Returns:
        Signed compact badge string, or ``N/A`` when no comparison is available.
    """
    if delta is None:
        return 'N/A'
    if delta > 0:
...
def _badge_tone(delta: int | float | None, inverse: bool = False) -> str:
    ...
    Args:
        delta: Current-minus-previous value.
        inverse: Whether lower values should be considered positive, used for
            failure-count badges.

    Returns:
        Template tone key: ``positive``, ``negative``, or ``neutral``.
    """
...
def _last_two_values(series: list[dict], key: str) -> tuple[float, float] | None:
    """Return the previous/current values for a numeric trend key.

    Args:
        series: Ordered trend rows from index queries.
        key: Metric key to extract while ignoring missing values.

    Returns:
        Two-float tuple for delta calculations, or ``None`` when fewer than two
        comparable points exist.
    """
    values = [float(row.get(key) or 0) for row in series if row.get(key) is not None]
    if len(values) < _PREVIOUS_CURRENT_VALUE_COUNT:
        return None
    return values[-2], values[-1]


def _series_delta_badge(series: list[dict], key: str, as_percent: bool = False) -> str:
    """Build a KPI delta badge from the final two trend points.

    Args:
        series: Ordered trend rows from dashboard index queries.
        key: Metric key to compare.
        as_percent: Whether the delta should be relative to the previous value.

    Returns:
        Signed badge string, preserving ``N/A`` when comparison is undefined.
    """
...
def _series_delta_tone(series: list[dict], key: str, inverse: bool = False) -> str:
    """Build a KPI badge tone from the final two trend points.

    Args:
        series: Ordered trend rows from dashboard index queries.
        key: Metric key to compare.
        inverse: Whether lower deltas map to a positive tone.

    Returns:
        Visual tone key for the KPI badge.
    """
    pair = _last_two_values(series, key)
    if pair is None:
        return 'neutral'
...
def _cache_ratio_value(row: dict) -> float | None:
    """Compute cache read ratio for one trend row.

    Args:
        row: Trend bucket containing fresh, cache-read, and cache-write tokens.

    Returns:
        Cache read divided by input-side tokens, or ``None`` when input-side
        tokens are zero.
    """
    input_side = (
...
def _cache_ratio_delta_badge(series: list[dict]) -> str:
    """Build the cache read ratio delta badge in percentage points.

    Args:
        series: Ordered trend rows with token components.

    Returns:
        Signed percentage-point badge, or ``N/A`` when fewer than two valid
        cache-ratio points exist.
    """
    ratios = [_cache_ratio_value(row) for row in series]
    ratios = [r for r in ratios if r is not None]
    if len(ratios) < _PREVIOUS_CURRENT_VALUE_COUNT:
        return 'N/A'
    return f'{(ratios[-1] - ratios[-2]) * 100:+.1f}pp'


def _cache_ratio_delta_tone(series: list[dict]) -> str:
    """Build the visual tone for cache ratio movement.

    Args:
        series: Ordered trend rows with token components.

    Returns:
        Badge tone where higher cache read ratio is positive.
    """
    ratios = [_cache_ratio_value(row) for row in series]
    ratios = [r for r in ratios if r is not None]
    if len(ratios) < _PREVIOUS_CURRENT_VALUE_COUNT:
...
def _count_active_projects(conn: sqlite3.Connection, agent_scope: str, days: int) -> int:
    ...
    Args:
        conn: SQLite connection used by the dashboard presenter.
        agent_scope: Normalized agent scope; ``all`` keeps all agents.
        days: Relative trailing window in days.

    Returns:
        Count of distinct project keys with sessions ended inside the window.
    """
    where, params = _build_agent_where(agent_scope)
    row = conn.execute(
        f"""SELECT COUNT(DISTINCT project_key)
            FROM sessions {where}
            AND ended_at >= date('now', '-{days} days')""",
        params,
    ).fetchone()
...
def _count_active_projects_window(
    ...
    Args:
        conn: SQLite connection used by the dashboard presenter.
        agent_scope: Normalized agent scope; ``all`` keeps all agents.
        start_days_ago: Inclusive lower boundary measured backward from today.
        end_days_ago: Exclusive upper boundary measured backward from today.

    Returns:
        Count of distinct project keys active in the requested relative window.
    """
    where, params = _build_agent_where(agent_scope)
    upper_bound = f"AND ended_at < date('now', '-{end_days_ago} days')" if end_days_ago > 0 else ''
    row = conn.execute(
...
def _count_new_projects(conn: sqlite3.Connection, agent_scope: str, days: int) -> int:
    ...
    Returns:
        Count of distinct project keys whose first session start falls inside
        the window.
    """
    where, params = _build_agent_where(agent_scope)
    row = conn.execute(
...
def _count_today_sessions(conn: sqlite3.Connection, agent_scope: str) -> int:
    ...
    Returns:
        Count of sessions whose ``started_at`` date is today.
    """
    where, params = _build_agent_where(agent_scope)
    row = conn.execute(
...
def _avg_daily_sessions(conn: sqlite3.Connection, agent_scope: str, days: int) -> float:
    ...
    Args:
        conn: SQLite connection used by the dashboard presenter.
        agent_scope: Normalized agent scope; ``all`` keeps all agents.
        days: Divisor and trailing window size in days.

    Returns:
        Average sessions per day over the requested window.
    """
    where, params = _build_agent_where(agent_scope)
    row = conn.execute(
...
def _median_duration(conn: sqlite3.Connection, agent_scope: str) -> float:
    ...
    Returns:
        Median positive session duration in seconds, or ``0`` without samples.
    """
    where, params = _build_agent_where(agent_scope)
    rows = conn.execute(
...
def _count_eligible_sessions(conn: sqlite3.Connection, agent_scope: str) -> int:
    ...
    Returns:
        Count of sessions with fresh, cache-read, or cache-write tokens.
    """
    where, params = _build_agent_where(agent_scope)
    row = conn.execute(
        f"""SELECT COUNT(*)
...
def _p50_cache_ratio(conn: sqlite3.Connection, agent_scope: str) -> float | None:
    ...
    Returns:
        Median cache-read/input-side ratio across eligible sessions, or ``None``
        when no eligible sessions exist.
    """
    where, params = _build_agent_where(agent_scope)
    rows = conn.execute(
        f"""SELECT
...
def _count_low_read_sessions(conn: sqlite3.Connection, agent_scope: str) -> int:
    ...
    Returns:
        Count of cache-eligible sessions whose cache read ratio is below the
        low-reuse threshold.
    """
    where, params = _build_agent_where(agent_scope)
    row = conn.execute(
...
def _count_affected_failure_sessions(conn: sqlite3.Connection, agent_scope: str) -> int:
    ...
    Returns:
        Count of sessions containing at least one failed tool result.
    """
    where, params = _build_agent_where(agent_scope)
    row = conn.execute(
...
def _count_repeated_failure_sessions(conn: sqlite3.Connection, agent_scope: str) -> int:
    ...
    Returns:
        Count of sessions containing repeated failed tool results.
    """
    where, params = _build_agent_where(agent_scope)
    row = conn.execute(
...
def _format_duration(seconds: float) -> str:
    ...
    Args:
        seconds: Duration in seconds from aggregate or per-model metrics.

    Returns:
        Compact duration string using seconds, minutes, or hours.
    """
    if seconds <= 0:
        return '0s'
    ...
    return (
        f'{int(seconds // _SECONDS_PER_HOUR)}h '
        f'{int((seconds % _SECONDS_PER_HOUR) // _SECONDS_PER_MINUTE)}m'
    )
...
def _build_model_donut_gradient(model_rows: list[dict[str, Any]]) -> str:
    ...
    Args:
        model_rows: Model rows containing ``token_share_raw`` and color fields.

    Returns:
        CSS ``conic-gradient`` string preserving token-share ordering and using a
        neutral fallback for empty data.
    """
    if not model_rows:
    ...
    stops = []
    cursor = 0.0
    total_share = sum(max(0.0, float(row.get('token_share_raw', 0) or 0)) for row in model_rows)

    if total_share <= 0:
        share = _PERCENT_TOTAL / len(model_rows)
        for row in model_rows:
            next_cursor = min(_PERCENT_TOTAL, cursor + share)
            stops.append(f'{row.get("color", "#5b5ce2")} {cursor:.2f}% {next_cursor:.2f}%')
            cursor = next_cursor
        return 'conic-gradient(' + ', '.join(stops) + ')'

    for row in model_rows:
        share = max(0.0, float(row.get('token_share_raw', 0) or 0)) / total_share * _PERCENT_TOTAL
        next_cursor = min(_PERCENT_TOTAL, cursor + share)
        stops.append(f'{row.get("color", "#5b5ce2")} {cursor:.2f}% {next_cursor:.2f}%')
        cursor = next_cursor

    if cursor < _PERCENT_TOTAL:
...
def _qoder_file_reports_cache_metrics(file_path: str) -> bool:
    ...
    Args:
        file_path: Raw Qoder session path stored in the index.

    Returns:
        ``True`` when the raw file contains known cache metric keys; missing or
        unreadable files are treated as unreported cache metrics.
    """
    if not file_path:
        return False
    try:
...
def _compute_cache_health_series(conn: sqlite3.Connection, days: int) -> list[dict]:
    ...
    Args:
        conn: SQLite connection used by the dashboard presenter.
        days: Trailing window size in days, derived from the selected grain.

    Returns:
        Ordered daily cache-health rows. Each row contains agent-specific token
        components, average components, and Qoder cache-metric availability.
    """
    ...
    for row in rows:
        day = row['day']
        point = by_day.setdefault(f'{day}', {'date': day})
        agent = row['agent']
...
def _compute_cache_health_stats(series: list[dict], agent_scope: str) -> dict:
    ...
    Args:
        series: Ordered rows from ``_compute_cache_health_series``.
        agent_scope: Normalized dashboard scope used to pick an agent prefix or
            the average aggregate.

    Returns:
        Cache-health template payload with latest ratio, lowest ratio, and the
        unmodified series for chart rendering.
    """
    if not series:
...
def _count_agent_sessions(conn: sqlite3.Connection, db_agent: str) -> int:
    ...
    Args:
        conn: SQLite connection used by the dashboard presenter.
        db_agent: Database agent key such as ``claude_code``.

    Returns:
        Total sessions for the agent, or ``0`` if the count cannot be coerced.
    """
    try:
        row = conn.execute(
            'SELECT COUNT(*) FROM sessions WHERE agent = ?',
            [db_agent],
        ).fetchone()
...
def _build_agent_where(agent_scope: str) -> tuple[str, list]:
    ...
    Args:
        agent_scope: Normalized dashboard scope from query controls.

    Returns:
        SQL WHERE prefix and positional parameters for the agent filter.
    """
    db_agent = _DB_AGENT.get(agent_scope)
    if db_agent:
...
def _compute_all_agents_branch(  # noqa: PLR0912, PLR0915 - preserves branch contract
    conn: sqlite3.Connection,
) -> dict:
    """计算 data,用于 All agents mode: contribution comparison, 所有 agents table, efficiency.

    Args:
        conn: SQLite connection used to query agent, prompt, and model aggregates.

    Returns:
        Template payload for the all-agent branch. Agent rows are ordered Claude
        Code, Qoder, Codex; efficiency rows are sorted by session count
        descending and retain raw values for client-side sorting.
    """
    ...
    for day_data in total_prompts_data:
        for db_a, key in [
            ('claude_code', 'claude_prompts'),
            ('codex', 'codex_prompts'),
            ('qoder', 'qoder_prompts'),
        ]:
            if db_a in agent_map:
                agent_map[db_a]['prompts'] = agent_map.get(db_a, {}).get(
                    'prompts', 0
                ) + day_data.get(key, 0)
        ...
        if last_active:
            try:
                ...
                diff = (
                    ...
                    if dt.tzinfo
                    else (datetime.utcnow() - dt.replace(tzinfo=None)).total_seconds()
                )
                if diff < _SECONDS_PER_MINUTE:
                    last_active_display = 'Just now'
                elif diff < _SECONDS_PER_HOUR:
                    last_active_display = f'{int(diff // _SECONDS_PER_MINUTE)} min ago'
                elif diff < _SECONDS_PER_DAY:
                    last_active_display = f'{int(diff // _SECONDS_PER_HOUR)}h ago'
                else:
                    last_active_display = f'{int(diff // _SECONDS_PER_DAY)}d ago'
            except Exception:
...
def _compute_single_agent_branch(  # noqa: PLR0912, PLR0915 - preserves branch contract
    conn: sqlite3.Connection,
    db_agent: str,
    scope_key: str,
    page: int | None = None,
    ...
    Args:
        conn: SQLite connection used to query model aggregates.
        db_agent: Database agent key used to filter model rows.
        scope_key: Route/query scope key used for display labels.
        page: One-based embedded sessions page retained in the branch payload.
        page_size: Embedded sessions page size retained in the branch payload.

    Returns:
        Template payload for the single-agent branch. Model rows are sorted by
        total tokens, session rows by session count, and efficiency notes use the
        fixed thresholds defined in this module.
    """
        ...
        model_rows.append(
            {
                ...
                'avg_tokens_display': _fmt_compact(total_tokens / total_sessions)
                if total_sessions > 0
                else '0',
                'cache_read': f'{cache_ratio * 100:.1f}%' if cache_ratio is not None else 'N/A',
    ...
    for m in model_rows:
        ...
        if sessions_count >= _PRIMARY_MODEL_SESSION_THRESHOLD:
            notes.append('Primary model')
        if avg_tokens_val > _HIGH_INPUT_TOKEN_THRESHOLD:
        ...
        try:
            ...
            cache_val = 0
        if (
            cache_val < _LOW_CACHE_REUSE_PERCENT_THRESHOLD
            and sessions_count >= _LOW_SAMPLE_SESSION_THRESHOLD
        ):
        ...
        try:
            ...
            failed_val = 0
        if failed_val > _HIGH_FAILURE_PER_SESSION_THRESHOLD:
            notes.append('High failure')
        if sessions_count < _LOW_SAMPLE_SESSION_THRESHOLD:
