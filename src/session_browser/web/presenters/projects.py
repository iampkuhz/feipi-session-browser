...
def compute_projects_pagination(
    ...
    Args:
        total_count: Total rows matching the current project or session filter.
        page: Requested one-based page; values beyond the last page are clamped.
        page_size: Integer page size or ``all`` for an unbounded page.
...
def parse_project_detail_query_params(raw_params: dict[str, list[str]]) -> dict[str, Any]:
    ...
    Args:
        raw_params: Result of ``urllib.parse.parse_qs()`` for a project route.

    Returns:
        Normalized pagination, filter, session sort, sort-key echo, and trend
        grain values used by the project detail presenter.
    """
...
def _parse_dt(value: str | None) -> datetime | None:
    """Parse indexed ISO datetimes used by project trend buckets.

    Args:
        value: Timestamp string from a session row; ``Z`` is accepted as UTC.

    Returns:
        Parsed ``datetime`` or ``None`` for missing/invalid input.
    """
    if not value:
        return None
    try:
...
def _fmt_percent(numerator: float, denominator: float) -> str:
    """Format a project ratio as a one-decimal percentage.

    Args:
        numerator: Ratio numerator after coercion to a safe number.
        denominator: Ratio denominator; non-positive values render as zero.
...
def _fmt_seconds(value: float) -> str:
    """Format project duration metrics using compact units.

    Args:
        value: Duration in seconds.

    Returns:
        Duration display string using seconds, minutes, or hours.
    """
...
def _session_input_side(session: object) -> int:
    """Compute input-side tokens for a project session row.

    Args:
        session: Session summary object returned by the index layer.

    Returns:
        Sum of fresh input, cache read, and cache write tokens.
    """
    return int(
...
def _session_total_tokens(session: object) -> int:
    """Compute total tokens for a project session row.

    Args:
        session: Session summary object returned by the index layer.

    Returns:
        Input-side tokens plus output tokens.
    """
    return int(_session_input_side(session) + getattr(session, 'output_tokens', 0))


def _project_attr(project: object, name: str, default: object = 0) -> object:
    """Read a project aggregate field from dict-like or object rows.

    Args:
        project: Project stats row from the index layer.
        name: Field name to read.
        default: Value returned when the field is absent.

    Returns:
        Raw field value without coercion.
    """
...
def _safe_number(value: object, default: float = 0.0) -> float:
    """Coerce loose index values to floats for project calculations.

    Args:
        value: Numeric, boolean, or numeric string value from index rows.
        default: Fallback used when conversion fails.

    Returns:
        Float value safe for ratios and totals.
    """
...
def _project_number(project: object, name: str, default: float = 0.0) -> float:
    """Read and coerce a numeric project aggregate field.

    Args:
        project: Project stats row from the index layer.
        name: Numeric field name to read.
        default: Fallback used when the field is absent or invalid.

    Returns:
        Float value safe for arithmetic.
    """
    return _safe_number(_project_attr(project, name, default), default)


def _bucket_for_grain(dt: datetime, grain: str) -> str:
    """Return the project trend bucket label for a datetime.

    Args:
        dt: Session start timestamp.
        grain: Trend grain normalized to day, week, or month.

    Returns:
        Bucket label sorted lexicographically in chronological order.
    """
    if grain == 'month':
...
def _build_project_token_trend(sessions: list[Any], grain: str) -> dict[str, Any]:
    """Build stacked token trend data for a project detail page.

    Args:
        sessions: All sessions for the project, sorted by start time by the
            caller.
        grain: Normalized trend grain controlling bucket labels.

    Returns:
        Trend payload with points, SVG area layers, maximum total, and data
        availability flag. Points are sorted by bucket label.
    """
...
def _build_project_detail_stats(
    project: object,
    sessions: list[Any],
    grain: str,
) -> dict[str, Any]:
    """Build aggregate cards for one project detail page.

    Args:
        project: Project stats row selected by the route.
        sessions: All sessions for the project used for agent mix and trend
            calculations.
        grain: Normalized trend grain for token trend buckets.

    Returns:
        Project-detail payload with sessions, agents, tokens, cache, failure,
        agent mix, token trend, and tool-hotspot availability contracts.
    """
    ...
    for session in sessions:
        ...
        process = float(getattr(session, 'model_execution_seconds', 0) or 0) + float(
        ...
        if input_side > 0:
            eligible_sessions += 1
            if read / input_side < LOW_CACHE_READ_RATIO:
                low_read_sessions += 1
        ...
        agent_rows.append(
            {
                ...
                'tokens': tokens,
                'failed': failed,
                'session_share': count / _project_number(project, 'total_sessions', 0) * 100
                if _project_number(project, 'total_sessions', 0)
                else 0,
    ...
    return {
        'active_period': (
            f'Active: {_project_attr(project, "first_seen", "")[:10] or "N/A"} '
            f'to {_project_attr(project, "last_seen", "")[:10] or "N/A"}'
        ),
        'sessions_kpi': {
        ...
        'cache_kpi': {
            'ratio': _fmt_percent(
                _project_number(project, 'total_cache_read_tokens', 0), input_side_total
            ),
        ...
        'failure_kpi': {
            ...
            'failure_rate': _fmt_percent(
                _project_number(project, 'total_failed_tools', 0),
                _project_number(project, 'total_tool_calls', 0),
            ),
...
def build_project_detail_view_model(
    ...
    if (
        not _project_attr(pstats, 'project_name', '')
        and _project_attr(pstats, 'total_sessions', 0) == 0
    ):
        return {
            'project': pstats,
            'sessions': [],
