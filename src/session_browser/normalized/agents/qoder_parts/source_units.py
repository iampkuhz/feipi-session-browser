"""Qoder normalized source unit helpers.

Source unit 是 Qoder 归因链路的 normalized 边界. 本模块在 parser 发现 text
或 payload 来源片段时创建草稿, 并在 call 输出前提供稳定去重和 legacy
candidate 派生. 它不跨 call 推断来源.
"""
...
class QoderSourceUnitDraft:
    """Qoder source unit draft before call-scoped hydration.

    该草稿是 parser 与 call-scoped 输出之间的边界对象. 它在发现一个 Qoder
    text 或 payload 来源片段时创建, 不负责解释 accounting 字段.

    Attributes:
        origin_path: 原始 session 文件或虚拟来源路径.
        canonical_source_locator: 用于同 call 去重的规范来源定位符.
...
def text_unit(  # noqa: PLR0913
    *,
    ...
    """Create a Qoder text source-unit draft.
    ...
    Returns:
        尚未绑定 call_id 的 Qoder source-unit draft.
    """
...
def payload_unit(  # noqa: PLR0913
    *,
    ...
    """Create a Qoder payload source-unit draft.

    触发点是 parser 已经定位到结构化 payload 边界时. 函数以稳定 JSON 文本计算
    byte range, 但保留 JSON-safe payload 供后续展示使用.
...
def finalize_source_units(call_id: str, drafts: list[QoderSourceUnitDraft]) -> list[dict[str, Any]]:
    """Hydrate Qoder drafts into call-scoped source_units.
...
def _normalized_content(draft: QoderSourceUnitDraft) -> str:
    """Return the stable content string used for hashing a Qoder draft.
