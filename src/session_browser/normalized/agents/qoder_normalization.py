"""Build Qoder normalized session attribution records.

Qoder sources call this adapter after parsing local session payloads into a
SessionSummary, chat messages, tool calls, and optional warnings. The adapter
converts those parsed inputs into normalized call records, source units, tool
steps, and semantic session metadata.
"""
...
class QoderNormalizationAdapter:
    """Build Qoder call drafts and source units from parsed payloads.

    Instances are used by module-level entrypoints after raw Qoder events have been
    parsed. They emit normalized records and do not read or mutate raw session files.
    """
    ...
    def _request_units_for_call(  # noqa: PLR0913 - Source-unit attribution requires explicit call context.
        ...
        *,
        ...
        for idx, segment in enumerate(_request_segments(msg.request_full), 1):
            ...
            elif segment and not _matches_current_user(
                segment, messages_before, current_user_index
            ):
                units.append(
                    text_unit(
...
def build_qoder_normalized_session(  # noqa: PLR0913 - Public adapter mirrors parsed session payload fields.
    *,
    summary: SessionSummary,
    messages: list[ChatMessage],
    tool_calls: list[ToolCall],
    ...
    """Build Qoder normalized records from parsed session models.

    Triggered by Qoder parsers after building parsed session models. It emits normalized
    records, source units, tool steps, and warnings.
...
def parse_qoder_session_file(
    ...
    *,
    ...
    """Parse one Qoder JSONL file into normalized session records.

    Triggered by Qoder session loading for a local JSONL path. It reads raw events,
    ...
    from session_browser.sources.qoder_parts.parse import (  # noqa: PLC0415 - Lazy import keeps parser internals optional for adapter users.
...
def _qoder_instruction_paths(root: Path | None) -> list[tuple[str, Path]]:
    """Support normalized adapter processing for _qoder_instruction_paths.

    Args:
        root: Input value for normalized adapter processing.
...
def _looks_like_instruction_file(rel: str, text: str) -> bool:
    ...
    return any(name in rel_lower for name in ('agents.md', 'claude.md', 'rules')) or any(
