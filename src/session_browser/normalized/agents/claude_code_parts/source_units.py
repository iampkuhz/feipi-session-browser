"""Claude Code normalized source unit helpers.

Source unit 是 Claude Code 归因链路的 normalized 边界. 本模块在 parser 发现
text 或 payload 来源片段时创建草稿, 并在 call/catalog 输出前提供稳定去重和
legacy candidate 派生. 它不跨 call 推断来源.
"""
...
class ClaudeCodeSourceUnitDraft:
    """Claude Code source unit draft before call-scoped hydration.

    该草稿是 parser 与 catalog/call-scoped 输出之间的边界对象. 它在发现一个
    Claude Code text 或 payload 来源片段时创建, 不负责解释 accounting 字段.
...
def text_unit(  # noqa: PLR0913
    *,
    ...
    """Create a Claude Code text source-unit draft.
    ...
    Returns:
        尚未绑定 call_id 的 Claude Code source-unit draft.
    """
...
def payload_unit(  # noqa: PLR0913
    *,
    ...
    """Create a Claude Code payload source-unit draft.
...
def finalize_source_units(
    call_id: str, drafts: list[ClaudeCodeSourceUnitDraft]
) -> list[dict[str, Any]]:
    """Hydrate Claude Code drafts into call-scoped source_units.
...
def draft_to_catalog_unit(draft: ClaudeCodeSourceUnitDraft) -> dict[str, Any]:
    """Convert one Claude Code draft into a call-independent catalog entry.
...
def _draft_to_unit(call_id: str, index: int, draft: ClaudeCodeSourceUnitDraft) -> dict[str, Any]:
    """Build a call-scoped Claude Code source unit from a draft.

    Args:
        call_id: Stable call identifier.
        index: Draft position within the prepared call list.
        draft: Claude Code source-unit draft.
...
def _catalog_unit_to_call_unit(
    ...
    """Build a call-scoped Claude Code source unit from a catalog entry.
...
def _normalized_content(draft: ClaudeCodeSourceUnitDraft) -> str:
    """Return the stable content string used for hashing a Claude Code draft.
