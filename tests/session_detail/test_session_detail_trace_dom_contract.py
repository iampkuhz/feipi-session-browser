...
class TestToggleButtonContract:
    ...
    def test_toggle_has_aria_expanded(self, template_source):
        assert 'aria-expanded=' in template_source, 'Toggle button must have aria-expanded'

    @pytest.mark.contract_case('UI-SD-017')
    def test_toggle_has_aria_controls(self, template_source):
        assert 'aria-controls=' in template_source, 'Toggle button must have aria-controls'

    @pytest.mark.contract_case('UI-SD-017')
    def test_toggle_has_round_id(self, template_source):
        assert 'data-round=' in template_source, 'Toggle button must have data-round attribute'

    @pytest.mark.contract_case('UI-SD-017')
    def test_toggle_has_preview(self, template_source):
        assert 'sd-round-preview' in template_source, 'Toggle must have preview section'
        assert 'sd-round-preview__title' in template_source, 'Toggle must have preview title'
...
class TestTraceDetailSibling:
    ...
    def test_detail_outside_summary(self):
        ...
        assert '{% macro expanded_row' in source, 'expanded_row must be a separate macro'
    ...
    def test_trace_round_calls_both_macros(self):
        ...
        trace_round_section = trace_round_section[
            : trace_round_section.find('{%- endmacro %}') + len('{%- endmacro %}')
        ]
        ...
        if '{{ round_row(row) }}' in trace_round_section:
            ...
            if round_table_path.exists():
                ...
                assert (
                    '{{ round_row(row) }}' in rt_content or '_sdt_round.round_row' in rt_content
                ), 'round_table must call round_row'
                assert (
                    '{{ expanded_row(row) }}' in rt_content
                    or '_sdt_round.expanded_row' in rt_content
...
class TestNoNestedButtonConflict:
    ...
    def test_row_clickable_for_toggle(self, template_source):
        ...
        assert len(matches) >= 1, (
            f'Expected at least 1 data-trace-round-row for row-click toggle, found {len(matches)}'
        )
    ...
    def test_no_open_column(self, template_source):
        ...
        assert 'col-open' not in template_source, 'Open column should be removed from colgroup'
...
class TestTraceRowGridStructure:
    ...
    def test_has_round_id_cell(self, template_source):
        assert 'sd-round-id' in template_source, 'Must have round ID cell'

    @pytest.mark.contract_case('UI-SD-017')
    def test_has_status_cell(self, template_source):
        assert 'round_status(' in template_source, 'Must have status cell via macro'

    @pytest.mark.contract_case('UI-SD-017')
    def test_has_preview_cell(self, template_source):
        assert 'sd-round-preview' in template_source, 'Must have preview cell'

    @pytest.mark.contract_case('UI-SD-017')
    def test_has_metric_cells(self, template_source):
        assert 'sd-round-metric' in template_source, 'Must have metric cells'

    @pytest.mark.contract_case('UI-SD-017')
    def test_has_token_mix(self, template_source):
        assert 'sd-round-mix' in template_source, 'Must have token mix bar'
...
class TestNoIllegalNestedButtons:
    ...
    def test_trace_row_is_tr_not_button(self, template_source):
        ...
        pattern = re.compile(r'<(button|div|tr)\b[^>]*data-trace-round-row', re.DOTALL)
        matches = pattern.findall(template_source)
...
class TestJSEventDispatchOnDataAction:
    r"""JS must dispatch toggle on data-action=\"toggle-round\"."""
    ...
    def test_toggle_round_action_in_js(self):
        js = self._js_source()
        assert 'toggle-round' in js, 'JS must reference toggle-round action'

    @pytest.mark.contract_case('UI-SD-017')
    def test_toggle_round_function_exists(self):
        js = self._js_source()
        assert 'toggleRound' in js, 'JS must contain toggleRound function'
    ...
    def test_js_uses_data_action_dispatch(self):
        ...
        assert (
            'action ===' in js
            or 'action ===' in js
            or 'dataset.action' in js
            or 'getAttribute("data-action")' in js
...
class TestCSSStyles:
    ...
    def test_css_has_trace_table(self):
        css = self._css_source()
        assert '.trace-table' in css, 'CSS must include .trace-table styles'

    @pytest.mark.contract_case('UI-SD-017')
    def test_css_has_round_row(self):
        css = self._css_source()
        assert '.round-row' in css, 'CSS must include .round-row styles'

    @pytest.mark.contract_case('UI-SD-017')
    def test_css_has_expanded_row(self):
        css = self._css_source()
        assert '.expanded-row' in css, 'CSS must include .expanded-row styles'

    @pytest.mark.contract_case('UI-SD-017')
    def test_css_has_sd_round_summary(self):
        css = self._css_source()
        assert '.sd-round-summary' in css, 'CSS must include .sd-round-summary styles'

    @pytest.mark.contract_case('UI-SD-017')
    def test_css_has_sd_round_preview(self):
        css = self._css_source()
        assert '.sd-round-preview' in css, 'CSS must include .sd-round-preview styles'

    @pytest.mark.contract_case('UI-SD-017')
    def test_css_has_sd_round_metric(self):
        css = self._css_source()
        assert '.sd-round-metric' in css, 'CSS must include .sd-round-metric styles'
