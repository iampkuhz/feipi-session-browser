...
class TestToolCountNoDuplication:
    ...
    def test_no_tools_in_preview_text(self):
        ...
        r = ConversationRound(
            ...
            assistant_msg=ChatMessage(role='assistant', content='Found the issue', timestamp=''),
    ...
    def test_empty_tools(self):
        ...
        r = ConversationRound(
            ...
            assistant_msg=ChatMessage(role='assistant', content='Hi there!', timestamp=''),
            tool_calls=[],
        )
        r.interactions = []
        apply_round_preview(r)
    ...
    def test_user_only_no_tools(self):
        """仅用户消息无工具的轮次应显示用户文本.."""
        r = ConversationRound(
            user_msg=ChatMessage(role='user', content='Please explain this code', timestamp=''),
            assistant_msg=ChatMessage(role='assistant', content='', timestamp=''),
            tool_calls=[],
        )
        r.interactions = []
    ...
    def test_preview_and_tool_summary_separated(self):
        ...
        r = ConversationRound(
            ...
            assistant_msg=ChatMessage(role='assistant', content='Response text', timestamp=''),
...
class TestPreviewDoesNotRepeatText:
    ...
    def test_preview_no_html_badges(self):
        ...
        r = ConversationRound(
            user_msg=ChatMessage(role='user', content='test', timestamp=''),
            assistant_msg=ChatMessage(role='assistant', content='answer', timestamp=''),
...
class TestTraceRowDOMContract:
    ...
    def test_template_uses_component_macros(self):
        ...
        template_path = (
            __file__.rsplit('/', 1)[0].rsplit('tests', 1)[0]
        ...
        assert 'for row in trace_rows' in content, 'Template must iterate over trace_rows'
    ...
    def test_normalized_trace_row_text_no_dup(self):
        ...
        r = ConversationRound(
            user_msg=ChatMessage(role='user', content='test', timestamp=''),
            assistant_msg=ChatMessage(role='assistant', content='Let me help', timestamp=''),
            tool_calls=[
                _tool('Read'),
                _tool('Read'),
                _tool('Bash'),
