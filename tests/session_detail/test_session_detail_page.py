...
class TestHeroArea:
    ...
    def test_hero_section_exists(self, timeline):
        ...
        assert 'data-session-overview-hero' in timeline, 'Hero missing data-session-overview-hero'
    ...
    def test_agent_pill_exists(self, timeline):
        ...
        assert 'summary.agent_label' in timeline, 'Hero must render summary.agent_label'
    ...
    def test_hero_title_exists(self, timeline):
        ...
        assert 'summary.title' in timeline, 'Hero title must use summary.title'
    ...
    def test_hero_meta_chips(self, timeline):
        ...
        assert 'summary.project_name' in timeline, 'Missing project chip in hero'
    ...
    def test_hero_session_file_path_copy(self, timeline):
        ...
        assert 'summary.session_file_path' in timeline, 'Hero must render summary.session_file_path'
    ...
    def test_kpi_tokens(self, timeline):
        ...
        assert 'metrics.tokens' in timeline, 'KPI must render metrics.tokens value'
    ...
    def test_kpi_run_health(self, timeline):
        ...
        assert 'metrics.run_health' in timeline, 'KPI must render metrics.run_health value'

    @pytest.mark.contract_case('UI-SD-001')
    def test_kpi_tools(self, timeline):
        """KPIs must include Tools."""
        assert 'Tools' in timeline, 'Missing Tools KPI label'
        assert 'metrics.tool_calls' in timeline, 'KPI must render global metrics.tool_calls value'

    @pytest.mark.contract_case('UI-SD-001')
    def test_kpi_failed(self, timeline):
        """KPIs must include Failed."""
        assert 'Failed' in timeline, 'Missing Failed KPI label'
        assert 'metrics.failed_tools' in timeline, 'KPI must render metrics.failed_tools value'

    @pytest.mark.contract_case('UI-SD-001')
    def test_summary_strip_exists(self, timeline):
        """Summary strip must exist with data-summary-strip."""
        assert 'data-summary-strip' in timeline, 'Missing data-summary-strip'
        assert 'sd-summary-strip' in timeline, 'Missing .sd-summary-strip'
    ...
    def test_issue_strip_in_hero(self, timeline):
        ...
        assert 'data-issue-strip' in timeline, 'Missing data-issue-strip in hero'
        assert 'sd-issue-strip' in timeline, 'Missing .sd-issue-strip'
    ...
    def test_issue_strip_has_jump_action(self, timeline):
        ...
        assert 'data-round=' in timeline, 'Issue links missing data-round attribute'
...
class TestTabNavigation:
    ...
    def test_tab_container_exists(self, session):
        ...
        assert 'data-session-tabs' in session, 'Missing data-session-tabs attribute'

    @pytest.mark.contract_case('UI-SD-001')
    def test_trace_tab_exists(self, session):
        r"""Trace tab must exist with data-action=\"tab-trace\"."""
        assert 'data-action="tab-trace"' in session, 'Missing Trace tab'
        assert 'data-tab="trace"' in session, 'Missing data-tab="trace"'

    @pytest.mark.contract_case('UI-SD-001')
    def test_payload_tab_is_removed(self, session):
        """Payload tab and persistent payload panel must not render."""
        assert 'data-action="tab-payload"' not in session, 'Payload tab action must be removed'
        assert 'data-tab="payload"' not in session, 'Payload top-level tab must be removed'
        assert 'data-payload-tab-panel' not in session, 'Persistent Payload panel must be removed'
    ...
    def test_trace_tab_is_active(self, session):
        ...
        tabs_section = session[
            session.find('data-session-tabs') : session.find('data-session-tabs') + 500
        ]
        assert 'is-active' in tabs_section, 'No active tab found'
        # Trace tab should be active
        assert (
            'data-action="tab-trace"'
            in tabs_section.split('is-active')[0] + tabs_section.split('is-active')[1][:200]
        ), 'Trace tab should be active by default'

    @pytest.mark.contract_case('UI-SD-001')
    def test_tab_aria_label(self, session):
        """Tab nav must have accessible aria-label."""
        assert 'aria-label="Session view tabs"' in session, 'Tab nav missing aria-label'
...
class TestTraceTableStructure:
    ...
    def test_trace_table_element(self, session):
        r"""session.html must have <table class=\"trace-table\">."""
        assert 'class="trace-table"' in session, 'Missing <table class="trace-table">'

    @pytest.mark.contract_case('UI-SD-001')
    def test_trace_table_data_attr(self, session):
        """Trace table must have data-trace-list."""
        assert 'data-trace-list' in session, 'Missing data-trace-list on table'
    ...
    def test_round_row_is_tr(self, timeline):
        r"""round_row macro must render <tr class=\"round-row\">."""
        pattern = re.compile(r'<tr\b[^>]*class="[^"]*round-row[^"]*"')
        assert pattern.search(timeline), 'round_row must render <tr class="round-row">'

    @pytest.mark.contract_case('UI-SD-001')
    def test_round_row_data_attrs(self, timeline):
        """round-row must have data-trace-round-row, data-round, data-status."""
        assert 'data-trace-round-row' in timeline, 'Missing data-trace-round-row'
        pattern = re.compile(r'data-round="\{\{\s*row\.round_id\s*\}\}"')
        assert pattern.search(timeline), 'Missing data-round template attribute'
        assert 'data-status="' in timeline, 'Missing data-status on round-row'

    @pytest.mark.contract_case('UI-SD-001')
    def test_expanded_row_is_tr(self, timeline):
        r"""expanded_row macro must render <tr class=\"expanded-row\">."""
        pattern = re.compile(r'<tr\b[^>]*class="[^"]*expanded-row[^"]*"')
        assert pattern.search(timeline), 'expanded_row must render <tr class="expanded-row">'

    @pytest.mark.contract_case('UI-SD-001')
    def test_expanded_row_data_attr(self, timeline):
        """expanded-row must have data-trace-detail."""
        assert 'data-trace-detail' in timeline, 'Missing data-trace-detail on expanded-row'
    ...
    def test_expanded_row_id_pattern(self, timeline):
        ...
        assert pattern.search(timeline), 'Missing round-{id}-detail id pattern'

    @pytest.mark.contract_case('UI-SD-001')
    def test_trace_round_calls_both_macros(self, timeline):
        """trace_round macro must call round_row and expanded_row."""
        assert '{{ round_row(row) }}' in timeline, 'trace_round must call round_row'
        assert '{{ expanded_row(row) }}' in timeline, 'trace_round must call expanded_row'

    @pytest.mark.contract_case('UI-SD-001')
    def test_session_calls_trace_round(self, session):
        """session.html must call sdt.trace_round for each row."""
        assert 'sdt.trace_round(row' in session, 'session.html must call sdt.trace_round(row, ...)'
        assert 'for row in trace_rows' in session, 'session.html must iterate over trace_rows'

    @pytest.mark.contract_case('UI-SD-001')
    def test_round_row_has_is_open_class(self, timeline):
        """round-row must support is-open class for expanded state."""
        assert 'is-open' in timeline, 'Missing is-open class support in round-row'

    @pytest.mark.contract_case('UI-SD-001')
    def test_row_clickable_for_toggle(self, timeline):
        """Round rows must be clickable to toggle (cursor:pointer on round-row)."""
        assert 'data-trace-round-row' in timeline, 'Missing data-trace-round-row on round rows'
        assert 'is-open' in timeline, 'Missing is-open class support for expanded state'
...
class TestFilterButtons:
    ...
    def test_filter_status_action(self, timeline):
        ...
        assert (
            'data-action="status-all"' in timeline or 'data-action="status-failed"' in timeline
        ), 'Must have status-all or status-failed action'

    @pytest.mark.contract_case('UI-SD-001')
    def test_status_all_button(self, timeline):
        """Must have status-all button."""
        assert 'data-action="status-all"' in timeline, 'Missing status-all button'

    @pytest.mark.contract_case('UI-SD-001')
    def test_status_failed_button(self, timeline):
        """Must have status-failed button."""
        assert 'data-action="status-failed"' in timeline, 'Missing status-failed button'

    @pytest.mark.contract_case('UI-SD-001')
    def test_status_low_cache_button(self, timeline):
        """Must have status-low-cache button."""
        assert 'data-action="status-low-cache"' in timeline, 'Missing status-low-cache button'
    ...
    def test_collapse_all_button(self, timeline):
        ...
        assert 'data-action="toggle-all"' in timeline, 'Missing toggle-all button'

    @pytest.mark.contract_case('UI-SD-001')
    def test_filter_buttons_in_seg_group(self, timeline):
        """Filter buttons must be in a segmented control group."""
        assert 'class="sd-seg"' in timeline, 'Missing sd-seg group for filter buttons'
        assert 'sd-seg-btn' in timeline, 'Missing sd-seg-btn class for filter buttons'
    ...
    def test_status_all_is_active_by_default(self, timeline):
        ...
        all_btn_match = re.search(r'<button[^>]*data-action="status-all"[^>]*>', timeline)
    ...
    def test_trace_actions_container(self, timeline):
        ...
        assert 'sd-trace-actions' in timeline, 'Missing sd-trace-actions container'
...
class TestTokenBar:
    ...
    def test_tokenbar_in_row(self, timeline):
        ...
        assert 'class="tokenbar"' in timeline, 'Missing .tokenbar in round-row'

    @pytest.mark.contract_case('UI-SD-001')
    def test_tokenbar_aria_hidden(self, timeline):
        """Tokenbar must be aria-hidden (decorative)."""
        assert 'aria-hidden="true"' in timeline, 'Tokenbar should be aria-hidden'

    @pytest.mark.contract_case('UI-SD-001')
    def test_fresh_segment(self, timeline):
        """Tokenbar must have fresh segment."""
        pattern = re.compile(r'class="fresh"')
        assert pattern.search(timeline), 'Missing fresh segment in tokenbar'
        assert '--segment-width' in timeline, 'Missing --segment-width CSS variable'

    @pytest.mark.contract_case('UI-SD-001')
    def test_read_segment(self, timeline):
        """Tokenbar must have read segment."""
        pattern = re.compile(r'class="read"')
        assert pattern.search(timeline), 'Missing read segment in tokenbar'

    @pytest.mark.contract_case('UI-SD-001')
    def test_write_segment(self, timeline):
        """Tokenbar must have write segment."""
        pattern = re.compile(r'class="write"')
        assert pattern.search(timeline), 'Missing write segment in tokenbar'

    @pytest.mark.contract_case('UI-SD-001')
    def test_out_segment(self, timeline):
        """Tokenbar must have out segment."""
        pattern = re.compile(r'class="out"')
        assert pattern.search(timeline), 'Missing out segment in tokenbar'

    @pytest.mark.contract_case('UI-SD-001')
    def test_token_mix_data(self, timeline):
        """Tokenbar segments must be driven by row.token_mix."""
        assert 'row.token_mix.fresh' in timeline, 'Missing token_mix.fresh data binding'
        assert 'row.token_mix.read' in timeline, 'Missing token_mix.read data binding'
        assert 'row.token_mix.write' in timeline, 'Missing token_mix.write data binding'
        assert 'row.token_mix.out' in timeline, 'Missing token_mix.out data binding'

    @pytest.mark.contract_case('UI-SD-001')
    def test_primitives_token_bar_macro(self, primitives):
        """Primitives must define token_bar macro (3-segment variant)."""
        assert '{% macro token_bar' in primitives, 'Missing token_bar macro in primitives'
        assert 'sd-tokenbar' in primitives, 'Missing sd-tokenbar class in primitives macro'
        assert 'sd-tokenbar__fresh' in primitives, 'Missing sd-tokenbar__fresh segment'
        assert 'sd-tokenbar__cache' in primitives, 'Missing sd-tokenbar__cache segment'
        assert 'sd-tokenbar__out' in primitives, 'Missing sd-tokenbar__out segment'
...
class TestPayloadModal:
    ...
    def test_modal_element_exists(self, timeline):
        ...
        assert 'id="payload-modal"' in timeline, 'Missing id="payload-modal"'
        assert 'sd-payload-modal' in timeline, 'Missing .sd-payload-modal class'

    @pytest.mark.contract_case('UI-SD-001')
    def test_modal_title(self, timeline):
        """Modal must have title with data-payload-title."""
        assert 'data-payload-title' in timeline, 'Missing data-payload-title'
        assert 'id="payload-title"' in timeline, 'Missing payload-title id'
        assert 'aria-labelledby="payload-title"' in timeline, 'Modal missing aria-labelledby'

    @pytest.mark.contract_case('UI-SD-001')
    def test_modal_subtitle(self, timeline):
        """Modal must have subtitle area."""
        assert 'data-payload-subtitle' in timeline, 'Missing data-payload-subtitle'

    @pytest.mark.contract_case('UI-SD-001')
    def test_modal_close_button(self, timeline):
        """Modal must have close button with close-payload action."""
        assert 'data-action="close-payload"' in timeline, 'Missing close-payload action'
        assert 'sd-modal-close' in timeline, 'Missing sd-modal-close class'
    ...
    def test_modal_empty_state(self, timeline):
        ...
        assert 'data-payload-empty' in timeline, 'Missing data-payload-empty'
        assert 'sd-payload-empty-state' in timeline, 'Missing .sd-payload-empty-state'

    @pytest.mark.contract_case('UI-SD-001')
    def test_payload_source_template(self, timeline):
        """Payload sources must use <template> with data attributes."""
        assert '<template' in timeline, 'Missing <template> for payload sources'
        assert 'data-payload-source' in timeline, 'Missing data-payload-source'
        assert 'data-payload-kind' in timeline, 'Missing data-payload-kind'
        assert 'data-payload-status' in timeline, 'Missing data-payload-status'
        assert 'data-payload-size' in timeline, 'Missing data-payload-size'
    ...
    def test_open_payload_buttons(self, timeline, primitives):
        ...
        assert 'open-payload' in timeline, 'Missing open-payload action in timeline'
        assert 'data-action=' in primitives, 'Primitives button macro must support data-action'
    ...
    def test_assistant_payload_button(self, timeline):
        ...
        assert 'data-payload-kind="response"' in timeline, 'Missing data-payload-kind="response"'
    ...
    def test_result_payload_button(self, timeline):
        ...
        assert 'data-payload-kind="result"' in timeline, 'Missing data-payload-kind="result"'
...
class TestSessionPageMarkers:
    ...
    def test_trace_panel_container(self, session):
        ...
        assert 'data-trace-panel' in session, 'Missing data-trace-panel'
        assert 'sd-trace-panel' in session, 'Missing .sd-trace-panel'

    @pytest.mark.contract_case('UI-SD-001')
    def test_payload_sources_macro_call(self, session):
        """Session must call payload_sources macro."""
        assert 'sdt.payload_sources' in session, 'Missing sdt.payload_sources macro call'

    @pytest.mark.contract_case('UI-SD-001')
    def test_payload_modal_macro_call(self, session):
        """Session must call payload_modal macro."""
        assert 'sdt.payload_modal' in session, 'Missing sdt.payload_modal macro call'

    @pytest.mark.contract_case('UI-SD-001')
    def test_no_content_modal(self, session):
        """content-modal must NOT exist (已移除)."""
        assert 'id="content-modal"' not in session, 'content-modal must be removed'

    @pytest.mark.contract_case('UI-SD-001')
    def test_no_workbench_views(self, session):
        """Old workbench views must NOT exist."""
        assert 'data-workbench-view="calls"' not in session, 'calls workbench view must be removed'
