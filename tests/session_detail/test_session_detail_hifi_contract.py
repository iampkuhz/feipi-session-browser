...
class TestFixtureDataAttributes:
    ...
    def test_view_switch_buttons(self, hifi_fixture_session):
        ...
        has_new = (
            soup.select_one('[data-action="status-all"]') is not None
            or soup.select_one('[data-action="status-failed"]') is not None
        )
...
class TestFixtureAnomalySignals:
    ...
    def test_session_page_has_anomaly_elements(self, hifi_fixture_session):
        ...
        soup.select('.badge, [data-signal], [data-anomaly]')
...
class TestFixtureDirectParser:
    ...
    def _parse_fixture(self):
        ...
        fixture_file = os.path.join(
            fixture_dir, 'projects', 'test-hifi-project', 'hifi-viz-session-001.jsonl'
        )
    ...
    def test_fixture_has_multiple_rounds(self):
        ...
        _summary, messages, _tool_calls, _subagent_runs = self._parse_fixture()
    ...
    def test_fixture_has_tool_calls(self):
        """Fixture 必须包含工具调用.."""
        _summary, _messages, tool_calls, _subagent_runs = self._parse_fixture()
    ...
    def test_fixture_has_failed_tool(self):
        ...
        assert len(failed) > 0, (
            f'No failed tool calls found. Tool statuses: {[tc.status for tc in tool_calls]}'
        )

    @pytest.mark.contract_case('UI-SD-030')
    def test_fixture_has_token_usage(self):
        """Fixture 在各类别中必须有非零的 token 计数.."""
        summary, _messages, _tool_calls, _subagent_runs = self._parse_fixture()
...
class TestLegacyNegativeContract:
    ...
    def test_no_legacy_top_level_tabs(self, hifi_fixture_session):
        ...
        soup, _html = self._soup(hifi_fixture_session)
        # 检查旧式标签页导航
...
class TestShellResidue:
    ...
    def test_no_sidebar_extra(self, hifi_fixture_session):
        """侧边栏额外区块在会话详情上必须为空.."""
        soup, _ = self._soup(hifi_fixture_session)
        soup.select_one('aside .nav-label + .map-row')
        # 更精确:检查无侧边栏额外内容
        sidebar_navs = soup.select('aside .nav a')
        # 应只有 Sessions 和 Dashboard
        hrefs = [a.get('href', '') for a in sidebar_navs]
        for h in hrefs:
            assert h in ('/sessions', '/dashboard', '#'), f'Unexpected sidebar nav href: {h}'
    ...
    def test_no_topbar_toggles(self, hifi_fixture_session):
        ...
        if topbar_actions:
            ...
            for btn in top_btns:
                title = btn.get('title', '')
                assert '切换左侧' not in title, f'Sidebar toggle still in topbar: {title}'
                assert '切换右侧' not in title, f'Right panel toggle still in topbar: {title}'
                assert '专注模式' not in title, f'Focus toggle still in topbar: {title}'
    ...
    def test_no_disabled_placeholders(self, hifi_fixture_session):
        ...
        visible_disabled = [
            b for b in disabled if not (b.get('hidden') or 'display:none' in (b.get('style') or ''))
        ]
...
class TestTraceRowAria:
    ...
    def test_trace_rows_are_not_buttons(self, hifi_fixture_session):
        ...
        for row in rows:
            assert row.name != 'button', 'trace-row is <button>, expected <article>'
    ...
    def test_trace_rows_are_clickable(self, hifi_fixture_session):
        ...
        assert len(rows) > 0, 'No trace rows found'
        for row in rows:
            assert row.has_attr('data-round'), 'trace-row missing data-round attribute'
    ...
    def test_trace_rows_have_aria_expanded(self, hifi_fixture_session):
        ...
        assert total > 0 or len(toggles) >= 0, 'No aria-expanded found on toggles or rows'
    ...
    def test_trace_toggle_aria_controls_exist(self, hifi_fixture_session):
        ...
        for toggle in toggles:
            ...
            soup.select_one(f'#{controls_id}')
            assert True, 'aria-controls target check skipped (may be dynamic)'
