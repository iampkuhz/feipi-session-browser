...
class TestBaseHtml:
    ...
    def test_no_inspector_removed(self, base_text):
        ...
        assert 'inspector.html' not in base_text, 'base.html must not include inspector component'
    ...
    def test_payload_modal_element_exists(self, base_text):
        ...
        assert 'id="payload-modal"' in base_text, 'payload-modal element must exist'

    @pytest.mark.contract_case('UI-SD-016')
    def test_payload_modal_has_header(self, base_text):
        """Modal must have header."""
        assert 'class="payload-modal__header"' in base_text, 'Modal must have header'

    @pytest.mark.contract_case('UI-SD-016')
    def test_payload_modal_has_title(self, base_text):
        """Modal must have title element."""
        assert 'class="payload-modal__title"' in base_text, 'Modal must have title element'

    @pytest.mark.contract_case('UI-SD-016')
    def test_payload_modal_has_rendered_tab(self, base_text):
        """Modal must have Rendered tab."""
        assert 'data-mode="rendered"' in base_text, 'Modal must have Rendered tab'

    @pytest.mark.contract_case('UI-SD-016')
    def test_payload_modal_has_raw_tab(self, base_text):
        """Modal must have Raw tab."""
        assert 'data-mode="raw"' in base_text, 'Modal must have Raw tab'

    @pytest.mark.contract_case('UI-SD-016')
    def test_payload_modal_has_close_button(self, base_text):
        """Modal must have close button."""
        assert 'data-action="close-modal"' in base_text, 'Modal must have close button'

    @pytest.mark.contract_case('UI-SD-016')
    def test_payload_modal_has_rendered_section(self, base_text):
        """Modal must have rendered section."""
        assert 'class="payload-modal__rendered"' in base_text, 'Modal must have rendered section'

    @pytest.mark.contract_case('UI-SD-016')
    def test_payload_modal_has_raw_section(self, base_text):
        """Modal must have raw section."""
        assert 'class="payload-modal__raw"' in base_text, 'Modal must have raw section'
...
class TestSessionHtml:
    ...
    def test_extends_base(self, session_text):
        ...
        assert '{% extends' in session_text, 'session.html does not extend base.html'

    @pytest.mark.contract_case('UI-SD-016')
    def test_shell_class_declares_sd_shell(self, session_text):
        """session.html shell_class must include sd-shell."""
        assert 'sd-shell' in session_text, 'session.html shell_class lacks sd-shell'
    ...
    def test_session_has_trace_page_marker(self, session_text):
        ...
        assert 'data-trace-page' in session_text, 'session.html lacks data-trace-page'

    @pytest.mark.contract_case('UI-SD-016')
    def test_hero_title_hook(self, session_text):
        """session.html must have hero title via sdt macro."""
        assert 'sdt.hero' in session_text, 'session.html lacks sdt.hero macro call'

    @pytest.mark.contract_case('UI-SD-016')
    def test_kpi_metrics_hook(self, session_text):
        """session.html must have KPI / metrics via sdt macro."""
        assert 'hero_metrics' in session_text, 'session.html lacks hero_metrics hook'

    @pytest.mark.contract_case('UI-SD-016')
    def test_trace_round_hook(self, session_text):
        """session.html must have trace_round macro call."""
        assert 'sdt.trace_round' in session_text, 'session.html lacks sdt.trace_round macro call'

    @pytest.mark.contract_case('UI-SD-016')
    def test_no_old_workbench_views(self, session_text):
        """Calls and Hotspots workbench views should be removed."""
        assert 'data-workbench-view="calls"' not in session_text, 'calls view should be removed'
        assert 'data-workbench-view="hotspots"' not in session_text, (
            'hotspots view should be removed'
        )
        assert 'data-workbench' not in session_text, 'workbench container should be removed'

    @pytest.mark.contract_case('UI-SD-016')
    def test_content_modal_element_removed(self, session_text):
        """content-modal element must be removed."""
        assert 'id="content-modal"' not in session_text, 'content-modal element must be removed'

    @pytest.mark.contract_case('UI-SD-016')
    def test_content_modal_js_removed(self, session_text):
        """Content modal JS must be removed."""
        assert 'openContentModal' not in session_text, 'openContentModal JS must be removed'
        assert 'closeContentModal' not in session_text, 'closeContentModal JS must be removed'
        assert 'switchContentView' not in session_text, 'switchContentView JS must be removed'
    ...
    def test_diagnostic_cards_order_and_context_layout_hook(self, session_text):
        ...
        assert positions == sorted(positions), 'diagnostic cards must render in v0.4 DOM order'
        assert (
            'sd-diagnostic-card--wide sd-diagnostic-card--subagents sd-diagnostic-card--agents'
            in session_text
    ...
    def test_agents_breakdown_table_copy_contract(self, session_text):
        ...
        assert (
            "{{ row.failure_label|default(row.failures ~ ' failed · ' ~ row.failure_rate) }}"
            in session_text
        )
...
class TestIssueStrip:
    ...
    def test_issue_strip_exists(self):
        source = self._source()
        assert 'data-issue-strip' in source, 'Issue strip section must exist'
    ...
    def test_issue_links_have_jump_action(self):
        source = self._source()
        assert 'data-action="jump-round"' in source, 'Issue links must have jump-round action'
        assert 'data-round=' in source, 'Issue links must have data-round attribute'
...
class TestTracePanel:
    ...
    def test_trace_panel_exists(self):
        source = self._source()
        assert 'data-trace-panel' in source, 'Trace panel must exist'

    @pytest.mark.contract_case('UI-SD-016')
    def test_has_all_failed_segmented_control(self):
        source = self._timeline()
        assert 'data-action="status-all"' in source, 'Trace must have status-all action'
        assert 'data-action="status-failed"' in source, 'Trace must have status-failed action'
        assert 'data-action="status-low-cache"' in source, 'Trace must have status-low-cache action'
    ...
    def test_has_collapse_all_button(self):
        ...
        assert 'data-action="toggle-all"' in source, 'Trace must have toggle-all button'

    @pytest.mark.contract_case('UI-SD-016')
    def test_has_trace_round_row(self):
        source = self._timeline()
        assert 'data-trace-round-row' in source, 'Trace must have trace-round-row elements'

    @pytest.mark.contract_case('UI-SD-016')
    def test_has_trace_detail(self):
        source = self._timeline()
        assert 'data-trace-detail' in source, 'Trace must have trace-detail elements'

    @pytest.mark.contract_case('UI-SD-016')
    def test_trace_row_has_status(self):
        source = self._timeline()
        assert 'data-status=' in source, 'Trace rows must have data-status attribute'

    @pytest.mark.contract_case('UI-SD-016')
    def test_uses_sdt_macros(self):
        source = self._source()
        assert 'sdt.trace_round' in source, 'Session must use sdt.trace_round macro'
        assert 'sdt.hero' in source, 'Session must use sdt.hero macro'
        assert 'sdt.trace_header' in source, 'Session must use sdt.trace_header macro'
    ...
    def test_toggle_js_in_timeline_js(self):
        ...
        assert 'toggleRound' in js, 'Must have toggleRound JS function'
...
class TestMetricsStrip:
    ...
    def test_kpis_exist(self):
        source = self._timeline()
        assert 'sd-kpis' in source, 'KPI metrics container must exist'
        assert 'sd-kpi' in source, 'KPI items must exist'

    @pytest.mark.contract_case('UI-SD-016')
    def test_has_tokens_metric(self):
        source = self._timeline()
        assert 'tokens' in source.lower(), 'Metrics must include tokens'

    @pytest.mark.contract_case('UI-SD-016')
    def test_has_rounds_metric(self):
        source = self._timeline()
        assert 'rounds' in source.lower(), 'Metrics must include rounds'
