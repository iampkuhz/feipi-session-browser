...
def test_run_analysis_hero_kpi_contract_and_global_tool_count():
    ...
    ix1 = _llm(
        0, 'one', input_tokens=500, cache_read_tokens=0, cache_write_tokens=0, output_tokens=100
    )
    ix1.tool_calls = [failed_tool]
    ix2 = _llm(
        1, 'two', input_tokens=100, cache_read_tokens=600, cache_write_tokens=200, output_tokens=200
    )
...
def test_assistant_text_payload_excludes_tool_use_blocks():
    ...
    assistant_rows = [
        item
        for item in vm['trace_rows'][0]['timeline_items']
...
def test_subagent_workload_and_breakdown_use_normalized_llm_calls():
    main_ix = _llm(
        0, 'main', input_tokens=80, cache_read_tokens=120, cache_write_tokens=20, output_tokens=40
    )
    ...
    expected_footprint = 200 + 300 + 50 + 70 + len(parent_result) // 4 + len(sub_result) // 4
    ...
    subagent_call_row = next(
        item for item in vm['diagnostics']['call_distribution'] if item['lane'] == 'subagent'
    )
...
def test_codex_spawn_agent_subagent_appears_in_breakdown_and_trace():
    ...
    subagent_item = next(
        item for item in trace_row['timeline_items'] if item.get('type') == 'subagent'
    )
    ...
    tool_item = next(
        item for item in trace_row['timeline_items'] if item.get('type') == 'tool_call'
    )
...
def test_run_analysis_template_sections_exist():
    session_html = (ROOT / 'src/session_browser/web/templates/session.html').read_text(
        encoding='utf-8'
    )
    summary_html = (
...
def test_context_budget_uses_shared_segment_bar_and_legend_colors():
    ...
    css = (ROOT / 'src/session_browser/web/static/css/session-detail/08-payload-tab.css').read_text(
        encoding='utf-8'
    )
