...
def test_modal_close_has_aria_label():
    ...
    source = _base_source()
    pattern = re.compile(r'class="payload-modal__close"[^>]*aria-label="[^"]*"')
...
def test_modal_title_has_id():
    ...
    source = _base_source()
    pattern = re.compile(r'class="payload-modal__title"[^>]*id="[^"]*"')
    if not pattern.search(source):
        pattern2 = re.compile(r'id="[^"]*"[^>]*class="payload-modal__title"')
        assert pattern2.search(source), 'Modal title must have an id attribute'
...
def test_no_visible_raw_json_pre():
    r"""Must not have a visible <pre id=\"raw-json\"> in normal page flow."""
    source = _session_source()
    pre_pattern = re.compile(r'<pre\s+id="raw-json"')
    pre_matches = pre_pattern.findall(source)
    assert len(pre_matches) == 0, f'Found visible <pre id="raw-json"> in template: {pre_matches}'


@pytest.mark.contract_case('UI-SD-029')
def test_raw_json_moved_to_script_tag():
    r"""Raw JSON data must be in a <script type=\"application/json\"> tag."""
    source = _session_source()
    script_pattern = re.compile(r'<script\s+type="application/json"\s+id="raw-json"')
    if not script_pattern.search(source):
        script_pattern2 = re.compile(r'<script\s+id="raw-json"\s+type="application/json"')
...
def test_filter_buttons_exist():
    ...
    chips_all = re.findall(r'<button[^>]*data-action="status-all"[^>]*>', timeline)
    chips_failed = re.findall(r'<button[^>]*data-action="status-failed"[^>]*>', timeline)
    chips_low_cache = re.findall(r'<button[^>]*data-action="status-low-cache"[^>]*>', timeline)
    assert len(chips_all) > 0 and len(chips_failed) > 0 and len(chips_low_cache) > 0, (
        'Filter status chips must exist'
    )
...
def test_payload_tabs_have_aria_selected():
    ...
    source = _base_source()
    tabs = re.findall(r'<button[^>]*data-action="payload-mode"[^>]*>', source)
    for tab in tabs:
        assert 'aria-selected=' in tab, f'Payload tab must have aria-selected: {tab[:120]}'
...
def test_css_has_issue_strip_styles():
    ...
    css = _read_css()
    assert 'sd-issue-strip' in css or 'sd-issue-link' in css, 'CSS must include issue strip styles'
