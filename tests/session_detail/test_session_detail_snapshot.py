...
def _restore_env(old_data_dir: str) -> None:
    if old_data_dir:
...
def _build_view_model():
    ...
    try:
        ...
        # Tool 调用
        tool_names = sorted({tc.name for tc in tool_calls})
        failed_tools = sorted({tc.name for tc in tool_calls if tc.is_failed})
        ...
        input_side_total = (
            summary.fresh_input_tokens + summary.cache_read_tokens + summary.cache_write_tokens
        )
...
class TestSessionDetailSnapshot:
    ...
    def test_round_details(self, snapshot):
        ...
        for i, (actual_r, expected_r) in enumerate(
            zip(snapshot['rounds']['rounds'], expected['rounds']['rounds'], strict=False)
        ):
            for key in (
                'tool_count',
                'total_tokens',
    ...
    def test_subagent_run_details(self, snapshot):
        ...
        for actual_run, expected_run in zip(
            snapshot['subagent_runs']['runs'], expected['subagent_runs']['runs'], strict=False
        ):
    ...
    def test_session_anomaly_count(self, snapshot):
        ...
        expected = _load_expected()
        assert (
            snapshot['anomalies']['session_anomaly_count']
            == expected['anomalies']['session_anomaly_count']
        )
    ...
    def test_round_signal_count(self, snapshot):
        ...
        expected = _load_expected()
        assert (
            snapshot['round_signals']['total_signal_count']
            == expected['round_signals']['total_signal_count']
        )
    ...
    def test_round_signals_by_round(self, snapshot):
        ...
        for round_key in expected_by_round:
            ...
            for act_sig, exp_sig in zip(actual_signals, expected_signals, strict=False):
    ...
    def test_summary_message_counts(self, snapshot):
        ...
        for key in (
            ...
            assert snapshot['summary'][key] == expected['summary'][key], f"Summary '{key}' mismatch"
