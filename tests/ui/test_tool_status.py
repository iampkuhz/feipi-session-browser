...
class TestClaudeToolResultHeuristic:
    ...
    def test_git_fatal_error_is_failed(self):
        """Git 致命错误应判定为失败。"""
        assert self._looks_failed(
            'fatal: not a git repository (or any of the parent directories)', tool_name='Bash'
        )
...
class TestClaudeExtractToolCalls:
    ...
    def test_bash_exit_code_1_not_failed(self):
        ...
        events = [
            {
                'type': 'user',
                'message': {'role': 'user', 'content': 'run lint'},
    ...
    def test_api_error_in_result_is_failed(self):
        ...
        events = [
            {
                'type': 'user',
                'message': {'role': 'user', 'content': 'call api'},
                'timestamp': '2026-01-01T00:00:00Z',
            },
            {
                'type': 'assistant',
