...
def _make_env() -> jinja2.Environment:
    ...
    env.filters['format_duration'] = lambda seconds: (
        f'{int(seconds // 3600)}h {int((seconds % 3600) // 60)}min'
        if seconds >= 3600
        else f'{int(seconds // 60)}min {int(seconds % 60)}s'
        if seconds >= 60
...
class TestMacroMinimalRender:
    ...
    def test_popover_renders(self):
        html = _render_macro(
            'popover', content={'title': 'Menu', 'body': 'Items'}, trigger_element='...'
        )
        assert 'Menu' in html
    ...
    def test_session_row_hides_columns(self):
        session = _make_session_stub()
        html = _render_macro(
            'session_row', _context={'session': session}, show_project=False, show_agent=False
        )
    ...
    def test_session_row_extra_classes(self):
        session = _make_session_stub()
        html = _render_macro(
            'session_row', _context={'session': session}, extra_classes='custom-class'
        )
...
class TestAriaAttributes:
    ...
    def test_button_has_aria_label(self):
        ...
        html = _render_macro(
            'button', label='X', data_action='delete-item', aria_label='Delete item'
        )
...
class TestUIPrimitivesJS:
    ...
    def test_valid_syntax(self, capsys):
        ...
        result = subprocess.run(
            ['node', '--check', str(self.JS_PATH)], capture_output=True, text=True
        )
