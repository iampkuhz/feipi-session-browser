...
def test_request_payload_has_required_fields():
    ...
    assert (
        payload['usage_summary']['request_content_denominator']['value']
        == payload['coverage']['request_content_denominator']
    )
...
def test_request_bucket_taxonomy_normalizes_labels_and_color_keys_across_agents():
    ...
    assert {bucket['agent_label'] for bucket in tool_buckets} == {'工具定义'}
