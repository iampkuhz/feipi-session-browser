...
def populate_index(claude_data_dir: Path, index_dir: Path) -> str | None:
    ...
    if launcher is None:
        return 'Java CLI not built; run ./gradlew :java:app-cli:installDist'
    env = os.environ.copy()
    env['CLAUDE_DATA_DIR'] = str(claude_data_dir)
    proc = subprocess.run(
        [str(launcher), 'scan', '--full', '--index-dir', str(index_dir)],
        cwd=SB_ROOT,
        env=env,
        text=True,
    ...
    if proc.returncode != 0:
        return f'fixture scan failed: {(proc.stdout or "").strip()[-2000:]}'
