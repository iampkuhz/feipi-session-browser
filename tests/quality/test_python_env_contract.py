...
def test_resolve_python_order_prefers_env_then_venv_then_fallback(monkeypatch, tmp_path):
    venv_python = tmp_path / ".venv" / "bin" / "python"
    venv_python.parent.mkdir(parents=True)
    venv_python.write_text("", encoding="utf-8")
    selected = []

    def fake_supports(candidate):
    ...
    monkeypatch.setattr(python_env, "_supports_python_version", lambda candidate: candidate == "/tmp/project-python")

    assert python_env.resolve_python(tmp_path) == "/tmp/project-python"


@pytest.mark.contract_case("HOOK-HARNESS-010")
def test_lock_check_requires_requirements_pyproject_and_locks_to_match(tmp_path):
...
@pytest.mark.contract_case("HOOK-HARNESS-010")
def test_lock_check_rejects_unpinned_lock_entries(tmp_path):
    _write_project(tmp_path)
...
@pytest.mark.contract_case("HOOK-HARNESS-010")
def test_quality_gate_project_python_uses_shared_resolver(monkeypatch, tmp_path):
