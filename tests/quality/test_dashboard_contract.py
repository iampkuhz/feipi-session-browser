"""Dashboard JS/CSS contract test.
...
class TestDashboardJSContract:
    ...
    _BANNED_KEYWORDS: ClassVar[tuple[str, ...]] = (
        'density-toggle',
        'settingsDrawer',
        'settings-drawer',
        'chart-export',
    ...
    def test_js_no_banned_keywords(self, keyword: str):
...
class TestDashboardCSSContract:
    ...
    _SHARED_SELECTORS: ClassVar[tuple[str, ...]] = (
        '.btn {',
        '.btn--primary {',
        '.badge {',
        '.badge--danger {',
    ...
    def test_css_no_shared_selectors(self, selector: str):
    ...
    def test_average_series_uses_neutral_color(self):
        ...
        assert all(
            'var(--brand)' not in block and 'var(--purple)' not in block for block in average_blocks
        )
    ...
    def test_scope_selector_agent_active_colors_use_agent_tokens(self):
        ...
        css = _read(_CSS_PATH)
        expected = {
            r'\.scope-selector__btn\[data-scope=\"claude-code\"\]\.is-active': ('--agent-claude'),
            r'\.scope-selector__btn\[data-scope=\"qoder\"\]\.is-active': ('--agent-qoder'),
            r'\.scope-selector__btn\[data-scope=\"codex\"\]\.is-active': ('--agent-codex'),
        }
        for selector, token in expected.items():
            pattern = selector + r'\s*\{[^}]*background:\s*var\(' + re.escape(token) + r'\);'
        ...
        assert scope_blocks
        assert all(
            'var(--brand)' not in block and 'var(--purple)' not in block for block in scope_blocks
        )
    ...
    def test_prompt_activity_line_uses_bar_center_coordinates(self):
        ...
        body = js[start:end]
        assert (
            'linePath(data, maxAvg, function(d, i) { return avgValues[i]; }, xBandCenterPct)'
            in body
        )
    ...
    def test_line_path_connects_across_missing_points(self):
        ...
        body = js[start:end]
        missing_branch = body[
            body.index('if (val == null || !isFinite(val))') : body.index('var cmd = open')
        ]
...
class TestDashboardTemplateContract:
    ...
    _BANNED_PHRASES: ClassVar[tuple[str, ...]] = (
        'Hot Sessions',
        'Context Budget',
        'Dense',
        'Comfortable',
    ...
    def test_template_no_banned_phrases(self, phrase: str):
