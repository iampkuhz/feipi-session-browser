...
# 自动发现所有 fixture 对.
FIXTURE_PAIRS = sorted(p.stem for p in FIXTURES_DIR.glob('*.jsonl'))


def _serialize_issue(issue_item: ParseIssueItem) -> dict:
...
def _serialize_diagnostics(diag: JsonlDiagnostics) -> dict:
...
class TestParserSnapshots:
    ...
    def test_diagnostics_issues_match(self, fixture_name: str):
        ...
        for idx, (act, exp) in enumerate(zip(actual_issues, expected_issues, strict=False)):
            assert act == exp, (
