...
from session_browser.sources import claude, qoder
...
class TestReturnValueContracts:
    ...
    def test_claude_parse_session_detail_returns_correct_types(self):
        """parse_session_detail must return expected claude adapter types.

        Returns (SessionSummary, list[ChatMessage],
    ...
    def test_codex_parse_session_detail_returns_correct_types(self):
        """parse_session_detail must return expected codex adapter types.
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                summary, messages, tool_calls, subagent_runs = codex.parse_session_detail(
                    'nonexistent-session'
                )
    ...
    def test_qoder_parse_session_detail_returns_correct_types(self):
        """parse_session_detail must return expected qoder adapter types.
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                summary, messages, tool_calls, subagent_runs = qoder.parse_session_detail(
                    'test-proj', 'nonexistent-session'
                )
...
class TestBadJsonTolerance:
    ...
    def test_completely_unparseable_lines(self):
        ...
        try:
            events, diag = parse_jsonl_events(p)
            assert len(events) == EXPECTED_VALID_EVENTS
    ...
    def test_claude_adapter_with_bad_json_does_not_crash(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # Must not raise
                summary, _messages, _tool_calls, _subagent_runs = claude.parse_session_detail(
                    'test-proj', 'sid-bad'
                )
                assert isinstance(summary, SessionSummary)
            finally:
