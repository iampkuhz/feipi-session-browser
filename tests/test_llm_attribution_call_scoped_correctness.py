...
def test_captured_context_fragment_excludes_current_user_message(agent):
    ...
    ro = _make_ro(user_content=user_text)
    result = build_llm_request_attribution(
        agent, lc, ro, session_context=_ctx(agent, _unit('user_input', 'request', user_text))
    )
...
def test_captured_context_fragment_excludes_preceding_tool_results(agent):
    ...
    tool_result = (
        'This is a tool result that appears in both request_full and preceding_tool_results'
    )
    lc = _make_lc(
        input_tokens=10000,
...
def test_second_llm_call_uses_only_preceding_tool_results(agent):
    ...
    tc1 = ToolCall(
        name='Read', parameters={'file_path': '/tmp/a.py'}, result='result from first call'
    )
    tc2 = ToolCall(
        name='Bash', parameters={'command': 'echo second'}, result='result from second call'
    )
...
def test_response_attribution_does_not_include_tool_result(agent):
    ...
    result = build_llm_response_attribution(
        ...
        lc,
        ro,
        session_context=_ctx(agent, _unit('assistant_output', 'response', 'This is my response')),
    )
...
def test_avail_returns_availability_row_dataclass():
    ...
    assert isinstance(row, AvailabilityRow), f'_avail 应返回 AvailabilityRow，实际为 {type(row)}'
...
def test_serialized_payload_has_availability_rows_as_dicts(agent):
    ...
    for row in payload['availability_rows']:
        assert isinstance(row, dict), f'序列化 availability row 应为 dict，实际为 {type(row)}'
...
def test_token_estimated_fields_have_exact_false():
    ...
    for row in req.availability_rows:
        ...
        if 'tokens' in field:
            ...
            assert exact is False, f"Token field '{field}' 应设置 exact=False"
