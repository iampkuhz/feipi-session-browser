...
def _default_dashboard_mocks(patchers) -> None:
...
class TestDashboardRoutePresenter:
    ...
    def test_route_presenter_is_pure_function(self):
        ...
        for forbidden in _FORBIDDEN_GLOBALS:
            assert forbidden not in source, f'Presenter references forbidden global: {forbidden}'
...
class TestProjectsRoutePresenter:
    ...
    def test_projects_list_returns_expected_keys(self):
        conn = MagicMock()
        with (
            patch('session_browser.web.presenters.projects.count_projects') as mock_count,
    ...
    def test_project_detail_returns_expected_keys(self):
        conn = MagicMock()
        with (
            patch('session_browser.web.presenters.projects.count_sessions') as mock_count,
...
class TestSessionsRoutePresenter:
    ...
    def test_sessions_context_returns_expected_keys(self):
        ...
        with (
            patch('session_browser.web.presenters.sessions.count_sessions') as mock_count,
    ...
    def test_fetch_sessions_view_model_returns_expected_keys(self):
        ...
        with (
            patch('session_browser.web.presenters.sessions.count_sessions') as mock_count,
            patch(
                'session_browser.web.presenters.sessions.get_sessions_list_aggregate'
            ) as mock_agg,
            patch('session_browser.web.presenters.sessions.list_sessions') as mock_list,
            patch(
                'session_browser.web.presenters.sessions.compute_derived_metrics'
            ) as mock_derived,
            patch('session_browser.web.presenters.sessions.detect_all_anomalies') as mock_anomalies,
            patch(
                'session_browser.web.presenters.sessions.enrich_sessions_with_anomalies'
            ) as mock_enrich,
        ):
            mock_count.return_value = 0
...
class TestPresenterReturnTypes:
    ...
    def test_project_detail_returns_dict(self):
        conn = MagicMock()
        with (
            patch('session_browser.web.presenters.projects.get_project_stats') as m_stats,
    ...
    def test_sessions_context_returns_dict(self):
        ...
        with (
            patch('session_browser.web.presenters.sessions.count_sessions') as mc,
...
class TestRouteHttp200:
    ...
    def test_dashboard_route_200(self, local_test_server):
        base_url, _agent, _session_id = local_test_server
...
class TestRoutePresenterDispatchMapping:
    ...
    def test_route_path_to_presenter_map(self):
        ...
        for route, presenter_name in mapping.items():
            assert presenter_name in funcs, (
                f'Route {route} references unknown presenter {presenter_name}'
            )
