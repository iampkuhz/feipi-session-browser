...
from session_browser.attribution.collectors.session.llm_call_locator import locate_call_boundary, find_preceding_events
from session_browser.attribution.collectors.session.current_user_message import extract_current_user_message
from session_browser.attribution.collectors.session.tool_result_extractor import extract_tool_results
...
class TestJsonlReader:
    def test_read_valid_jsonl(self):
        path = _write_jsonl([{"type": "user", "content": "hello"}, {"type": "assistant", "content": "hi"}])
        try:
