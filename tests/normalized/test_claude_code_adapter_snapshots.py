...
if TYPE_CHECKING:
    ...
    from session_browser.normalized.agents.claude_code_parts import ClaudeCodeSourceUnitDraft
...
def test_claude_code_subagent_normalized_semantics():
    ...
    assert [
        (c['call_id'], c['scope'], c['parent_call_id'], c['parent_tool_call_id'])
    ...
    assert [
        (
            t['tool_call_id'],
            t['declared_by_call_id'],
            t['result_consumed_by_call_id'],
            t.get('subagent_id', ''),
        )
        for t in actual['tool_executions']
    ] == [
    ...
    _assert_source_units(
        c1h, request={'user_input', 'runtime_context'}, response={'assistant_output', 'tool_calls'}
    )
    _assert_source_units(
        c2h,
...
def test_claude_code_away_summary_leaf_uuid_becomes_call_without_sidecar(tmp_path: Path) -> None:
...
def test_claude_code_compact_source_units_stay_linear_and_hydrate_last_call(
    ...
    def counted_draft_to_catalog_unit(draft: ClaudeCodeSourceUnitDraft) -> dict[str, object]:
