...
def test_codex_tool_loop_normalized_semantics():
    ...
    assert (
        h1['attribution_candidates']['response']['tool_calls'][0]['payload']['call_id']
        == 'call_run_tests'
    )
    assert 'tool_results' not in h1['attribution_candidates']['request']
    assert (
        h2['attribution_candidates']['request']['tool_results'][0]['payload']['call_id']
        == 'call_run_tests'
    )
...
def test_codex_subagent_rollout_links_parent_tool_and_child_calls():
    ...
    parent_spawn = next(
        tool for tool in actual['tool_executions'] if tool['tool_call_id'] == 'call_spawn'
    )
    ...
    child_tool = next(
        tool for tool in actual['tool_executions'] if tool['tool_call_id'] == 'call_child_exec'
    )
...
def test_codex_three_agent_sample_matches_recovered_call_graph():
    ...
    assert (
        request_candidates['tool_definitions'][0]['payload']['call_id']
        == 'call_XsbhHJb7vGOu1oAP5oFqgBr7'
    )
