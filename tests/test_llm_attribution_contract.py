...
def _make_round(
    ...
    return ConversationRound(
        ...
        assistant_msg=ChatMessage(
            role='assistant', content='world', timestamp='2025-01-01T00:00:00Z'
        ),
...
def test_bucket_sums_not_exceed_total_input(agent):
    lc = _make_llm_call(input_tokens=10000, output_tokens=5000, cache_read=5000, cache_write=1000)
    ro = _make_round(user_content='test')
    result = build_llm_request_attribution(agent, lc, ro)
    total = result.total_input.value or 0
    bucket_sum = sum(b.tokens for b in result.buckets)
    assert bucket_sum <= total, f'Bucket sum {bucket_sum} exceeds total {total} for {agent}'
...
def test_bucket_sums_not_exceed_total_output(agent):
    ...
    bucket_sum = sum(b.tokens for b in result.buckets if getattr(b, 'contributes_to_total', True))
    assert bucket_sum <= total, f'Bucket sum {bucket_sum} exceeds total {total} for {agent}'
