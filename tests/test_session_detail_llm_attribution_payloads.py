...
def _make_round_with_llm_call():
    ...
    user_msg = ChatMessage(
        role='user',
        content='Hello, please analyze this code.',
...
def test_qoder_attribution_payload_has_no_cache():
    ...
    assert (
        usage['cache_read']['value'] in (0, None)
        or usage['cache_read']['precision'] == 'unavailable'
    )
    assert (
        usage['cache_write']['value'] in (0, None)
        or usage['cache_write']['precision'] == 'unavailable'
    )
...
def test_codex_attribution_payload_has_no_cache():
    ...
    assert (
        usage['cache_write']['value'] is None or usage['cache_write']['precision'] == 'unavailable'
    )
