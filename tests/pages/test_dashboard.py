...
class TestDashboardTemplate:
    ...
    def test_template_file_exists(self):
        assert os.path.isfile(_DASHBOARD_PATH), f'{_DASHBOARD_PATH} 必须存在'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_extends_base(self):
        content = _read_dashboard()
        assert '{% extends "base.html" %}' in content, 'Dashboard 必须继承 base.html'
    ...
    def test_no_inline_onclick(self):
        ...
        assert len(matches) == 0, f'Dashboard 不得有 inline onclick，发现 {len(matches)} 处'
...
class TestDashboardImports:
    ...
    def test_css_import_dashboard_css(self):
        content = _read_dashboard()
        assert 'href="/static/css/dashboard.css"' in content, 'Dashboard 必须导入 dashboard.css'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_js_import_dashboard_js(self):
        content = _read_dashboard()
        assert 'src="/static/js/dashboard.js"' in content, 'Dashboard 必须导入 dashboard.js'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_css_file_exists_on_disk(self):
        assert os.path.isfile(_DASHBOARD_CSS_PATH), 'dashboard.css 必须存在于磁盘上'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_js_file_exists_on_disk(self):
        assert os.path.isfile(_DASHBOARD_JS_PATH), 'dashboard.js 必须存在于磁盘上'
...
class TestDashboardPageHead:
    ...
    def test_page_head_has_h1(self):
        content = _read_dashboard()
        assert 'Dashboard' in content, "Dashboard 必须有标题 'Dashboard'"

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_page_head_has_subtitle(self):
        content = _read_dashboard()
        assert '全局概览' in content or '单 agent 深度分析' in content, '页面头部必须有说明副标题'
...
class TestDashboardMetricCards:
    ...
    def test_metric_grid_present(self):
        content = _read_dashboard()
        assert 'class="kpi-grid"' in content, 'Dashboard 必须有 metric-grid 区域'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_four_metric_cards(self):
        content = _read_dashboard()
        # Template uses Jinja for loop for KPI cards
        assert '{% for kpi in kpis %}' in content, 'Dashboard 必须使用 {% for kpi in kpis %} 循环'
        assert 'aria-label="{{ kpi.label }}"' in content, 'Dashboard KPI card 必须有动态 aria-label'
...
class TestDashboardChartCards:
    ...
    def test_session_trend_chart(self):
        content = _read_dashboard()
        assert 'Session Trend' in content, 'Dashboard 必须有 Session Trend 图表'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_token_trend_chart(self):
        content = _read_dashboard()
        assert 'Token Trend' in content, 'Dashboard 必须有 Token Trend 图表'
    ...
    def test_chart_has_legend(self):
        ...
        for agent in ['Claude Code', 'Codex', 'Qoder']:
            assert agent in content, f"Dashboard 必须包含 agent 名称 '{agent}'"
...
class TestDashboardInfoButtons:
    ...
    def test_chart_notes(self):
        """每个图表卡片必须使用常驻说明小字。"""
        content = _read_dashboard()
        assert 'class="chart-card__note"' in content, '图表卡片必须有 chart note 小字'
        assert 'chart_notes.sessions' in content, 'Session Trend 必须使用动态 chart note'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_dashboard_removes_info_buttons(self):
        """Dashboard 不得使用旧 info 图标。"""
        content = _read_dashboard()
        assert 'icon-button--info' not in content, 'Dashboard 图表说明不得使用 info icon'
...
class TestDashboardEmptyState:
    ...
    def test_empty_state_uses_ui_macro(self):
        content = _read_dashboard()
        assert 'ui.empty_state' in content, '空状态必须使用 ui.empty_state 宏'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_empty_state_has_action_button(self):
        content = _read_dashboard()
        assert '运行 Scan' in content or 'Run Scan' in content, "空状态必须有 'Run Scan' 操作按钮"
...
class TestDashboardErrorState:
    ...
    def test_error_state_condition(self):
        content = _read_dashboard()
        assert '{% if error %}' in content, 'Dashboard 必须检查 error 变量'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_error_state_uses_ui_macro(self):
        content = _read_dashboard()
        assert 'ui.error_state' in content, '错误状态必须使用 ui.error_state 宏'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_error_state_has_dashboard_link(self):
        content = _read_dashboard()
        assert '/dashboard' in content, '错误状态必须链接回 /dashboard'
...
class TestDashboardFloatingOverlays:
    ...
    def test_tooltip_element(self):
        content = _read_dashboard()
        assert 'id="chartTooltip"' in content, 'Dashboard 必须有 chartTooltip 元素'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_info_popover_removed(self):
        content = _read_dashboard()
        assert 'id="infoPopover"' not in content, 'Dashboard 不得保留 infoPopover 元素'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_toast_element(self):
        content = _read_dashboard()
        assert 'id="toast"' in content, 'Dashboard 必须有 toast 元素'
...
class TestDashboardNoHero:
    ...
    def test_no_hero_section(self):
        content = _read_dashboard()
        assert 'class="hero"' not in content, 'Dashboard 不得有 hero 区域'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_no_hero_title(self):
        content = _read_dashboard()
        assert 'class="hero-title"' not in content, 'Dashboard 不得有 hero-title'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_no_hero_kpis(self):
        content = _read_dashboard()
        assert 'class="hero-kpis"' not in content, 'Dashboard 不得有 hero-kpis'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_no_hero_chips(self):
        content = _read_dashboard()
        assert 'class="hero-chips"' not in content, 'Dashboard 不得有 hero-chips'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_no_range_tabs(self):
        content = _read_dashboard()
        assert 'class="range-tabs"' not in content, 'Dashboard 不得有 range-tabs'
