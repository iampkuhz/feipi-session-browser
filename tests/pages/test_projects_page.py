...
class TestProjectsTemplatePathDisplay:
    ...
    def test_path_tooltip_shows_full_key(self):
        ...
        assert 'ui.project_cell(' in content, 'projects.html must use project_cell macro'
        # 验证宏模板有 data-tooltip
...
class TestProjectsTemplate:
    ...
    def test_template_file_exists(self):
        assert os.path.isfile(_PROJECTS_PATH), f'{_PROJECTS_PATH} must exist'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_extends_base(self):
        content = _read_template()
        assert '{% extends "base.html" %}' in content, 'Projects must extend base.html'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_active_page_set(self):
        content = _read_template()
        assert "active_page = 'projects'" in content, "Projects must set active_page = 'projects'"
...
class TestProjectsImports:
    ...
    def test_css_import_projects_css(self):
        content = _read_template()
        assert 'href="/static/css/projects.css"' in content, 'Projects must import projects.css'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_js_import_projects_js(self):
        content = _read_template()
        assert 'src="/static/js/projects.js"' in content, 'Projects must import projects.js'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_css_file_exists_on_disk(self):
        assert os.path.isfile(_PROJECTS_CSS_PATH), 'projects.css must exist on disk'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_js_file_exists_on_disk(self):
        assert os.path.isfile(_PROJECTS_JS_PATH), 'projects.js must exist on disk'
...
class TestProjectsPageHead:
    ...
    def test_page_head_macro_used(self):
        content = _read_template()
        assert 'ui.page_head(' in content, 'Projects must use ui.page_head() macro'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_page_head_has_h1(self):
        content = _read_template()
        assert "'Projects'" in content, "Page-head must have 'Projects' as title"
    ...
    def test_page_head_stat_pills_used(self):
        content = _read_template()
        assert 'stat_pills=' in content, 'Page-head must use stat_pills parameter for project count'
...
class TestProjectsMetricCards:
    ...
    def test_metric_grid_present(self):
        content = _read_template()
        assert 'class="metric-grid"' in content, 'Projects must have a metric-grid section'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_four_metric_cards(self):
        content = _read_template()
        cards = re.findall(r'class="metric-card"', content)
        assert len(cards) == 4, f'Projects must have exactly 4 metric cards, found {len(cards)}'
    ...
    def test_metric_card_labels(self, label):
        ...
        assert f'>{label}' in content, f"Projects must have a metric card labeled '{label}'"
    ...
    def test_metric_info_buttons(self):
        ...
        assert len(buttons) == 4, f'Projects must have 4 metric-info buttons, found {len(buttons)}'
    ...
    def test_metric_info_uses_icon_button_info_class(self):
        ...
        content = _read_template()
        assert 'icon-button--info' in content, 'Info buttons must use icon-button--info class'
...
class TestProjectsFilterCard:
    ...
    def test_search_input_with_data_search(self):
        ...
        content = _read_template()
        assert (
            'data-search="project-name"' in content
    ...
    def test_search_input_id(self):
        content = _read_template()
        assert 'id="project-search"' in content or "id='project-search'" in content, (
    ...
    def test_clear_button(self):
        content = _read_template()
        assert 'data-action="clear-search"' in content or "data_action='clear-search'" in content, (
    ...
    def test_active_filters_aria_live(self):
        content = _read_template()
        assert 'class="active-filters"' in content or 'class="filter-footer"' in content, (
...
class TestProjectsTableStructure:
    ...
    def test_table_has_id(self):
        content = _read_template()
        assert 'id="projects-table"' in content, "Projects table must have id='projects-table'"
    ...
    @pytest.mark.parametrize(
        'column', ['Project', 'Agents', 'Sessions', 'Tokens', 'Tools', 'Last Active']
    )
    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_column_headers_present(self, column):
        content = _read_template()
        assert f'<th>{column}</th>' in content or ('<th class=' in content and column in content), (
    ...
    def test_table_has_table_note(self):
        content = _read_template()
        assert 'ui.table_card(' in content, 'Table must use ui.table_card macro'
        assert 'subtitle=' in content, 'table_card call must pass subtitle parameter'
...
class TestProjectsRowStructure:
    ...
    def test_row_has_open_project_action(self):
        content = _read_template()
        assert 'data-action="open-project"' in content, "Row must have data-action='open-project'"
    ...
    def test_tokenbar_present(self):
        ...
        content = _read_template()
        assert 'ui.token_cell' in content, 'projects.html must call ui.token_cell macro'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_tokenbar_has_four_segments(self):
        macro = _read_ui_primitives()
        segs = re.findall(r'class="tokenbar-seg (fresh|read|write|out)"', macro)
        assert len(segs) >= 4, f'token_cell macro must have 4 segments, found {len(segs)}'
    ...
    def test_tokenbar_tooltip_present(self):
        macro = _read_ui_primitives()
        assert 'class="token-tooltip"' in macro, 'token_cell macro must have tooltip element'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_tokenbar_tooltip_has_breakdown(self):
        macro = _read_ui_primitives()
        assert 'Token Breakdown' in macro, 'token_cell tooltip must show token breakdown'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_tools_failed_badge(self):
        content = _read_template()
        assert 'class="badge err tools-failed"' in content, 'Failed tools must use err badge'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_relative_time_filter(self):
        content = _read_template()
        assert 'relative_time' in content, 'Last Active column must use relative_time filter'
...
class TestProjectsPagination:
    ...
    def test_pagination_has_role_navigation(self):
        content = _read_template_with_macros()
        assert 'role="navigation"' in content, "Pagination must have role='navigation'"
    ...
    def test_pagination_next_page(self):
        content = _read_template_with_macros()
        assert 'data-action="next-page"' in content, "Pagination must have data-action='next-page'"

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_pagination_page_status(self):
        content = _read_template_with_macros()
        assert 'class="page-status"' in content, 'Pagination must have page-status element'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_pagination_has_aria_label(self):
        content = _read_template_with_macros()
        assert 'aria-label="{{ label }}"' in content, 'Pagination macro must expose an aria-label'
...
class TestProjectsEmptyState:
    ...
    def test_empty_state_macro_used(self):
        content = _read_template()
        assert 'ui.empty_state' in content, 'Projects must use ui.empty_state macro'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_empty_state_has_action_button(self):
        content = _read_template()
        assert 'Run Scan' in content, "Empty state must have 'Run Scan' action button"

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_empty_state_has_icon(self):
        content = _read_template()
        # 检查内联空态条带有 state-icon 元素
        assert 'class="state-icon"' in content, 'Empty state must have state-icon element'
        assert 'state-strip' in content, 'Empty state must use state-strip class'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_empty_state_has_clear_search(self):
        content = _read_template()
        assert 'Clear Search' in content, 'Empty state must have Clear Search button'
...
class TestProjectsErrorState:
    ...
    def test_error_state_condition(self):
        content = _read_template()
        assert '{% if error %}' in content, 'Projects must check for error variable'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_error_state_uses_ui_macro(self):
        content = _read_template()
        assert 'ui.error_state' in content, 'Error state must use ui.error_state macro'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_error_state_has_dashboard_link(self):
        content = _read_template()
        assert '/dashboard' in content, 'Error state must link back to /dashboard'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_error_state_has_go_dashboard_action(self):
        content = _read_template()
        assert 'Back to Dashboard' in content, "Error state must have 'Back to Dashboard' button"
...
class TestProjectsNoStalePatterns:
    ...
    def test_no_cache_r_columns(self):
        content = _read_template()
        assert 'Cache R' not in content, 'Projects must not have Cache R column'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_no_cache_w_columns(self):
        content = _read_template()
        assert 'Cache W' not in content, 'Projects must not have Cache W column'
    ...
    def test_no_select_based_sorting(self):
        ...
        content = _read_template()
        # 不应有通过 select 元素的排序
        assert '<select' not in content, 'Projects must not use select-based sorting'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_no_inline_onclick(self):
        content = _read_template()
        matches = re.findall(r'\bonclick\s*=', content, re.IGNORECASE)
        assert len(matches) == 0, f'Projects must not have inline onclick, found {len(matches)}'
    ...
    def test_no_output_tokens_column(self):
        content = _read_template()
        assert 'Output Tokens' not in content, 'Projects must not have Output Tokens column'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_no_tools_per_round_column(self):
        content = _read_template()
        assert 'Tools/R' not in content, 'Projects must not have Tools/R column'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_no_hero_section(self):
        content = _read_template()
        assert 'class="hero"' not in content, 'Projects must not have hero section'
...
class TestProjectsDataActions:
    ...
    def test_data_action_present(self, action):
        ...
        assert f'data-action="{action}"' in content or f"data_action='{action}'" in content, (
    ...
    def test_data_action_go_dashboard(self):
        ...
        assert 'go-dashboard' in content, 'Template must have go-dashboard action'
    ...
    def test_data_action_run_scan(self):
        ...
        assert 'run-scan' in content, 'Template must have run-scan action'
...
class TestProjectsPageRender:
    ...
    def test_page_returns_200(self, projects_html):
        ...
        assert len(projects_html) > 500, 'Projects HTML must be substantial'
    ...
    def test_title_contains_projects(self, projects_html):
        """页面标题必须包含 'Projects'。"""
        assert '<title>Projects' in projects_html, "Page title must contain 'Projects'"

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_has_page_head_title(self, projects_html):
        """页面必须显示 'Projects' 作为页面标题。"""
        assert 'Projects' in projects_html, "'Projects' text must appear in rendered page"
...
class TestProjectsListRender:
    ...
    def test_metric_grid_present(self, projects_html):
        ...
        assert 'class="metric-grid"' in projects_html, 'metric-grid must be present'
    ...
    def test_project_count_positive(self, projects_html):
        """使用夹具数据时项目计数必须大于 0。"""
        match = re.search(r'class="metric-card__value mono">(\d[\d,]*)<', projects_html)
        assert match, 'Projects metric value must be a number'
        count = int(match.group(1).replace(',', ''))
        assert count > 0, f'Projects count must be > 0 with fixture data, got {count}'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_projects_count_element(self, projects_html):
        """projects-count 元素必须存在并已填充。"""
        assert 'id="projects-count"' in projects_html, 'projects-count element must be present'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_projects_table_present(self, projects_html):
        """Projects 表格必须已渲染。"""
        assert 'id="projects-table"' in projects_html, 'projects-table must be present'
...
class TestProjectsColumnHeaders:
    ...
    def test_column_header_rendered(self, projects_html, column):
        ...
        assert f'<th>{column}</th>' in projects_html or (
            '<th ' in projects_html and column in projects_html
        ), f"Table must have '{column}' column header"
    ...
    def test_project_column_not_sortable(self, projects_html):
        ...
        project_th_section = projects_html[
            projects_html.find('Project') : projects_html.find('Project') + 200
        ]
        # Project 表头本身不应携带 data-sort-key
        assert (
            'data-sort-key' not in project_th_section.split('Agents')[0]
            if 'Agents' in project_th_section
            else True
...
class TestProjectsProjectData:
    ...
    def test_project_row_has_data_name(self, projects_html):
        ...
        assert 'data-name=' in projects_html, 'Project rows must have data-name attribute'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_project_row_has_data_path(self, projects_html):
        """每个项目行必须有 data-path 属性。"""
        assert 'data-path=' in projects_html, 'Project rows must have data-path attribute'
    ...
    def test_token_cell_rendered(self, projects_html):
        """Token 条段必须在表格中渲染。"""
        assert 'tokenbar-seg' in projects_html, 'Token bar segments must be rendered'
    ...
    def test_relative_time_rendered(self, projects_html):
        ...
        assert re.search(r'\d+[dhm]+\s+ago', projects_html) or re.search(
            r'\d+mo\s+ago', projects_html
...
class TestProjectsFilterRender:
    ...
    def test_filter_card_present(self, projects_html):
        """过滤卡片必须出现在渲染后的页面中。"""
        assert 'class="card filter-card"' in projects_html, 'Filter card must be present'
    ...
    def test_clear_button_present(self, projects_html):
        """清除搜索按钮必须存在。"""
        assert 'data-action="clear-search"' in projects_html, 'Clear search button must be present'
...
class TestProjectsPaginationRender:
    ...
    def test_page_input_present(self, projects_html):
        """页面输入必须已渲染。"""
        assert 'data-action="page-input"' in projects_html, 'Page input must be present'

    @pytest.mark.contract_case('UI-PROJECTS-001')
    def test_next_page_present(self, projects_html):
        """下一页按钮必须已渲染。"""
        assert 'data-action="next-page"' in projects_html, 'Next page button must be present'
...
class TestProjectsBreadcrumb:
    ...
    def test_breadcrumb_has_dashboard_link(self, projects_html):
        """面包屑必须链接回 dashboard。"""
        assert 'href="/dashboard"' in projects_html, 'Breadcrumb must have link to /dashboard'
