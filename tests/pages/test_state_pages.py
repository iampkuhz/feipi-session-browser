...
class Test404Template:
    ...
    def test_template_file_exists(self):
        """404.html 必须存在于磁盘上。"""
        assert os.path.isfile(_404_PATH), f'{_404_PATH} 必须存在'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_extends_base(self):
        """404 必须继承 base.html。"""
        content = _read_404()
        assert '{% extends "base.html" %}' in content, '404 必须继承 base.html'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_title_block(self):
        """404 必须设置描述性的页面标题。"""
        content = _read_404()
        assert 'Not Found' in content, "404 标题必须包含 'Not Found'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_states_css_import(self):
        """404 必须通过 head_extra 块导入 states.css。"""
        content = _read_404()
        assert 'href="/static/css/states.css"' in content, '404 必须导入 states.css'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_no_page_specific_js(self):
        """404 不得导入页面级 JS（静态页面不需要 states.js）。"""
        content = _read_404()
        assert 'states.js' not in content, '404 不得导入 states.js（静态页面无需 JS）'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_head_extra_block(self):
        """404 必须使用 head_extra 块来导入 CSS。"""
        content = _read_404()
        assert '{% block head_extra %}' in content, '404 必须使用 head_extra 块'
...
class Test404Breadcrumb:
    ...
    def test_breadcrumb_dashboard_link(self):
        """面包屑必须链接到 /dashboard。"""
        content = _read_404()
        assert 'href="/dashboard"' in content, '404 面包屑必须链接到 /dashboard'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_breadcrumb_current(self):
        """面包屑必须显示当前页面标签。"""
        content = _read_404()
        assert 'Not Found' in content, "404 面包屑必须显示 'Not Found' 为当前页"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_breadcrumb_separator(self):
        """面包屑必须使用分隔符 span。"""
        content = _read_404()
        assert 'class="sep"' in content, '404 面包屑必须使用分隔符 span'
...
class Test404StatePanel:
    ...
    def test_state_panel_class(self):
        ...
        content = _read_404()
        assert 'class="state-panel"' in content, '404 必须有 state-panel 类'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_role_region(self):
        """404 的 state-panel 必须有 role='region'。"""
        content = _read_404()
        assert 'role="region"' in content, "404 的 state-panel 必须有 role='region'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_aria_live(self):
        """404 的 state-panel 必须有 aria-live='polite'。"""
        content = _read_404()
        assert 'aria-live="polite"' in content, "404 的 state-panel 必须有 aria-live='polite'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_icon(self):
        """404 必须有 .state-panel__icon 元素。"""
        content = _read_404()
        assert 'class="state-panel__icon"' in content, '404 必须有 state-panel__icon'
    ...
    def test_state_panel_icon_value(self):
        """404 图标必须显示 '404'。"""
        content = _read_404()
        assert '>404<' in content, "404 的 state-panel__icon 必须显示 '404'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_title(self):
        """404 必须有 .state-panel__title 且内容为 'Page Not Found'。"""
        content = _read_404()
        assert 'class="state-panel__title"' in content, '404 必须有 state-panel__title'
        assert 'Page Not Found' in content, "404 标题必须包含 'Page Not Found'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_desc(self):
        """404 必须有 .state-panel__desc 元素。"""
        content = _read_404()
        assert 'class="state-panel__desc"' in content, '404 必须有 state-panel__desc'
...
class Test404Navigation:
    ...
    def test_state_panel_links_nav(self):
        ...
        content = _read_404()
        assert 'class="state-panel__links"' in content, '404 必须有 state-panel__links'
    ...
    def test_dashboard_link(self):
        """404 必须链接回 /dashboard。"""
        content = _read_404()
        assert 'href="/dashboard"' in content, '404 必须有 /dashboard 链接'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_projects_link(self):
        """404 必须链接到 /projects。"""
        content = _read_404()
        assert 'href="/projects"' in content, '404 必须有 /projects 链接'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_sessions_link(self):
        """404 必须链接到 /sessions。"""
        content = _read_404()
        assert 'href="/sessions"' in content, '404 必须有 /sessions 链接'
    ...
    def test_three_nav_links(self):
        ...
        assert len(links) == 3, f'404 必须有 3 个 state-panel__link 元素，发现 {len(links)} 个'
...
class TestErrorTemplate:
    ...
    def test_template_file_exists(self):
        """error.html 必须存在于磁盘上。"""
        assert os.path.isfile(_ERROR_PATH), f'{_ERROR_PATH} 必须存在'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_extends_base(self):
        """Error 必须继承 base.html。"""
        content = _read_error()
        assert '{% extends "base.html" %}' in content, 'Error 必须继承 base.html'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_title_block(self):
        """Error 必须设置描述性的页面标题。"""
        content = _read_error()
        assert 'Error' in content, "Error 标题必须包含 'Error'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_states_css_import(self):
        """Error 必须通过 head_extra 块导入 states.css。"""
        content = _read_error()
        assert 'href="/static/css/states.css"' in content, 'Error 必须导入 states.css'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_no_page_specific_js(self):
        """Error 不得导入页面级 JS（静态页面不需要 states.js）。"""
        content = _read_error()
        assert 'states.js' not in content, 'Error 不得导入 states.js（静态页面无需 JS）'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_head_extra_block(self):
        """Error 必须使用 head_extra 块来导入 CSS。"""
        content = _read_error()
        assert '{% block head_extra %}' in content, 'Error 必须使用 head_extra 块'
...
class TestErrorBreadcrumb:
    ...
    def test_breadcrumb_dashboard_link(self):
        """面包屑必须链接到 /dashboard。"""
        content = _read_error()
        assert 'href="/dashboard"' in content, 'Error 面包屑必须链接到 /dashboard'
    ...
    def test_breadcrumb_current(self):
        ...
        content = _read_error()
        # 面包屑显示 "Error" 为当前页
        assert 'Error' in content, "Error 面包屑必须显示 'Error' 为当前页"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_breadcrumb_separator(self):
        """面包屑必须使用分隔符 span。"""
        content = _read_error()
        assert 'class="sep"' in content, 'Error 面包屑必须使用分隔符 span'
...
class TestErrorStatePanel:
    ...
    def test_state_panel_class(self):
        ...
        content = _read_error()
        assert 'class="state-panel"' in content, 'Error 必须有 state-panel 类'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_role_alert(self):
        """Error 的 state-panel 必须有 role='alert'。"""
        content = _read_error()
        assert 'role="alert"' in content, "Error 的 state-panel 必须有 role='alert'"
    ...
    def test_state_panel_icon(self):
        ...
        content = _read_error()
        assert 'class="state-panel__icon' in content, 'Error 必须有 state-panel__icon'
    ...
    def test_state_panel_icon_value(self):
        ...
        assert '!' in icon_snippet, "Error 的 state-panel__icon 必须显示 '!'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_title(self):
        """Error 必须有 .state-panel__title。"""
        content = _read_error()
        assert 'class="state-panel__title"' in content, 'Error 必须有 state-panel__title'
        assert 'Something Went Wrong' in content, "Error 标题必须包含 'Something Went Wrong'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_desc(self):
        """Error 必须有 .state-panel__desc 元素。"""
        content = _read_error()
        assert 'class="state-panel__desc"' in content, 'Error 必须有 state-panel__desc'
...
class TestErrorNavigation:
    ...
    def test_state_panel_links_nav(self):
        ...
        content = _read_error()
        assert 'class="state-panel__links"' in content, 'Error 必须有 state-panel__links'
    ...
    def test_dashboard_link(self):
        ...
        content = _read_error()
        assert 'href="/dashboard"' in content, 'Error 必须有 /dashboard 链接'
    ...
    def test_single_nav_link(self):
        ...
        assert len(links) == 1, f'Error 必须有 1 个 state-panel__link，发现 {len(links)} 个'
...
class TestErrorConditionalDetails:
    ...
    def test_jinja_if_error_block(self):
        ...
        content = _read_error()
        assert '{% if error_details %}' in content, 'Error 必须有 {% if error_details %} 条件块'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_state_panel_details(self):
        """Error 必须有 .state-panel__details 可折叠区域。"""
        content = _read_error()
        assert 'class="state-panel__details"' in content, 'Error 必须有 state-panel__details'

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_details_summary(self):
        """Error 详情必须有 summary 元素。"""
        content = _read_error()
        assert '<summary>' in content, 'Error 详情必须有 summary 元素'
        assert 'Error details' in content, "Error 详情的 summary 必须显示 'Error details'"

    @pytest.mark.contract_case('UI-VISUAL-015')
    def test_raw_error_output(self):
        """Error 必须有 .state-panel__raw 用于原始错误输出。"""
        content = _read_error()
        assert 'class="state-panel__raw"' in content, 'Error 必须有 state-panel__raw'
...
class Test404NoInlinePatterns:
    ...
    def test_no_inline_onclick(self):
        ...
        assert len(matches) == 0, f'404 不得有 inline onclick，发现 {len(matches)} 处'
    ...
    def test_no_inline_script_tags(self):
        ...
        assert len(script_tags) == 0, f'404 不得有 inline 脚本标签，发现 {len(script_tags)} 个'
    ...
    def test_no_inline_style_blocks(self):
        ...
        assert len(style_blocks) == 0, f'404 不得有 inline 样式块，发现 {len(style_blocks)} 个'
    ...
    def test_no_inline_style_attribute(self):
        ...
        assert len(inline_styles) == 0, f'404 不得有 inline 样式属性，发现 {len(inline_styles)} 处'
...
class TestErrorNoInlinePatterns:
    ...
    def test_no_inline_onclick(self):
        ...
        content = _read_error()
        matches = re.findall(r'\bonclick\s*=', content, re.IGNORECASE)
        assert len(matches) == 0, f'Error 不得有 inline onclick，发现 {len(matches)} 处'
    ...
    def test_no_inline_script_tags(self):
        ...
        content = _read_error()
        script_tags = re.findall(r'<script(?! src)[^>]*>', content)
        assert len(script_tags) == 0, f'Error 不得有 inline 脚本标签，发现 {len(script_tags)} 个'
    ...
    def test_no_inline_style_blocks(self):
        ...
        content = _read_error()
        style_blocks = re.findall(r'<style[^>]*>', content)
        assert len(style_blocks) == 0, f'Error 不得有 inline 样式块，发现 {len(style_blocks)} 个'
