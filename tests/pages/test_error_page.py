...
class Test404Page:
    ...
    def test_has_description(self, error_404_response):
        """页面必须显示错误描述。"""
        _, html = error_404_response
        assert "doesn't exist" in html or 'has been removed' in html, '404 页面必须描述该错误'
...
class Test404Template:
    ...
    def test_has_state_panel_links(self):
        ...
        html = _render_404_template()
        assert 'class="state-panel__link"' in html, '404 页面必须为导航链接使用 state-panel__link'
...
class Test500ErrorPage:
    ...
    def test_has_something_went_wrong_text(self):
        ...
        html = _render_error_template()
        assert 'Something Went Wrong' in html, "错误页面必须显示 'Something Went Wrong'"
    ...
    def test_has_error_icon(self):
        """错误页面必须显示错误图标。"""
        html = _render_error_template()
        assert 'state-panel__icon--error' in html, '错误页面必须有错误图标修饰类'
...
class TestErrorMessageDisplay:
    ...
    def test_error_message_shown_in_details(self):
        ...
        assert 'Database connection failed' in html, '错误消息必须在页面中渲染'
    ...
    def test_error_in_pre_tag(self):
        ...
        assert 'Traceback: line 42' in html, '错误消息文本必须原样显示'
    ...
    def test_no_error_details_when_none(self):
        ...
        assert '<details' not in html, '当 error 为 None 时错误详情块不应渲染'
    ...
    def test_html_escaped_in_error(self):
        ...
        assert '&lt;script&gt;' in html, '错误消息必须转义 script 标签'
        # 原始未转义的 <script>alert 不应出现在错误区域
        assert '<script>alert' not in html, '错误消息不得包含未转义的 script 标签'
...
class TestReturnToDashboard:
    ...
    def test_404_has_dashboard_link(self):
        ...
        assert '/dashboard' in html, '404 页面必须链接到 /dashboard'
        # 验证它看起来像一个正常的导航链接
        assert 'state-panel__link' in html, '404 的 Dashboard 链接必须使用 state-panel__link 类'
    ...
    def test_500_has_dashboard_link(self):
        ...
        assert 'state-panel__link' in html, '错误的 Dashboard 链接必须使用 state-panel__link 类'
    ...
    def test_404_has_primary_dashboard_label(self):
        ...
        html = _render_404_template()
        assert 'Go to Dashboard' in html, '404 页面必须提供清晰的 Dashboard 主操作'

    @pytest.mark.contract_case('UI-VISUAL-007')
    def test_500_has_primary_dashboard_label(self):
        """500 错误页面的主操作应清晰指向 Dashboard。"""
        html = _render_error_template()
        assert 'Go to Dashboard' in html, '错误页面必须提供清晰的 Dashboard 主操作'
