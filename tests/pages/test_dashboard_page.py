...
class TestDashboardPageRender:
    ...
    def test_page_returns_200(self, dashboard_html):
        """Dashboard 必须成功渲染。"""
        assert len(dashboard_html) > 500, 'Dashboard HTML 必须有足够内容'
    ...
    def test_title_contains_dashboard(self, dashboard_html):
        """页面标题必须包含 'Dashboard'。"""
        assert '<title>Dashboard' in dashboard_html, "页面标题必须包含 'Dashboard'"

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_has_h1_dashboard(self, dashboard_html):
        """页面必须有可见的 'Dashboard' 标题。"""
        # 可能在 h1 或 page-head 组件中
        assert 'Dashboard' in dashboard_html, 'Dashboard 文本必须出现在渲染页面中'
...
class TestDashboardMetrics:
    ...
    def test_metric_grid_present(self, dashboard_html):
        ...
        assert 'class="kpi-grid"' in dashboard_html, 'metric-grid 必须存在'
    ...
    def test_four_metric_cards(self, dashboard_html):
        ...
        assert len(cards) == 6, f'预期 6 个指标卡片，发现 {len(cards)} 个'
    ...
    def test_metric_values_nonzero(self, dashboard_html):
        ...
        values = re.findall(r'class="metric-card__value(?:\s[^"]*)?"[^>]*>([^<]+)<', dashboard_html)
        if '暂无已索引 session' in dashboard_html:
            pytest.fail('Fixture 未产生数据，页面渲染空态')
        assert len(values) == 6, f'预期 6 个指标值，发现 {len(values)} 个'
    ...
    def test_has_info_buttons(self, dashboard_html):
        ...
        assert 'data-kpi-tooltip=' in dashboard_html, 'Dashboard KPI 主指标必须有 tooltip target'
...
class TestDashboardCharts:
    ...
    def test_chart_title_present(self, dashboard_html, title):
        """每个图表卡片必须显示其标题。"""
        assert title in dashboard_html, f"图表 '{title}' 必须存在"
    ...
    def test_chart_containers_rendered(self, dashboard_html):
        ...
        assert len(containers) >= 2, f'预期至少 2 个图表容器，发现 {len(containers)} 个'
    ...
    def test_chart_json_data_embedded(self, dashboard_html):
        ...
        if '暂无已索引 session' in dashboard_html:
            pytest.fail('Fixture 未产生数据，页面渲染空态')
        # 查找包含图表数据的 script 标签
        assert 'id="dashboard-graph-data"' in dashboard_html, 'Dashboard 必须嵌入趋势图表 JSON 数据'
    ...
    def test_chart_has_legend(self, dashboard_html):
        ...
        assert 'Claude Code' in dashboard_html, '图例必须包含 Claude Code'
    ...
    def test_chart_subtitles(self, dashboard_html):
        ...
        for phrase in [
            ...
            assert phrase not in dashboard_html, f"图表 subtitle 文案 '{phrase}' 不应出现在模板中"
...
class TestDashboardScopeSwitch:
    ...
    def test_scope_button_labels(self, dashboard_html):
        ...
        for label in ['Day', 'Week', 'Month']:
            assert f'>{label}<' in dashboard_html, f"范围按钮 '{label}' 必须可见"
...
class TestDashboardOverlays:
    ...
    def test_tooltip_present(self, dashboard_html):
        """图表 tooltip 元素必须存在。"""
        assert 'id="chartTooltip"' in dashboard_html, 'chartTooltip 元素必须存在'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_info_popover_removed(self, dashboard_html):
        """Dashboard 不得保留旧信息 popover。"""
        assert 'id="infoPopover"' not in dashboard_html, 'infoPopover 元素不得存在'

    @pytest.mark.contract_case('ROUTE-API-005')
    def test_toast_present(self, dashboard_html):
        """Toast 通知元素必须存在。"""
        assert 'id="toast"' in dashboard_html, 'toast 元素必须存在'
