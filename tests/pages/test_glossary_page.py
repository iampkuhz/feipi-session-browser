...
class TestGlossaryTemplate:
    ...
    def test_template_file_exists(self):
        ...
        assert os.path.isfile(_GLOSSARY_PATH), f'{_GLOSSARY_PATH} 必须存在'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_extends_base(self):
        """Glossary 必须继承 base.html。"""
        content = _read_template()
        assert '{% extends "base.html" %}' in content, 'Glossary 必须继承 base.html'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_active_page_set(self):
        """Glossary 必须设置 active_page = 'glossary'。"""
        content = _read_template()
        assert "active_page = 'glossary'" in content, "Glossary 必须设置 active_page = 'glossary'"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_ui_primitives_imported(self):
        """Glossary 必须导入 ui_primitives.html。"""
        content = _read_template()
        assert 'components/ui_primitives.html' in content, 'Glossary 必须导入 ui_primitives.html'
    ...
    def test_no_inline_onclick(self):
        ...
        assert len(matches) == 0, f'Glossary 不得有 inline onclick，发现 {len(matches)} 次'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_page_specific_css_import(self):
        """Glossary 必须导入页面专属 CSS 用于术语样式。"""
        content = _read_template()
        assert 'href="/static/css/glossary.css"' in content, 'Glossary 必须导入 glossary.css'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_page_specific_js_import(self):
        """Glossary 必须导入页面专属 JS 用于术语功能。"""
        content = _read_template()
        assert 'src="/static/js/glossary.js"' in content, 'Glossary 必须导入 glossary.js'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_script_extra_block(self):
        """Glossary 必须使用 script_extra 块加载 JS。"""
        content = _read_template()
        assert '{% block script_extra %}' in content, 'Glossary 必须使用 script_extra 块'
...
class TestGlossaryHeader:
    ...
    def test_page_head_present(self):
        ...
        content = _read_template()
        assert 'ui.page_head(' in content, 'Glossary 必须使用 ui.page_head() 宏'
    ...
    def test_subtitle_text(self):
        """副标题必须按页面规约描述术语表目的。"""
        content = _read_template()
        assert 'badge 语义' in content, '副标题必须说明 badge 语义'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_breadcrumb(self):
        """面包屑必须链接到 dashboard。"""
        content = _read_template()
        assert 'href="/dashboard"' in content, '面包屑必须链接到 /dashboard'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_breadcrumb_current(self):
        """面包屑必须显示 glossary 为当前页。"""
        content = _read_template()
        assert '术语表' in content or 'Glossary' in content, '面包屑必须显示 glossary 名称'
...
class TestGlossaryMetricGrid:
    ...
    def test_metric_grid_present(self):
        ...
        content = _read_template()
        assert 'class="metric-grid"' in content, 'Glossary 必须有 metric-grid 区域'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_metric_grid_aria_label(self):
        """指标网格必须有 aria-label。"""
        content = _read_template()
        assert 'aria-label="术语页摘要指标"' in content, '指标网格必须有 aria-label'
    ...
    def test_four_metric_cards(self):
        ...
        assert len(cards) == 4, f'Glossary 必须恰好有 4 个指标卡片，发现 {len(cards)} 个'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_metric_card_token_types(self):
        """必须有 Token Types 指标卡片。"""
        content = _read_template()
        assert 'Token Types' in content, "Glossary 必须有 'Token Types' 指标卡片"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_metric_card_derived_metrics(self):
        """必须有 Derived Metrics 指标卡片。"""
        content = _read_template()
        assert 'Derived Metrics' in content, "Glossary 必须有 'Derived Metrics' 指标卡片"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_metric_card_provider_fields(self):
        """必须有 Provider Fields 指标卡片。"""
        content = _read_template()
        assert 'Provider Fields' in content, "Glossary 必须有 'Provider Fields' 指标卡片"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_metric_card_round_signals(self):
        """必须有 Round Signals 指标卡片。"""
        content = _read_template()
        assert 'Round Signals' in content, "Glossary 必须有 'Round Signals' 指标卡片"
    ...
    def test_metric_icons_present(self):
        ...
        assert len(icons) == 4, f'Glossary 必须有 4 个 metric-icon 元素，发现 {len(icons)} 个'
...
class TestGlossaryFilterCard:
    ...
    def test_search_input_present(self):
        """过滤卡片必须有搜索输入。"""
        content = _read_template()
        assert 'class="input search"' in content, "Glossary 必须有带 'input search' 类的搜索输入"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_search_input_id(self):
        """搜索输入必须有 id='glossary-search'。"""
        content = _read_template()
        assert 'id="glossary-search"' in content, "Glossary 搜索输入必须有 id='glossary-search'"
    ...
    def test_search_autocomplete_off(self):
        ...
        content = _read_template()
        assert 'autocomplete="off"' in content, "Glossary 搜索输入必须有 autocomplete='off'"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_search_placeholder(self):
        """搜索输入必须有占位文本。"""
        content = _read_template()
        assert '搜索术语' in content or 'Search' in content, 'Glossary 搜索输入必须有占位文本'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_match_count_element(self):
        """Glossary 必须有匹配计数 span。"""
        content = _read_template()
        assert 'id="glossary-match-count"' in content, 'Glossary 必须有 glossary-match-count span'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_filter_row_present(self):
        """过滤卡片必须有 filter-row。"""
        content = _read_template()
        assert 'class="filter-row"' in content, 'Glossary 必须有 filter-row'
...
class TestGlossaryEmptyState:
    ...
    def test_empty_state_present(self):
        ...
        content = _read_template()
        assert 'class="state-strip' in content, 'Glossary 必须有用于空态的 state-strip'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_empty_state_id(self):
        """空态必须有 id='glossary-empty'。"""
        content = _read_template()
        assert 'id="glossary-empty"' in content, "Glossary 空态必须有 id='glossary-empty'"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_empty_state_role_status(self):
        """空态必须有 role='status'。"""
        content = _read_template()
        assert 'role="status"' in content, "Glossary 空态必须有 role='status'"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_empty_state_aria_live(self):
        """空态必须有 aria-live='polite'。"""
        content = _read_template()
        assert 'aria-live="polite"' in content, "Glossary 空态必须有 aria-live='polite'"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_empty_state_is_hidden(self):
        """空态初始必须带 is-hidden 类。"""
        content = _read_template()
        assert 'is-hidden' in content, 'Glossary 空态必须有 is-hidden 类'
    ...
    def test_empty_state_icon_aria_hidden(self):
        ...
        assert 'class="state-icon"' in content, 'Glossary 空态必须有 state-icon'
        assert 'aria-hidden="true"' in content, "Glossary 空态图标必须有 aria-hidden='true'"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_empty_state_no_match_message(self):
        """空态必须有未匹配提示消息。"""
        content = _read_template()
        assert '没有匹配的术语' in content, 'Glossary 空态必须显示未匹配提示消息'
...
class TestGlossarySections:
    ...
    def test_expected_section_exists(self, section_name):
        """每个规约要求的分区必须存在。"""
        content = _read_template()
        assert section_name in content, f"Glossary 必须有 '{section_name}' 分区"
    ...
    def test_legacy_section_removed(self, section_name):
        """旧版额外知识库分区不得保留。"""
        content = _read_template()
        assert section_name not in content, f"Glossary 不应保留旧版分区 '{section_name}'"
    ...
    def test_section_head_elements(self):
        ...
        assert len(heads) == 5, f'Glossary 必须有 5 个 section-head 元素，发现 {len(heads)} 个'
    ...
    def test_section_title_elements(self):
        ...
        assert len(titles) == 5, f'Glossary 必须有 5 个 section-title 元素，发现 {len(titles)} 个'
    ...
    def test_section_desc_elements(self):
        ...
        assert len(subs) == 5, f'Glossary 必须恰好有 5 个 section-desc 元素，发现 {len(subs)} 个'
...
class TestGlossaryTables:
    ...
    def test_four_data_tables(self):
        ...
        assert len(tables) == 4, f'Glossary 必须恰好有 4 个 data-table 元素，发现 {len(tables)} 个'
    ...
    def test_all_tables_in_wrap(self):
        ...
        assert len(wraps) == 4, f'Glossary 必须有 4 个 table-wrap 元素，发现 {len(wraps)} 个'
    ...
    def test_table_section_exists(self, section_name):
        """每个预期的表格分区必须存在。"""
        content = _read_template()
        assert section_name in content, f"Glossary 必须有 '{section_name}' 表格分区"
    ...
    def test_token_composition_table_has_thead(self):
        ...
        content = _read_template()
        assert '<thead>' in content, 'Token composition 表格必须有 thead'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_token_composition_table_has_tbody(self):
        """Token composition 表格必须有 tbody。"""
        content = _read_template()
        assert '<tbody>' in content, 'Token composition 表格必须有 tbody'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_data_table_has_code_elements(self):
        """数据表格必须有用于公式/字段的 code 元素。"""
        content = _read_template()
        assert '<code>' in content, 'Glossary 表格必须有 code 元素'
    ...
    def test_badge_elements_in_tables(self):
        ...
        assert len(badges) >= 4, f'Glossary 必须至少有 4 个 badge 元素，发现 {len(badges)} 个'
...
class TestGlossaryDataActions:
    ...
    def test_sort_action_on_headers(self):
        ...
        assert len(sorts) == 4, f'Glossary 必须恰好有 4 个可排序列，发现 {len(sorts)} 个'
...
class TestGlossaryAccessibility:
    ...
    def test_metric_icon_emojis_aria_hidden(self):
        ...
        assert aria_hidden >= 4, f'Glossary 必须至少有 4 个 aria-hidden span，发现 {aria_hidden} 个'
    ...
    def test_empty_state_icon_aria_hidden(self):
        ...
        assert 'aria-hidden="true"' in state_icon_section, "空态图标必须有 aria-hidden='true'"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_search_input_aria_label(self):
        """搜索输入必须有 aria-label。"""
        content = _read_template()
        assert 'aria-label="Search glossary terms"' in content, 'Glossary 搜索输入必须有 aria-label'
    ...
    def test_metric_grid_aria_label(self):
        """指标网格必须有 aria-label。"""
        content = _read_template()
        assert 'aria-label="术语页摘要指标"' in content, 'Glossary 指标网格必须有 aria-label'
...
class TestGlossaryNoStalePatterns:
    ...
    def test_no_filter_bar_class(self):
        ...
        content = _read_template()
        assert 'class="filter-bar"' not in content, 'Glossary 不得有 filter-bar 类'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_no_table_scroll_class(self):
        """Glossary 不得使用 .table-scroll 类（使用 .table-wrap）。"""
        content = _read_template()
        assert 'class="table-scroll"' not in content, 'Glossary 不得有 table-scroll 类'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_no_select_elements(self):
        """无 <select> 元素。"""
        content = _read_template()
        assert '<select' not in content, 'Glossary 不得使用 select 元素'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_no_hero_section(self):
        """Glossary 不得有 hero 分区。"""
        content = _read_template()
        assert 'class="hero"' not in content, 'Glossary 不得有 hero 分区'
...
class TestGlossaryPageRender:
    ...
    def test_page_returns_200(self, glossary_html):
        """Glossary 必须成功渲染。"""
        assert len(glossary_html) > 500, 'Glossary HTML 必须有足够内容'
    ...
    def test_title_contains_glossary(self, glossary_html):
        ...
        assert '<title>' in glossary_html, 'Glossary 必须有 title 标签'
        assert (
            '术语表' in glossary_html
            or 'Glossary' in glossary_html
            or 'Token Glossary' in glossary_html
        ), '页面标题必须引用 Glossary'
...
class TestGlossaryPageDisplay:
    ...
    def test_has_page_head(self, glossary_html):
        ...
        assert 'Token Glossary' in glossary_html, "'Token Glossary' 标题必须可见"

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_has_subtitle(self, glossary_html):
        """页面必须显示关于术语的副标题。"""
        assert (
            '必要术语说明' in glossary_html
            or '术语说明' in glossary_html
            or '术语' in glossary_html
        ), '必须出现关于术语的副标题'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_has_metric_grid(self, glossary_html):
        """Glossary 必须渲染摘要指标网格。"""
        assert 'class="metric-grid"' in glossary_html, '指标网格必须已渲染'
    ...
    def test_breadcrumb_rendered(self, glossary_html):
        """Glossary 必须显示面包屑。"""
        assert 'href="/dashboard"' in glossary_html, '面包屑必须链接到 dashboard'
...
class TestGlossaryNoStalePatternsFixture:
    ...
    def test_no_filter_bar_class(self, glossary_html):
        ...
        assert 'class="filter-bar"' not in glossary_html, 'Glossary 不得有 filter-bar 类'

    @pytest.mark.contract_case('UI-GLOSSARY-001')
    def test_no_hero_section(self, glossary_html):
        """Glossary 不得有 hero 分区。"""
        assert 'class="hero"' not in glossary_html, 'Glossary 不得有 hero 分区'
