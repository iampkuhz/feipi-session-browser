...
def _make_page_env() -> jinja2.Environment:
    ...
    env.filters['format_bytes'] = lambda n: (
        ...
        if int(n) < 1024
        else f'{int(n) / 1024:.1f} KB'
        if int(n) < 1024 * 1024
        else f'{int(n) / (1024 * 1024):.1f} MB'
        if int(n) < 1024 * 1024 * 1024
        else f'{int(n) / (1024 * 1024 * 1024):.1f} GB'
    )
    env.filters['format_duration'] = lambda s: (
        f'{int(s // 3600)}h {int((s % 3600) // 60)}min'
        if s >= 3600
        else f'{int(s // 60)}min {int(s % 60)}s'
        if s >= 60
        else f'{int(s)}s'
    )
    ...
    env.filters['strip_line_numbers'] = lambda t: (
        ...
        if t and re.search(r'^\d+\t', t, flags=re.MULTILINE)
        else t
    )
    env.filters['renumber_lines'] = lambda t: t  # 测试用直通
    env.filters['display_path'] = lambda p: p or ''
    env.filters['truncate_path'] = lambda p: (
        (p or '')[:40] + '…' if p and len(p) > 40 else (p or '')
    )
                    ...
                    'agent_timelines': [
                        {
                            ...
                            'subagent_id': '',
                            'short_id': 'main',
                            'summary': '1 LLM · 24K tokens · 2 tools · 0 failures',
                            'token_rounds': [token_round],
                            'cache_line_points': '',
                            'cache_line_width': 0,
                            'cache_line_plot_width': 44,
                            'cache_line_left': 22,
                            'is_selected': True,
                        }
                    ],
