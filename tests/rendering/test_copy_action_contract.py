...
class TestUiPrimitivesCopyHandler:
    ...
    def test_handleCopy_exists(self):
        ...
        assert 'function handleCopy' in text, f'{self._js_file}: handleCopy function not found'
    ...
    def test_handleCopy_reads_data_copy_text(self):
        ...
        body = match.group(1)
        assert 'data-copy-text' in body, f'{self._js_file}: handleCopy does not read data-copy-text'
    ...
    def test_handleCopy_has_no_legacy_fallback(self):
        ...
        assert not found, (
            f'{self._js_file}: handleCopy reads non-canonical copy fallback(s): ' + ', '.join(found)
        )
...
class TestProjectsJsCopyHandlers:
    ...
    def test_no_private_copy_project_path_handler(self):
        ...
        has_direct_handler = 'copy-project-path' in text and 'addEventListener' in text
    ...
    def test_no_private_copy_session_handler(self):
        ...
        text = self._js_file.read_text(encoding='utf-8')
        has_direct_handler = 'copy-session' in text and 'addEventListener' in text
