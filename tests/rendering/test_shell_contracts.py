...
class TestBaseHtmlShellStructure:
    ...
    def test_sidebar_aside_exists(self, base_text):
        ...
        assert '<aside class="sidebar"' in base_text, 'base.html lacks <aside class="sidebar">'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_main_exists(self, base_text):
        """Main content area must have .main class."""
        assert 'class="main"' in base_text, 'base.html lacks .main class'
...
class TestBaseHtmlNavItems:
    ...
    def test_has_nav_dashboard(self, base_text):
        assert 'data-action="nav-dashboard"' in base_text, 'Missing nav-dashboard'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_has_nav_sessions(self, base_text):
        assert 'data-action="nav-sessions"' in base_text, 'Missing nav-sessions'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_has_nav_projects(self, base_text):
        assert 'data-action="nav-projects"' in base_text, 'Missing nav-projects'
    ...
    def test_nav_item_has_data_target(self, base_text):
        ...
        for target in ['dashboard', 'sessions', 'projects']:
            assert f'data-target="{target}"' in base_text, f'Missing data-target="{target}"'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_nav_list_container(self, base_text):
        """导航必须包裹在 <nav class="nav-list"> 中。"""
        assert 'class="nav-list"' in base_text, 'base.html lacks .nav-list container'
...
class TestBaseHtmlSidebarA11y:
    ...
    def test_sidebar_aria_label(self, base_text):
        ...
        assert '<aside class="sidebar" aria-label=' in base_text, 'Sidebar lacks aria-label'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_nav_aria_label(self, base_text):
        """导航容器必须包含 aria-label。"""
        assert (
            'aria-label="主导航"' in base_text or 'aria-label="Primary navigation"' in base_text
        ), 'Nav container lacks aria-label'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_icon_aria_hidden(self, base_text):
        """导航图标应设置 aria-hidden 以支持无障碍访问。"""
        assert 'aria-hidden="true"' in base_text, 'Nav icons lack aria-hidden'
...
class TestBaseHtmlTopbar:
    ...
    def test_breadcrumb_exists(self, base_text):
        """面包屑导航必须存在。"""
        assert 'class="breadcrumb' in base_text, 'Topbar lacks .breadcrumb'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_breadcrumb_aria_label(self, base_text):
        """面包屑导航必须包含 aria-label。"""
        assert (
            'aria-label="页面导航"' in base_text or 'aria-label="Page navigation"' in base_text
        ), 'Breadcrumb lacks aria-label'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_top_actions_container(self, base_text):
        """顶部操作容器必须存在。"""
        assert 'class="top-actions"' in base_text, 'Topbar lacks .top-actions container'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_topbar_actions_block(self, base_text):
        """顶栏操作块必须存在，供页面模板扩展。"""
        assert '{% block topbar_actions %}' in base_text, 'base.html lacks topbar_actions block'
...
class TestBaseHtmlBrandCard:
    ...
    def test_brand_card_exists(self, base_text):
        assert 'class="brand-card"' in base_text, 'Missing .brand-card'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_brand_title_text(self, base_text):
        assert 'Agent Run Profiler' in base_text, 'Missing brand title text'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_brand_mark_exists(self, base_text):
        assert 'class="brand-mark"' in base_text, 'Missing .brand-mark'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_brand_meta_exists(self, base_text):
        assert 'class="brand-meta"' in base_text, 'Missing .brand-meta'
...
class TestCssShellRules:
    ...
    def test_shell_rule(self, shell_text):
        ...
        assert re.search(r'\.shell\s*\{', shell_text), 'shell.css lacks .shell rule'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_sidebar_rule(self, shell_text):
        """.sidebar 规则必须存在于 shell.css 中。"""
        assert re.search(r'\.sidebar\s*\{', shell_text), 'shell.css lacks .sidebar rule'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_main_rule(self, shell_text):
        """.main 规则必须存在于 shell.css 中。"""
        assert re.search(r'\.main\s*\{', shell_text), 'shell.css lacks .main rule'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_topbar_rule(self, shell_text):
        """.topbar 规则必须存在于 shell.css 中。"""
        assert re.search(r'\.topbar\s*\{', shell_text), 'shell.css lacks .topbar rule'
    ...
    def test_top_actions_rule(self, shell_text):
        ...
        assert re.search(r'\.top-actions\s*\{', shell_text), 'shell.css lacks .top-actions rule'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_footer_rule(self, shell_text):
        """.footer 规则必须存在于 shell.css 中。"""
        assert re.search(r'\.footer\s*\{', shell_text), 'shell.css lacks .footer rule'
...
class TestBaseHtmlShellBlocks:
    ...
    def test_shell_class_block(self, base_text):
        ...
        assert '{% block shell_class %}' in base_text, 'base.html lacks shell_class block'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_content_block(self, base_text):
        """必须有 content 块。"""
        assert '{% block content %}' in base_text, 'base.html lacks content block'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_breadcrumb_block(self, base_text):
        """必须有 breadcrumb 块供页面覆写。"""
        assert '{% block breadcrumb %}' in base_text, 'base.html lacks breadcrumb block'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_sidebar_nav_block(self, base_text):
        """必须有 sidebar_nav 块。"""
        assert '{% block sidebar_nav %}' in base_text, 'base.html lacks sidebar_nav block'

    @pytest.mark.contract_case('UI-VISUAL-001', 'UI-VISUAL-002')
    def test_head_extra_block(self, base_text):
        """必须有 head_extra 块供页面引入 CSS。"""
        assert '{% block head_extra %}' in base_text, 'base.html lacks head_extra block'
