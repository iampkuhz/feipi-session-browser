...
class TestProjectDetailSearchFields:
    ...
    def test_session_rows_have_data_title(self, project_html):
        ...
        if not rows:
            pytest.fail('No session rows found in project.html tbody')
        for row in rows:
            assert 'data-title' in row, f'Session row lacks data-title attribute: {row[:120]}...'
    ...
    def test_search_input_has_expected_id(self, project_html):
        ...
        has_search = 'search' in project_html.lower() and (
            'input' in project_html.lower() or 'filter_search' in project_html.lower()
        )
        assert has_search, 'project.html lacks a search input in the table toolbar'
