...
|---|---:|---|---|---|---|---|---|---|
| UI-PROJECTS-001 | P0 | visual | 项目列表页模板（KPI + 表格 + 筛选） | 访问 `/projects`，检查 DOM | `.page-head` 含 "Projects"，`.metric-grid` 存在，`.data-table` 有项目行 | pytest | snapshot 更新条件：当项目列表布局变更时需更新快照 | `tests/pages/test_projects_page.py` |
...
| UI-PROJECTS-004 | P0 | data | Projects Presenter 数据完整 | 调用 `build_projects_view_model` | 返回含 projects 列表、aggregate、pagination，每个项目含 session_count/total_tokens | pytest | — | `tests/web/test_projects_presenter.py` |
| UI-PROJECTS-005 | P0 | visual | 项目页多视口截图（1440x900 / 1280x800 / 1180x800 / 2560x1440）| 各视口访问 `/projects` 截图 | 截图通过视觉回归，maxDiffPixelRatio <= 0.05 | Playwright | snapshot 更新条件：当 UI 布局/设计令牌/颜色变更时需更新快照 | `tests/playwright/ui-contract.spec.ts` |
| UI-PROJECTS-006 | P0 | visual | 项目详情页多视口截图 | 各视口访问 fixture project URL 截图 | 截图通过视觉回归 | Playwright | snapshot 更新条件：当项目详情页布局变更时需更新快照 | `tests/playwright/ui-contract.spec.ts` |
...
| UI-PROJECTS-009 | P1 | visual | MacBook 视口项目页冒烟 | 设置 1280x800 / 1440x900 视口访问 `/projects` | body 可见，title 含 "Projects"，`.data-table` 可见 | Playwright | — | `tests/playwright/macbook-smoke.spec.js` |
