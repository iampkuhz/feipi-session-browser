...
def _make_lc(**kwargs):
    defaults = dict(
        ...
        input_tokens=5000, output_tokens=3000, cache_read_tokens=0, cache_write_tokens=0,
        finish_reason="end_turn", content_blocks=[],
        response_full="response text", request_full="request text", tool_calls_raw="",
    )
...
def test_request_and_response_error_titles_are_distinguishable():
    ...
    req_err = attribution_error_to_payload(
        agent="claude_code", call_id="c1", round_id="R1",
        error_type="RequestError", message="request failed",
    )
    resp_err = attribution_error_to_payload(
        agent="claude_code", call_id="c1", round_id="R1",
        error_type="ResponseError", message="response failed",
    )
    # Both are error kind
