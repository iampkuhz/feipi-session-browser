...
def _user_event(text: str = '', content_override: object = None, **extra: object) -> dict:
    """构建 user 事件."""
...
class TestTextCap:
    ...
    def test_empty_string(self) -> None:
        """空字符串和 None 应被 _cap_text 归一化为空字符串."""
        assert _cap_text('') == ''
        assert _cap_text(None) == ''


class TestCountTokens:
    """验证 Qoder 快速字节启发式 token 计数的语言和空文本边界."""

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_basic_english(self) -> None:
        """英文文本应通过快速估算得到正 token 数."""
        tok = _count_tokens('hello world')
        assert tok > 0

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_chinese_text(self) -> None:
        """中文文本应通过字节启发式得到正 token 数."""
        tok = _count_tokens('你好世界')
        assert tok > 0

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_empty_text(self) -> None:
        """空文本应返回最小 token 数 1, 避免零输出破坏估算."""
        tok = _count_tokens('')
        assert tok == 1  # max(1, ...) 通过字节启发式

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_long_text_is_fast(self) -> None:
        """统计极长字符串不应卡住."""
        s = 'x' * 1_000_000
...
class TestEstimatedUsage:
    """验证无真实 usage 时 Qoder 按对话上下文生成逐消息和会话估算."""

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_no_usage_generates_estimates(self) -> None:
    ...
    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_session_summary_matches_per_message_sum(self) -> None:
        ...
        assert est_input == sum_input, f'会话 input {est_input} != 逐消息之和 {sum_input}'
        assert est_output == sum_output, f'会话 output {est_output} != 逐消息之和 {sum_output}'
...
class TestRealUsagePreserved:
    """验证 Qoder 真实 provider usage 优先于估算且归一化到 canonical buckets."""

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_real_usage_skips_estimation(self) -> None:
    ...
    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_extract_messages_uses_real_usage(self) -> None:
        ...
        assert msg.usage.get('input_tokens') == events[1]['message']['usage']['input_tokens']
        assert msg.usage.get('output_tokens') == events[1]['message']['usage']['output_tokens']
        assert msg.usage.get('estimated') is None  # 真实用量不应设置 estimated 标志

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_qoder_provider_usage_normalized_to_canonical_buckets(self) -> None:
        ...
        first_usage = events[1]['message']['usage']
        second_usage = events[3]['message']['usage']
        assert records[0]['usage']['input_tokens'] == first_usage['input_tokens']
        assert records[0]['usage']['cache_creation_input_tokens'] == 0  # 不被推断覆盖
        assert records[0]['usage']['qoder_input_tokens_total'] == first_usage['input_tokens']
        # 推断值进入单独字段
        assert (
            records[0]['usage'].get('qoder_cache_write_inferred_tokens')
        ...
        assert records[1]['usage']['input_tokens'] == second_usage['input_tokens']
        assert (
            records[1]['usage']['cache_read_input_tokens']
            == second_usage['cache_read_input_tokens']
        )

        summary = _build_summary_from_events(events, 'sess-1', '/tmp')
        # fresh_input_tokens 是各 call Fresh 之和: 1000 + 1200 = 2200
        assert summary.fresh_input_tokens == (
            first_usage['input_tokens'] + second_usage['input_tokens']
        )
        assert summary.cache_read_tokens == second_usage['cache_read_input_tokens']
        # cache_write_tokens 是 cache_creation_input_tokens 之和: 0 + 0 = 0
        assert summary.cache_write_tokens == 0
        assert summary.output_tokens == first_usage['output_tokens'] + second_usage['output_tokens']
        assert summary.total_tokens == (
            first_usage['input_tokens']
            + second_usage['input_tokens']
            + second_usage['cache_read_input_tokens']
            + first_usage['output_tokens']
            + second_usage['output_tokens']
        )

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_extract_messages_uses_normalized_qoder_provider_usage(self) -> None:
        """_extract_messages 应保留 Qoder provider usage 的 input/cache canonical 字段."""
        events = [
            _user_event('hello'),
            _assistant_event(
                text='hi',
        ...
        source_usage = events[1]['message']['usage']
        assert usage['input_tokens'] == source_usage['input_tokens']
        assert usage['cache_read_input_tokens'] == source_usage['cache_read_input_tokens']
        assert usage['cache_creation_input_tokens'] == source_usage['cache_creation_input_tokens']
        assert usage['qoder_input_tokens_total'] == source_usage['input_tokens']

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_qoder_same_message_fragments_use_request_input_and_accounting_snapshot(self) -> None:
        """同一 msg_id 多片段应保留首个 request input 并采用最终 accounting 快照."""
        events = [
            _user_event('hello'),
            _assistant_event(
                text='thinking',
        ...
        final_fragment_usage = events[2]['message']['usage']
        assert records[0]['usage']['input_tokens'] == events[1]['message']['usage']['input_tokens']
        assert (
            records[0]['usage']['cache_read_input_tokens']
            == final_fragment_usage['cache_read_input_tokens']
        )
        assert (
            records[0]['usage']['cache_creation_input_tokens']
            == final_fragment_usage['cache_creation_input_tokens']
        )
        assert records[0]['usage']['output_tokens'] == final_fragment_usage['output_tokens']
...
class TestLongTextHandling:
    """验证超长 tool_result 或 assistant 文本不会使 Qoder 估算崩溃或变慢."""

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_long_tool_result_no_crash(self) -> None:
        ...
        events[2]['message']['content'] = [{'type': 'tool_result', 'content': long_text}]

        # 不应崩溃
        _est_input, _est_output, has_estimated = _estimate_tokens_from_events(events)
        assert has_estimated

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_long_assistant_text_no_crash(self) -> None:
        ...
        _est_input, est_output, has_estimated = _estimate_tokens_from_events(events)
        assert has_estimated
        # output 应被截断,而非完整 500K 字符对应的 token 数
        assert est_output < _count_tokens(long_text) + 10  # 应接近截断后的计数

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_estimation_is_fast_on_large_text(self) -> None:
        """大事件估算应在远低于 1 秒内完成."""
        ...
        expected_max_elapsed_seconds = 2.0
        assert elapsed < expected_max_elapsed_seconds, f'Estimation took {elapsed:.2f}s'
        assert has_estimated
...
class TestMultiFragmentMessages:
    """验证同一 Qoder assistant 消息多片段合并与不同 msg_id 隔离."""

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_fragments_merged_output(self) -> None:
    ...
    def test_different_message_ids(self) -> None:
        ...
        assert len(est_input) == expected_message_count
        assert len(est_output) == expected_message_count
...
class TestExtractMessagesEstimatedUsage:
    """验证 _extract_messages 在缺真实 usage 时注入可归一化的估算字段."""

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_estimated_usage_injected(self) -> None:
    ...
    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_multi_turn_estimated_usage(self) -> None:
        ...
        assert len(assistant_msgs) == expected_assistant_count
...
class TestToolExecutionTime:
    """验证 Qoder tool_use/tool_result 时间戳生成准确的工具耗时."""

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_non_overlapping_tools(self) -> None:
        ...
        expected_tool_seconds = 20.0
        assert summary.tool_execution_seconds == expected_tool_seconds, (
            f'期望 20s 但得到 {summary.tool_execution_seconds}'
        )

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_overlapping_tools_merged(self) -> None:
        ...
        expected_merged_tool_seconds = 10.0
        assert summary.tool_execution_seconds == expected_merged_tool_seconds, (
            f'期望 10s(合并后)但得到 {summary.tool_execution_seconds}'
        )

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_no_tool_results_gives_zero(self) -> None:
    ...
    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_tool_call_duration_ms(self) -> None:
        ...
        expected_tool_duration_ms = 10_000
        assert tool_calls[0].duration_ms == expected_tool_duration_ms, (
...
class TestCacheNoFabrication:
    """Qoder 会话无 cache read/write 时绝不能捏造 cache 值.
    ...
    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_summary_cache_zero_when_estimated(self) -> None:
    ...
    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_summary_cache_zero_when_real_usage_no_cache(self) -> None:
        ...
        assert summary.cache_read_tokens == 0
        assert summary.cache_write_tokens == 0
        assert summary.fresh_input_tokens == events[1]['message']['usage']['input_tokens']
        assert summary.output_tokens == events[1]['message']['usage']['output_tokens']

    @pytest.mark.contract_case('DATA-SOURCE-010', 'DATA-SOURCE-013')
    def test_per_message_cache_zero_when_estimated(self) -> None:
