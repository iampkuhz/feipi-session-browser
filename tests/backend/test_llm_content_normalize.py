...
MIN_TOOL_RESULT_BLOCKS = 2
MIN_RENDERED_BLOCKS = 2
...
def test_tool_result_with_markdown_file():
    ...
    has_filename = 'AGENTS.md' in all_content or any(
        'AGENTS.md' in b.get('title', '') for b in blocks
    )
...
def test_multiple_tool_results():
    ...
    blocks = normalize_llm_content(text)
    assert len(blocks) >= MIN_TOOL_RESULT_BLOCKS
    all_content = '\n'.join(b['content'] for b in blocks)
...
def test_render_multiple_blocks():
    ...
    html = render_llm_blocks_html(blocks)
    assert html.count('llm-block') >= MIN_RENDERED_BLOCKS
