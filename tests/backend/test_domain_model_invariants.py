...
from session_browser.domain.serializers import (
    normalized_token_breakdown_to_dict,
    subagent_summary_to_dict,
)

TOKEN_COMPONENT_SUM_TOTAL = 20
SESSION_COMPONENT_SUM_TOTAL = 14
LLM_CALL_COMPONENT_SUM_TOTAL = 17
LIVE_TOOL_CALL_COUNT = 2
...
def test_token_breakdown_recomputes_component_sum_total():
    ...
    assert bd.total_tokens == TOKEN_COMPONENT_SUM_TOTAL
    assert bd.component_total == TOKEN_COMPONENT_SUM_TOTAL
...
def test_session_summary_computes_missing_total_from_components():
    ...
    assert summary.token_component_total == SESSION_COMPONENT_SUM_TOTAL
    assert summary.total_tokens == SESSION_COMPONENT_SUM_TOTAL
...
def test_llm_call_builds_composed_submodels_and_derived_total():
    ...
    assert call.total_tokens == LLM_CALL_COMPONENT_SUM_TOTAL
    assert call.identity.id == 'call-1'
    assert call.usage.total_tokens == LLM_CALL_COMPONENT_SUM_TOTAL
...
def test_subagent_run_exposes_live_counts_from_tool_collection():
    ...
    assert run.live_tool_call_count == LIVE_TOOL_CALL_COUNT
