...
@pytest.mark.contract_case(
    'DATA-SOURCE-001', 'DATA-SOURCE-002', 'DATA-SOURCE-003', 'DATA-SOURCE-004'
)
@pytest.mark.contract_case('DATA-SOURCE-011', 'DATA-SOURCE-014')
def test_parse_session_events() -> None:
    """测试能否解析 Claude 会话事件."""
...
def test_build_summary_from_events() -> None:
    """测试从事件构建摘要."""
    fixture = FIXTURES / 'claude_session_sample.jsonl'
    events, _ = parse_jsonl_events(fixture)
...
def test_parse_history_empty_when_missing() -> None:
...
def test_parse_session_detail_includes_subagent_diagnostics() -> None:
    ...
    assert summary.tool_call_count == expected_tool_call_count
    assert summary.failed_tool_count == 1
    expected_fresh_input_tokens = 306  # 父 100 + 子 msg-child-1 max input 200 + 子 msg-child-2 6
    expected_cache_tokens = 300
    assert summary.fresh_input_tokens == expected_fresh_input_tokens
    assert summary.cache_read_tokens == expected_cache_tokens
    assert summary.cache_write_tokens == expected_cache_tokens
    ...
    expected_child_call_count = 2
    assert agent_call.llm_call_count == expected_child_call_count
    assert agent_call.subagent_tool_call_count == expected_child_call_count
    assert agent_call.subagent_summary['tool_counts'] == {'Read': expected_child_call_count}
...
def test_same_message_usage_fragments_keep_whole_provider_snapshot() -> None:
...
def test_parse_session_events_skips_non_dict_json_values() -> None:
    ...
    try:
        ...
        non_dict_types = [type(e).__name__ for e in events if not isinstance(e, dict)]
        assert all(isinstance(ev, dict) for ev in events), (
            f'parse_jsonl_events returned non-dict: {non_dict_types}'
        )
...
def test_parse_pretty_printed_multiline_json() -> None:
...
"""
    ...
    try:
        ...
        assert len(events) == expected_event_count, f'Expected 3 events, got {len(events)}'
...
def test_parse_concatenated_json_transition_line() -> None:
...
def test_brace_inside_string_values_not_counted() -> None:
    """JSON 字符串值中的大括号/方括号不能影响深度跟踪."""
    # 字符串中的大括号应被忽略
