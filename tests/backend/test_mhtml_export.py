...
class TestMhtmlTemplateContracts:
    ...
    def test_export_css_inlines_import_wrappers(self):
        ...
        assert '@import' not in css_without_comments, (
            'exported CSS must inline local @import wrappers'
        )

    @pytest.mark.contract_case('ROUTE-API-001', 'ROUTE-API-004')
    def test_routes_has_export_mhtml_support(self):
        routes = (ROOT / 'src' / 'session_browser' / 'web' / 'routes.py').read_text(
            encoding='utf-8'
        )
...
class TestPhase1SimplifiedStructure:
    ...
    def test_has_trace_panel(self):
        html = self._read('session.html')
        assert 'class="trace-panel"' in html or 'data-trace-panel' in html, 'missing trace-panel'
    ...
    def test_has_expand_collapse_buttons(self):
        ...
        assert 'data-action="collapse-all"' not in component, (
            'collapse-all must be removed; use toggle-all only'
        )
    ...
    def test_has_all_failed_segmented_control(self):
        ...
        component = _read_split_component('components/session_detail_timeline.html')
        has_new = (
            'data-action="status-all"' in component and 'data-action="status-failed"' in component
        )
