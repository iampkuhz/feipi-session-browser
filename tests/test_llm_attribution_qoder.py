...
def test_qoder_cache_split_from_normalized():
    ...
    lc = _make_lc(input_tokens=500, cache_read_tokens=300, cache_write_tokens=50, output_tokens=200)
...
def test_qoder_zero_values_are_valid():
    ...
    lc = _make_lc(input_tokens=1000, cache_read_tokens=0, cache_write_tokens=0, output_tokens=500)
...
def test_qoder_availability_notes_cache_from_usage():
    ...
    lc = _make_lc(
        input_tokens=5000, cache_read_tokens=2000, cache_write_tokens=500, output_tokens=1000
    )
    ro = _make_ro(user_content='test')
    builder = QoderAttributionBuilder(lc, ro)
    result = builder.build_request()

    fields = {r.field if hasattr(r, 'field') else r['field'] for r in result.availability_rows}
...
def test_qoder_transcript_reconstruction():
    ...
    lc = _make_lc(
        input_tokens=10000, output_tokens=5000, request_full='some request context\n\nmore context'
    )
...
def test_qoder_bucket_sum_within_total():
    lc = _make_lc(input_tokens=10000, output_tokens=5000, request_full='context\n\nmore\n\ndata')
...
def test_qoder_tool_schema_uses_claude_like_default_registry():
    ...
    assert any(
        item['unit_type'] == 'tool_definitions_unit' for item in tool_definitions.details['items']
    )
