...
| DATA-INDEX-013 | P2 | data | 分层扫描（hot/warm tier）配置验证 | 检查 TIER_HOT_SECONDS 和 TIER_HOT_INTERVAL 常量 | hot tier: 30min 内 session 每 30s 扫描；warm tier: 24h 内每 5min | pytest | — | `src/session_browser/index/indexer.py`（TIER_ 常量） |
