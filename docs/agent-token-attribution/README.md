...
Review 代码时先从入口函数看边界，再进入 agent builder 和 serializer。下表列出最小阅读路径：

| 层级 | 入口函数 | 作用 |
|---|---|---|
| 前端懒加载 | `openAttributionModal()` / `attributionApiUrl()`，位于 `src/session_browser/web/static/js/session-detail/attribution.js` | 用户点击 request/response attribution 后生成 API URL 并拉取 JSON。 |
| API 分发 | `_serve_api_attribution_path()`，位于 `src/session_browser/web/routes.py` | 区分 main call 与 subagent call URL，校验 `kind`、round、call index。 |
| Session 装载 | `_load_session_and_build_rounds()`，位于 `src/session_browser/web/routes.py` | 读取 session、解析 messages/tool calls/subagents，构造 `LLMCall`。 |
| 归因上下文 | `build_attribution_session_context()`，位于 `src/session_browser/attribution/context.py` | 收集当前 call 之前可见的 history、tool results、tool schemas、local instructions 和 normalized artifact。 |
| 服务分派 | `build_llm_request_attribution()` / `build_llm_response_attribution()`，位于 `src/session_browser/attribution/service.py` | 按 agent 选择 `ClaudeCodeAttributionBuilder`、`CodexAttributionBuilder`、`QoderAttributionBuilder` 或 `BaseAttributionBuilder`。 |
| Agent builder | `build_request()` / `build_response()`，位于 `src/session_browser/attribution/agents/*_attribution_builder.py` | 归一化 usage，构造 request/response buckets、coverage、residual 与 notes。 |
| Field-first 映射 | `*TokenAccountingMapper.build_request_accounting()` / `build_response_accounting()`，位于 `src/session_browser/attribution/mapping/agents/*_token_accounting_mapping.py` | 把 normalized `source_units` 映射到四个 `TokenAccountingField`。 |
| Payload 序列化 | `request_attribution_to_payload()` / `response_attribution_to_payload()`，位于 `src/session_browser/attribution/serializers.py` | 输出 `llm_attribution_v2` payload、`usage_summary`、`accounting_attribution` 和 diagnostics。 |

PlantUML 中黄色节点是优先 review 的函数入口：

```plantuml
@startuml
title LLM Token Attribution 代码活动流程
...
partition "1. 前端触发" {
  #FFF2B2:ENTRY
openAttributionModal(button)
attributionApiUrl(button, kind)
file: attribution.js;
  :生成 attribution API URL
main: /attribution/{round}/{call}/{kind}
subagent: /attribution/subagent/{sa}/{call}/{kind};
  :发起 fetch
缓存命中则直接 renderAttributionSuccess();
}

partition "2. API 路由与 LLMCall 定位" {
  #FFF2B2:ENTRY
_serve_api_attribution_path(path)
file: routes.py;
  :校验 kind=request/response
解析 main 或 subagent URL 参数;
  :分发到
_serve_api_attribution_main()
或 _serve_api_attribution_subagent();
  #DBEAFE:_load_session_and_build_rounds()
parse_session_detail()
build_rounds()
build_llm_calls()
assign_interactions_to_rounds();
  :按 round/call index
或 subagent id + call index
定位目标 LLMCall ix;
  #FFF2B2:ENTRY
_build_and_send_attribution(..., ix, kind)
file: routes.py;
}

partition "3. Call Scoped Context" {
  #FFF2B2:ENTRY
build_attribution_session_context(...)
file: attribution/context.py;
  #DBEAFE:收集当前 call 前可见上下文
preceding_tool_results
prior_messages / full_messages_array
available_tools / local_instructions / mcp metadata;
  #DBEAFE:尝试 _load_normalized_call_for_context()
按 call_id 精确匹配
否则按 scope ordinal fallback;
  #DBEAFE:命中 normalized artifact
_hydrate_normalized_call()
source_unit_ref_ranges -> source_units
缺 attribution_candidates 时由 source_units 推导;
  :未命中时
仅保留 session transcript / tool logs / local files 的可见上下文;
}

partition "4. Service 分派" {
  #FFF2B2:ENTRY
build_llm_request_attribution(...)
或 build_llm_response_attribution(...)
file: attribution/service.py;
  :按 agent 选择 builder
claude_code -> ClaudeCodeAttributionBuilder
codex -> CodexAttributionBuilder
qoder -> QoderAttributionBuilder
unknown -> BaseAttributionBuilder;
}

partition "5. Agent Builder 执行" {
  #FFF2B2:ENTRY
builder.build_request()
或 builder.build_response()
file: attribution/agents/*_attribution_builder.py;
  #DCFCE7:Codex usage 优先级
token_breakdown_normalized
LLMCall usage
assistant_msg.usage
raw response payload usage;
  #DCFCE7:normalized source_units 路径
按 request/response direction 聚合 candidates
调用 *TokenAccountingMapper.build_*_accounting();
  #DCFCE7:Codex fallback 路径
request: raw payload / request_full / transcript
估算 instructions、user、history、tool outputs、repo context、tool schemas、reasoning config、residual
response: assistant text、tool_call、reasoning_output、metadata、unknown residual;
  #DCFCE7:Claude Code / Qoder 路径
_load_source_units(direction)
source_units -> candidates -> buckets
缺失时返回低置信度 unavailable 与 residual 说明;
}

partition "6. Payload 序列化" {
  #FFF2B2:ENTRY
request_attribution_to_payload(attr)
或 response_attribution_to_payload(attr)
file: attribution/serializers.py;
  #FCE7F3:normalize/sort buckets
计算 usage_summary、coverage、diagnostics;
  #FCE7F3:优先使用 attr.accounting_attribution
否则 _build_request_accounting_attribution()
或 _build_response_accounting_attribution()
从 legacy buckets 聚合 candidates;
  #FCE7F3:输出 llm_attribution_v2 payload
包含 accounting_attribution field-first view;
}

partition "7. API 返回与前端渲染" {
  #FCE7F3:routes.py 封装 envelope
kind/source/session_id/round_index/call_index/data
_send_json(envelope);
  #FFF2B2:ENTRY
renderAttributionSuccess(body, payload, kind, cacheKey)
file: attribution.js;
  :渲染 usage、buckets、accounting_attribution、notes、diagnostics;
}
