...
| HOOK-HARNESS-010 | P0 | data | Gate executor | 测试唯一 executor 的命令、环境、无自动 deadline 与资源隔离 | catalog 命令自然完成；仅显式中断或父异常清理进程组；warning/skip/not-run/unavailable 不得 PASS | pytest | — | `tests/gates/test_executor.py`; `tests/gates/test_process.py`; `tests/quality/test_python_env_contract.py` |
