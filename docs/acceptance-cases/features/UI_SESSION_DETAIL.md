...
| UI-SD-021 | P1 | visual | 工具结果渲染 | 打开 typed payload modal | tool result 使用当前 typed renderer，内容可访问且无空占位 | Playwright | — | `tests/playwright/session-detail-behavior-contracts.spec.js` |
...
| UI-SD-037 | P1 | visual | Request attribution renderer | 打开 request attribution modal | 当前中文 provider/summary/distribution/bucket/leaf preview 语义在 1440 与 2560 视口稳定 | Playwright | — | `tests/playwright/session-detail-behavior-contracts.spec.js` |
| UI-SD-038 | P1 | visual | Response attribution renderer | 打开 response attribution modal | 当前中文 provider/summary/distribution/bucket/leaf preview 语义在 1440 与 2560 视口稳定 | Playwright | — | `tests/playwright/session-detail-behavior-contracts.spec.js` |
| UI-SD-040 | P1 | interaction | Trace toggle 对齐回归 | 展开、筛选并 toggle-all | 初始对齐偏差不超过 20px，交互回归不超过 2px | Playwright | — | `tests/playwright/session-detail-behavior-contracts.spec.js` |
| UI-SD-041 | P1 | interaction | Typed payload 可访问性 | 打开并关闭 payload modal | accessible name、typed body、Escape 关闭和 overflow 均有效 | Playwright | — | `tests/playwright/session-detail-behavior-contracts.spec.js` |
| UI-SD-042 | P1 | visual | 100-round 滚动契约 | 加载 deterministic long session | 100 行可纵向滚动且无水平溢出 | Playwright | — | `tests/playwright/session-detail-behavior-contracts.spec.js` |
