...
| 关联测试 | `java/web/src/test/java/com/feipi/session/browser/web/page/WebStaticResourceContractTest.java`、`java/web/src/test/java/com/feipi/session/browser/web/template/PebbleEnvironmentTest.java`、`tests/playwright/macbook-smoke.spec.js`、`tests/playwright/ui-contract.spec.ts`、`tests/playwright/shell-states.spec.js` |
...
| UI-VISUAL-004 | P0 | visual | Shell 状态多视口截图基线（normal/hide-right × 1440/1280/1180）| 各状态 × 视口截图 | 截图存入根 `tmp/agent-runtime/runs/<run-id>/playwright/screenshots/`，结构断言 main 宽度 > 600 | Playwright | snapshot 更新条件：当 shell.css 变更时需更新快照 | `tests/playwright/shell-states.spec.js` |
| UI-VISUAL-005 | P0 | visual | Shell 状态截图基线（hide-left/focus × 1440/1280/1180）| 各状态 × 视口截图 | 截图存入根 `tmp/agent-runtime/runs/<run-id>/playwright/screenshots/`，main 宽度 > 600 | Playwright | snapshot 更新条件：当 shell.css 变更时需更新快照 | `tests/playwright/shell-states.spec.js` |
...
| UI-VISUAL-010 | P1 | visual | 长内容滚动行为 | 加载 100-round session | 页面可纵向滚动且无水平 overflow | Playwright | — | `tests/playwright/session-detail-behavior-contracts.spec.js` |
