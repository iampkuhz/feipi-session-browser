...
1. ID 不可重复：每个用例 ID 只能出现在一个验收用例行中。
...
7. 用例表格行与测试绑定必须同步存在并使用同一 ID。
8. 验收用例 ID 总账不是 Gate Target；`acceptanceTraceability` 只校验映射，实际测试由对应 Pytest、JUnit 或 Playwright Gate 执行。
