...
| DATA-PRESENTER-010 | P1 | data | LLM Payload API 语义（payload 文本不截断） | 调用 payload API 端点 | 返回的 payload text 包含完整内容，未被预览长度截断 | pytest | — | `tests/session_detail/test_session_detail_api.py` |
