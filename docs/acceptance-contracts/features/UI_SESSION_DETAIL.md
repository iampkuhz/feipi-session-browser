...
| UI-SD-032 | P1 | visual | Main Agent Breakdown 承载主成本信号 | hover 主 agent round | 页面不再渲染独立 `Top Cost Drivers` 或 `Call Cost Distribution` 卡片；主 agent timeline tooltip 不展示 Call Tokens、Call Cost、Top Call、Top Lane 或 main/subagent split；主 agent 高成本 call 通过 Badge Text 展示，subagent 成本在 Subagent Breakdown 行内展示 | Playwright | — | `tests/playwright/session-detail.spec.js` |
| UI-SD-033 | P1 | interaction | Trace 深链定位 round 与 subagent round | 打开 `?tab=trace&round=...`，并从 Subagent Breakdown 右侧 timeline 跳转 | round 靠近视口顶部；subagent timeline round 写入 `subagent`/`subagentround` 参数并滚动到对应 `SRx` | Playwright | — | `tests/playwright/session-detail.spec.js` |
