...
| UI-PROJECTS-002 | P0 | visual | 项目详情页结构 | 多视口访问 `/projects/{key}` | page head、metric grid、data table 可见且无水平溢出 | Playwright | — | `tests/playwright/ui-contract.spec.ts` |
| UI-PROJECTS-003 | P0 | visual | 项目详情表格契约 | 检查真实表格列头 | 列头含 session/agent/model/tokens/rounds | Playwright | — | `tests/playwright/ui-contract.spec.ts` |
...
| UI-PROJECTS-007 | P0 | visual | 项目详情运行契约 | 真实浏览器加载项目详情 | 页面结构、表格和资源均可用 | Playwright | — | `tests/playwright/ui-contract.spec.ts` |
| UI-PROJECTS-008 | P0 | visual | 项目列表运行契约 | 真实浏览器加载项目列表 | 筛选栏和数据表区域可见 | Playwright | — | `tests/playwright/ui-contract.spec.ts` |
