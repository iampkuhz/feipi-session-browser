...
Token Glossary 是轻量术语说明页，解释 token 口径、派生指标、provider 字段映射和 badge 语义。该页只承载帮助理解数据口径的静态说明，不替代业务页面上的常驻指标。

## 页面布局

- 路由：`/glossary`；模板：`glossary.html`。
- Page Head 左侧显示 `Token Glossary` 和一句中文说明，英文标题作为固定产品术语保留。
...
- 固定覆盖 `Fresh`、`Provider Request Input`、`Cache Read`、`Cache Write`、`Output`、`Input-side Component Total`、`Total Tokens`。
- `Provider Fields` 单元格展示 Anthropic、OpenAI、Codex、Qoder 的原始字段名摘要，并用中文解释这些字段如何映射到当前组件。
...
- 固定覆盖 `Cache Read Ratio`、`Cache Write Ratio`、`Output/Input Ratio`、`Tools / Round`、`Tokens / Round`、`Tokens / Minute`、`Failed / Session`。
- 每个公式必须与产品聚合口径一致。
- Interpretation 必须用中文说明该指标在 Dashboard、Sessions、Session Detail 中的使用位置。
- 示例行固定包含 `Cache Read Ratio`、`Cache Read / Input-side Component Total`、`cache reuse health`。

### Provider Field Mapping

- 固定覆盖 Anthropic、qwen Anthropic compatible、OpenAI、Codex、Qoder。
- 每行必须说明 Fresh、Cache Read、Cache Write、Output 的来源字段。
- provider 未上报字段时使用 `Not reported`，不得留空；原始字段名保留英文。
- 示例行固定包含 `Anthropic`、`Claude 4`、`input_tokens`、`cache_read_input_tokens`、`cache_creation_input_tokens`、`output_tokens`，并在说明文本中解释这些字段映射到 Fresh、Cache Read、Cache Write、Output。
