...
- `Tool Call` 行固定展示 tool name、command preview、result preview、status、duration、`Result` action。
...
- 请求摘要字段固定为 agent、model、source、API family、provider、call id、输入分母、fresh input、cache read、cache write、Provider 总计、本地重建、coverage、残差；不得在归因明细底部另起 `覆盖率与不确定性` 表格。
...
- 全局 request token 归因分类树固定为：`当前用户输入`、`对话消息上下文`、`工具结果上下文`、`工具定义`、`MCP 工具元数据`、`运行上下文片段`、`系统/开发者指令`、`本地指令上下文`、`Agent/Subagent 提示`、`内置系统提示`、`Provider 缓存命中上下文`、`Provider 会话状态`、`推理配置`、`运行时封装开销`、`未定位`。
...
- Codex provider reported `cached_input_tokens` / cache read 必须作为 raw `provider_cached_context` bucket 计入 coverage，序列化后统一显示为 `Provider 缓存命中上下文`；bucket 文案必须说明该 token 来源是 provider cache hit，但本地 rollout 不展开完整缓存内容。
...
- Qoder provider reported `cache_read_input_tokens` 必须作为 raw `provider_cached_context` bucket 计入 coverage，序列化后统一显示为 `Provider 缓存命中上下文`；`cache_creation_input_tokens` / cache write 只在摘要中展示，不进入 request bucket 分母。
...
- attribution modal 的 bucket detail 必须是两层展开：第一层点击贡献来源 bucket 后展示该 bucket 内部的子分类 card；子分类 card 折叠态只展示精简摘要、来源/role/type/token 等短 meta，折叠态高度一致；第二层点击某个子分类 card 后，在该 card 内展示可得的完整原始内容或完整解释。
