...
- 诊断卡片区固定包含 `Token Timeline + Cache Health`、`Top Cost Drivers`、`Call Cost Distribution`、`Tool Impact`、`Subagent Breakdown`、`Issues & Repro Seeds`、`Payload Availability`、`Context Budget`。
...
- 诊断卡标题固定为 `Token Timeline + Cache Health`、`Top Cost Drivers`、`Call Cost Distribution`、`Tool Impact`、`Subagent Breakdown`、`Issues & Repro Seeds`、`Payload Availability`、`Context Budget`。
...
- 中间行按固定顺序两列排布：`Top Cost Drivers`、`Call Cost Distribution`、`Tool Impact`、`Subagent Breakdown`、`Issues & Repro Seeds`、`Payload Availability`。
...
- 列固定为 `Type`、`Driver`、`Tokens`、`Share`、`Reason`。
- `Tokens` 使用 driver token 贡献；`Share` 使用 driver tokens / session total tokens。
- subagent 内部 LLM call 聚合为一行 `Subagent` driver。
- 点击 driver 跳到 Trace 对应 round；subagent driver 必须额外写入 `subagent=<subagent_id>` 并定位到嵌套 subagent block；tool result driver 后续可扩展跳到 Payload。

### Call Cost Distribution

- 卡片定位：解释 `LLM Calls` 是否由少数昂贵 call 拉高成本。
- 图表类型固定为按 call 时间顺序排列的条形/散点分布图。
- x 轴为紧凑 call order，不在横坐标直接渲染 timestamp；y 轴为 total tokens；卡片内必须显式说明排序顺序和 y 轴含义。
- 主 agent call 与 subagent call 必须分色展示；subagent call 按 subagent 实例时间顺序循环使用 5 个候选颜色。
- 图例必须逐个列出 subagent 实例，文本固定包含 subagent name 和 subagent id；多个 subagent 共享同一颜色时仍各自占一项，不得聚合为 `Subagent 1/6/...` 这类槽位说明。
- Top 3 expensive calls 必须有视觉标记。
- hover/focus 必须使用自绘 tooltip，不依赖浏览器原生 `title`；tooltip 展示 call index、timestamp、round、lane、model、tokens。
- 点击 main call 跳到 Trace 对应 round；点击 subagent call 必须写入 `tab=trace&round=<parent_round>&subagent=<subagent_id>&subagentround=<sub_round_id>`，并定位到对应 subagent 的 `SR<sub_round_id>`。
...
- 每个 subagent 一行。
- 列固定为 `Subagent`、`Instance`、`LLM Calls`、`Footprint Tokens`、`Tools`、`Failures`。
- `Instance` 展示短 subagent id，并使用和 Call Cost Distribution 一致的实例颜色。
- `Footprint Tokens` 表示 subagent 内部 LLM token、父 Agent result token、内部 tool result token 的 footprint 合计；tooltip 必须拆分来源。
- `Failures` 展示 `失败数 · 失败率`，失败率按比例标记 ok/warn/bad 色。
- 点击 subagent 跳到对应父 round 的嵌套 subagent block，并写入 `tab=trace&round=<parent_round>&subagent=<subagent_id>`；后续可扩展到 payload call。
...
- 卡片定位：展示 transcript payload、tool result 和 attribution 的可用情况；Claude Code 日志无法拿到真实原始 provider request body，request 相关内容来自 transcript 重建和 attribution。
- 展示 coverage matrix。
...
- `Fresh` 表示 provider-reported uncached input token，不表示完整重建 request 文本 token 数。
...
- 请求摘要字段固定为 agent、model、source、API family、provider、call id、total input、fresh input、cache read、cache write、unknown、coverage。
