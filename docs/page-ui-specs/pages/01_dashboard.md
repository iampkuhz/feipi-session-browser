...
- 每张 trend card 的所有 range point 细节都通过 hover tooltip 和键盘 focus tooltip 展示，不把每个 range point 的数值同时铺成表格。
...
- Hover 图表点必须高亮当前 range point；tooltip 必须展示该点的 y 轴真实值、占比、辅助指标。
