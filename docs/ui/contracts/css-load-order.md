...
7. **payload-modal 裸定义应收敛至 ui-primitives.css**：在 `style.css`、`session-detail.css`、`legacy-aliases.css` 等处出现裸 `.payload-modal` 或 `#payload-modal` 定义时输出 **WARN**，不 BLOCK（历史债后续清理）。

8. **shell 级选择器不应出现在页面 CSS**：`.app-shell`、`.shell`、`body.hide-left` 等选择器应归属 `shell.css`，页面 CSS 出现时输出 **WARN**，不 BLOCK（历史存在可接受，新增应在后续 BLOCK）。
