...
val buildRoot = layout.projectDirectory
val repoRoot = buildRoot.dir("..")
extra["repoRoot"] = repoRoot
layout.buildDirectory.set(repoRoot.dir(".local/gradle/root-build"))

group = "com.feipi.session.browser"
version = buildRoot.file("gradle/VERSION").asFile.readText().trim()
...
dependencyLocking {
    lockFile.set(buildRoot.file("gradle/dependency-locks/root.lockfile"))
...
val verifyLeanQualityStack = tasks.register("verifyLeanQualityStack") {
    ...
    val catalogPath = buildRoot.file("gradle/libs.versions.toml").asFile.absolutePath
...
val policyFilePath = buildRoot.file("gradle/config/reuse-policy/policy.json").asFile.absolutePath
...
private class ReuseStandardCpdAction(
    ...
    private fun requiresCompleteCpdInput(path: String): Boolean =
        path == "java/build.gradle.kts" ||
            path == "java/settings.gradle.kts" ||
            path == "java/gradle/config/reuse-policy" ||
            path.startsWith("java/gradle/config/reuse-policy/") ||
...
gradle.projectsEvaluated {
    ...
    tasks.register("reuseStandardCpd") {
        ...
        val rootPath = repoRoot.asFile.absolutePath
