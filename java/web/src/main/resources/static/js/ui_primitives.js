// 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`UI primitives — canonical data-action delegation for Feipi Session Browser.`
// 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`All interactions route through document-level click/submit listeners.`
// 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`No inline event handlers in templates.`
/**
...
(function () {
  ...
  /* 中文说明：维护当前前端样式或交互约束，原注释作为代码上下文保留：`* * Helper: walk up the DOM tree to find the closest ancestor matching selector.` */
  ...
  /* 中文说明：维护当前前端样式或交互约束，原注释作为代码上下文保留：`* * Helper: find the nearest data-table ancestor for a given element.` */
  function closestTable(el) {
    return closest(el, 'table.data-table');
  }

  /* 中文说明：维护当前前端样式或交互约束，原注释作为代码上下文保留：`* * Helper: show a toast notification.` */
  function showToast(message) {
    ...
    if (existing) {
      ...
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Force reflow so the transition triggers`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Click delegation (data-action) ──────────────────────────────────────`
  document.addEventListener('click', function (event) {
    ...
    switch (action) {
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Sort ──────────────────────────────────────────────────────`
      case 'sort':
        handleSort(actionEl);
        break;

      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Filter / Clear ────────────────────────────────────────────`
      case 'filter':
        // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Let the form submit normally (data-action="filter" is on the submit button)`
        break;
      ...
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Pagination ────────────────────────────────────────────────`
      case 'prev-page':
      ...
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Modal ─────────────────────────────────────────────────────`
      case 'close-modal':
      ...
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Popover ───────────────────────────────────────────────────`
      case 'toggle-popover':
        handleTogglePopover(actionEl);
        break;

      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Copy ──────────────────────────────────────────────────────`
      case 'copy':
        handleCopy(actionEl);
        break;

      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Error State ───────────────────────────────────────────────`
      case 'error-retry':
      ...
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Default: dispatch custom event for page-specific handlers ──`
      default:
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Keyboard delegation ─────────────────────────────────────────────────`
  document.addEventListener('keydown', function (event) {
    if (event.key === 'Escape') {
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Close any visible modal backdrop`
      ...
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Close any visible popover`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Page input: Enter to navigate`
    if (event.key === 'Enter') {
      ...
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Sortable header: Enter to sort`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Sortable header: Space to sort (prevent page scroll)`
    if (event.key === ' ') {
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Change delegation (page-size select) ─────────────────────────────────`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Backdrop click closes modal ─────────────────────────────────────────`
  document.addEventListener('click', function (event) {
    var target = event.target;
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Also support payload-modal__overlay`
    if (target.classList.contains('payload-modal__overlay') && target.classList.contains('is-visible')) {
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Only close if clicking the overlay directly (not the dialog)`
      target.classList.remove('is-visible');
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Sort handler ────────────────────────────────────────────────────────`
  function handleSort(headerEl) {
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Determine current direction`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Clear all sort indicators in this table`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Set new sort direction`
    headerEl.setAttribute('data-sort-dir', newDir);
    headerEl.setAttribute('aria-sort', newDir === 'asc' ? 'ascending' : 'descending');

    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Dispatch custom event for page-specific sort handling`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Filter handlers ─────────────────────────────────────────────────────`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Pagination handlers ─────────────────────────────────────────────────`
  function handlePageNav(buttonEl, delta) {
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Don't navigate if button is disabled`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Clamp to total pages if available`
  ...
  function handlePageInput(inputEl) {
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Don't navigate if input is disabled (single-page state)`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Clamp to total pages`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Modal handlers ──────────────────────────────────────────────────────`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Payload mode switching ──────────────────────────────────────────────`
  /* 中文说明：维护当前前端样式或交互约束，原注释作为代码上下文保留：`* * handlePayloadMode: Switches between payload view modes * (context/response/result or rendered/raw). * * Reads data-mode from the clicked tab, updates aria-selected, * then shows/hides the corresponding content panels. * * Content panels are identified by: * - data-payload-panel="<mode>" (canonic` */
  function handlePayloadMode(tabEl) {
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Update tab active state`
    if (tabsContainer) {
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Find content panels and toggle visibility`
    var dialogEl;
    if (modal && modal.getAttribute('data-modal') !== null) {
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Canonical overlay modal`
      dialogEl = modal.querySelector('.modal-body');
    } else {
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Fallback: search entire document`
      dialogEl = document.querySelector('.modal-backdrop.is-visible .modal-body');
    }

    if (!dialogEl) return;

    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Toggle canonical data-payload-panel panels`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Toggle class-based panels (rendered vs raw)`
    ...
    if (renderedPanel && rawPanel) {
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Two-panel mode: rendered vs raw`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Dispatch custom event for external handlers (e.g. API fetch)`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Open payload ────────────────────────────────────────────────────────`
  /* 中文说明：维护当前前端样式或交互约束，原注释作为代码上下文保留：`* * handleOpenPayload: Opens the payload modal with data from trigger button. * * Reads data attributes from the trigger button: * - data-payload-id * - data-payload-title * - data-payload-kind * * Shows the modal-backdrop overlay and dispatches 'payload-open' event.` */
  function handleOpenPayload(triggerEl) {
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Find the modal backdrop`
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Update modal title`
    if (title) {
      var titleEl = backdrop.querySelector('[data-modal-title]');
      if (titleEl) titleEl.textContent = title;
    }

    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Update kind badge`
    if (kind) {
    ...
    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Show the modal`
    backdrop.classList.add('is-visible');
    backdrop.setAttribute('aria-hidden', 'false');

    // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Dispatch custom event for external handlers`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Popover handler ─────────────────────────────────────────────────────`
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Copy handler ────────────────────────────────────────────────────────`
  /* 中文说明：维护当前前端样式或交互约束，原注释作为代码上下文保留：`* * handleCopy: Unified copy handler. * * Attribute source: * - data-copy-text * * Clipboard fallback: * - If navigator.clipboard is unavailable, shows toast-only (no write). * - No inline styles are used (avoids layout-inline-style gate violation).` */
  function handleCopy(buttonEl) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(function () {
        ...
        // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Clipboard API rejected (e.g. permission denied) — toast-only`
        showToast('Copied!');
      });
    } else {
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Clipboard API not available (e.g. non-HTTPS context) — toast-only`
      showToast('Copied!');
    }
  }

  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Error State handlers ────────────────────────────────────────────────`
  /* 中文说明：维护当前前端样式或交互约束，原注释作为代码上下文保留：`* * error-retry: dispatches a custom 'error-retry' event from the * nearest .state-strip element so page-specific code can handle * the retry logic (e.g. reload data, retry fetch).` */
  ...
  /* 中文说明：维护当前前端样式或交互约束，原注释作为代码上下文保留：`* * error-dismiss: hides the nearest .state-strip by adding * the 'is-dismissed' class (CSS should set display: none).` */
  ...
  // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`── Global API ──────────────────────────────────────────────────────────`
  window.UiPrimitives = {
    showToast: showToast,
    closestTable: closestTable,
    openPayload: function(payloadId, title, kind) {
      // 中文说明：维护当前前端逻辑，原注释作为代码上下文保留：`Programmatic API: find or create a trigger, then dispatch`
      var backdrop = document.querySelector('.modal-backdrop[data-modal]');
      if (!backdrop) return;
      if (title) {
        var titleEl = backdrop.querySelector('[data-modal-title]');
