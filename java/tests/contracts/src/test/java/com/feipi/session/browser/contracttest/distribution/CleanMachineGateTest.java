 ...
 * 清洁机构建门禁测试。
 *
 * <p>验证发行包在无 JDK/Python/Gradle 环境的清洁机上可正常启动运行。 校验放置：构建产物完整性在 tests:contracts 边界验证一次； 单个模块的构建任务由
 * Gradle 配置保证。
 */
@DisplayName("清洁机构建门禁")
class CleanMachineGateTest {
  ...
  class NoPythonDependency {
    ...
    void noPythonScriptsInRuntimeClasspath() {
      ...
      assertThat(cl.getResource("scripts/checks/check_code_comment_language.py")).isNull();
