...
def check(content):
    ...
    if "body.hide-left .shell.phase1-shell" in content or \
       "body.hide-left .shell.no-inspector.phase1-shell" in content:
        ok("High-specificity body.hide-left .shell.phase1-shell override exists")
    else:
        fail("Missing 'body.hide-left .shell.phase1-shell' (or .no-inspector.variant) "
    ...
    if has_standalone and not has_hide_left_override:
        fail("Only low-specificity '.shell.phase1-shell' grid rule exists "
