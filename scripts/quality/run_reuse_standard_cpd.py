...
class CpdInputBlocked(RuntimeError):
...
def write_cpd_file_list(repo_root: Path, files: list[str], file_list: Path) -> Path:
    ...
    lines = [(repo_root / rel).resolve().as_posix() for rel in changed_file_utils.dedupe_paths(files)]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--mode',
        default=os.environ.get('FEIPI_REUSE_CPD_MODE') or os.environ.get('QUALITY_REUSE_CPD_MODE') or DEFAULT_MODE,
