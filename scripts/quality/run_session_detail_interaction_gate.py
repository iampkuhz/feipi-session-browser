...
async def run_interaction_gate(url: str, out_dir: Path) -> dict:
...
async def _reset_filter_to_all(page) -> None:
    """Ensure the filter chip is set to 'All'."""
    all_chip = await page.query_selector('.trace-panel__chip[data-status="all"]')
    if all_chip:
        is_active = await page.evaluate(
            "() => { const c = document.querySelector('.trace-panel__chip[data-status=\"all\"]'); return c && c.classList.contains('active'); }"
        )
        if not is_active:
...
async def _exercise_rounds(page) -> None:
    ...
    for idx in indices:
        ...
        () => {{
            ...
            if (toggles[{idx}]) window.toggleRoundDetail && window.toggleRoundDetail(toggles[{idx}], 'expand');
        }}
        """)
    ...
    for idx in indices:
        ...
        () => {{
            const toggles = document.querySelectorAll('.trace-round-toggle');
            if (toggles[{idx}]) window.toggleRoundDetail && window.toggleRoundDetail(toggles[{idx}], 'collapse');
        }}
        """)
        await page.wait_for_timeout(100)


async def _exercise_filters(page) -> None:
    """Click Failed, then All."""
    # Click Failed
    failed_chip = await page.query_selector('.trace-panel__chip[data-status="failed"]')
    if failed_chip:
        await page.evaluate("() => { const c = document.querySelector('.trace-panel__chip[data-status=\"failed\"]'); if (c && !c.classList.contains('active')) c.click(); }")
        await page.wait_for_timeout(300)

    # Click All
    all_chip = await page.query_selector('.trace-panel__chip[data-status="all"]')
    if all_chip:
        await page.evaluate("() => { const c = document.querySelector('.trace-panel__chip[data-status=\"all\"]'); if (c && !c.classList.contains('active')) c.click(); }")
        await page.wait_for_timeout(300)
...
async def _exercise_payload_modal(page) -> dict:
...
async def _exercise_expand_collapse_all(page) -> None:
    """Click Expand All, then Collapse All via JS."""
    await page.evaluate("""
    () => {
...
def _self_test():
    ...
    # Test overflow logic
    def check_overflow(scroll_w, inner_w):
    ...
    async def _run_browser_test(html, js):
        from playwright.async_api import async_playwright
        async with async_playwright() as pw:
            browser = await pw.chromium.launch(headless=True)
            context = await browser.new_context(viewport={"width": 1440, "height": 1100})
    ...
    def _sync_browser_test(html, js):
        try:
    ...
    try:
        ...
        _assert("toggle alignment JS works", align["count"] == 3, f"count={align.get('count')}")
    ...
    try:
        ...
        _assert("modal rendered JS works", mr["found"] and mr["length"] > 0, f"text={mr.get('text')}")
    except Exception as e:
    ...
    try:
        ...
        _assert("modal detects no-rendered", nr["isNoRenderedContent"], f"isNoRendered={nr.get('isNoRenderedContent')}")
    except Exception as e:
...
def main():
    ...
    if not args.url:
        print(
            ...
            "\n"
            "Example:\n"
            "  python3 " + " ".join(sys.argv) + " --url http://127.0.0.1:18999/sessions/claude_code/<session-id>\n",
            file=sys.stderr,
        )
        ...
        result = {
            ...
            "diagnostics": [{
                ...
                "nextInspection": [
                    ...
                    "Run: python3 " + " ".join(sys.argv) + " --url http://127.0.0.1:18999/sessions/claude_code/<id>",
                ],
            }],
        }
    ...
    url = args.url
    print(f"Session detail interaction gate")
    print(f"URL: {url}")
