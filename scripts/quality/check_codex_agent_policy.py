#!/usr/bin/env python3
"""检查 Codex custom agent 的参数和基础契约。"""
...
def check_agent(path: Path) -> list[str]:
    ...
    if not isinstance(instructions, str) or len(instructions.strip()) < 300:
    ...
    if name not in {"repo-mapper", "migration-planner", "ui-architect"} and "Validation" not in text:
