...
class LogCapture(logging.Handler):
    """Capture server errors emitted while the API smoke gate runs."""

    def emit(self, record: logging.LogRecord) -> None:
        """Append ERROR-level log messages to the smoke evidence buffer.
...
def _fetch_json(url: str, timeout: int = 10) -> tuple[int, Any | None, str | None]:
    """Fetch a JSON endpoint for the API smoke gate.

    Args:
        url: Input value for url.
        timeout: Input value for timeout.
...
def main() -> None:  # noqa: PLR0912, PLR0915 - smoke gate keeps sequential checks readable.
    """Run the API smoke quality gate against an ephemeral local server."""
    # 设置日志捕获
    handler = LogCapture()
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if m:
                ...
                status, data, _err = _fetch_json(round_url)
                if status >= HTTP_SERVER_ERROR:
                    errors.append(f'[BLOCK] Round API {round_url} 返回 {status}')
                elif status == HTTP_NOT_FOUND:
                    print('  [INFO] Round API 返回 404(session 可能无 round 数据)')
                elif status < HTTP_CLIENT_ERROR:
    ...
    if errors:
        ...
        print(
            f'\n[PASS] API Smoke 通过.{len(page_endpoints)} 个端点全部 < 500,'
            'Round API 正常,服务日志无 ERROR.'
        )
        sys.exit(0)
