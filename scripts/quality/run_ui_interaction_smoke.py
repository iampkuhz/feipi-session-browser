...
class LogCapture(logging.Handler):
    """Capture server log failures during the UI interaction smoke gate."""

    def emit(self, record: logging.LogRecord) -> None:
        """Store one ERROR log message for later smoke-gate evaluation.
...
async def run_smoke(base_url: str) -> list[str]:  # noqa: PLR0912, PLR0915 - UI flow is intentionally linear.
    """Exercise critical browser interactions and return blocking failures.
    ...
    Returns:
        Computed result.
    """
    from playwright.async_api import async_playwright  # noqa: PLC0415 - optional smoke dependency.
    ...
    async with async_playwright() as pw:
        ...
        def on_response(resp: object) -> None:
            """Collect non-static HTTP failures reported by Playwright.

            Args:
                resp: Input value for resp.
            """
            status = resp.status
            if status >= HTTP_CLIENT_ERROR:
                failed_requests.append(f'{resp.url} -> {status}')
        ...
        for path in ['/dashboard', '/sessions', '/projects', '/glossary']:
            try:
                resp = await page.goto(
                    f'{base_url}{path}', wait_until='domcontentloaded', timeout=15000
                )
                if resp and resp.status >= HTTP_SERVER_ERROR:
        ...
        try:
            resp = await page.goto(
                f'{base_url}/sessions', wait_until='domcontentloaded', timeout=15000
            )
            ...
            () => {
                const btn = document.querySelector(
                    '[data-action="clear"], [data-action="clear-search"]'
                );
        ...
        try:
            resp = await page.goto(
                f'{base_url}/projects', wait_until='domcontentloaded', timeout=15000
            )
            await page.wait_for_timeout(500)
            # 检查 table 是否存在
            has_table = await page.evaluate(
                "() => !!document.querySelector('#projects-table, .data-table')"
            )
            if has_table:
        ...
        try:
            ...
            () => {
                const a = document.querySelector(
                    '.sessions-row a, .col-title a, [data-action="row"] a'
                );
        ...
        if session_url:
            try:
                ...
                () => {
                    const btn = document.querySelector(
                        '[data-action="toggle-all"], [data-action="expand-all"]'
                    );
                ...
                if expand_result == 'clicked':
                    await page.wait_for_timeout(500)
                    print('  [PASS] Session detail Expand all 可点击')
                ...
                if failed_result == 'clicked':
                    await page.wait_for_timeout(500)
                    print('  [PASS] Session detail Failed filter 可点击')
                ...
                if round_result == 'expanded':
                    await page.wait_for_timeout(500)
                    print('  [PASS] Session detail round lazy load 可点击')
...
def main() -> None:
    """Run the browser interaction smoke gate against an ephemeral server."""
    # 设置日志捕获
    handler = LogCapture()
    ...
    from session_browser.web.routes import create_server  # noqa: PLC0415 - src path is set first.
    ...
    if errors:
        ...
        print(
            '\n[PASS] UI Interaction Smoke 通过.'
            '所有页面可访问,关键按钮可点击,无 console error,服务日志无 ERROR.'
        )
        sys.exit(0)
