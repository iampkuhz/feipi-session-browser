...
def _build_test_env() -> object:
    """创建 StrictUndefined 模板环境,注册所有必要 filter.

    Returns:
        Computed result.
    """
    import jinja2  # noqa: PLC0415 - optional template dependency for this gate only.
    ...
    from session_browser.web.renderers.markdown import render_markdown  # noqa: PLC0415
    from session_browser.web.safe_render import safe_json_display  # noqa: PLC0415
    from session_browser.web.template_env import (  # noqa: PLC0415
        _format_bytes,
        _format_compact_num,
    ...
    def _format_duration(seconds: float) -> str:
        """Format mock duration values for template filter parity.

        Args:
            seconds: Input value for seconds.
        ...
        if seconds >= SECONDS_PER_HOUR:
            return (
                ...
                f'{int((seconds % SECONDS_PER_HOUR) // SECONDS_PER_MINUTE)}min'
            )
        if seconds >= SECONDS_PER_MINUTE:
            return f'{int(seconds // SECONDS_PER_MINUTE)}min {int(seconds % SECONDS_PER_MINUTE)}s'
...
def _render_macro(env: object, template_name: str, macro_name: str, args: dict[str, object]) -> str:
    """渲染指定 macro,返回 HTML 或抛出 StrictUndefined 错误.

    Args:
        env: Input value for env.
        template_name: Input value for template_name.
        macro_name: Input value for macro_name.
...
def main() -> None:
    """Run the Jinja strict-render quality gate."""
    env = _build_test_env()
    errors = []
        ...
        'sub_rounds': [
            {
                ...
                'steps': [
                    {
                        'type': 'llm_call',
                        'call_id': 'nested-call-1',
                        'call_index': 1,
                        'status_label': 'OK',
                        'status_tone': 'ok',
                        'model': 'claude-sonnet-4',
                        'usage': {'input': 100, 'cache_read': 20, 'cache_write': 10, 'output': 50},
                        'context_payload_id': 'ctx-n1',
                        'response_payload_id': 'resp-n1',
                        'request_attribution_id': None,
                        'response_attribution_id': None,
                        'request_payload_missing_reason': None,
                        'response_payload_missing_reason': None,
                        'context_payload_title': 'Nested Request',
                        'response_payload_title': 'Nested Response',
                    },
                    {
                        'type': 'tool_batch',
                        'batch_id': 'nested-batch-1',
                        'title': 'Nested Tools',
                        'summary_label': 'Tools',
                        'meta': [],
                        'tools': [],
    ...
    mock_row = {
        ...
        'status_key': 'ok',
        'title': 'Test Round',
        'preview_title': 'Test Round',
        'tool_count_label': '3 tools',
        'token_total_raw': 500,
        'token_total': 500,
        'token_input': 300,
        'token_output': 100,
        'token_cache_read': 50,
        'token_cache_write': 50,
    ...
    tests = [
        (
            'expanded_row (wrapper)',
            'components/session_detail_timeline.html',
            'expanded_row',
            {'row': mock_row},
        ),
        (
            'expanded_row (round_table)',
            'components/session_detail_timeline/round_table.html',
            'expanded_row',
            {'row': mock_row},
        ),
        (
            ...
            'llm_call_card',
            {'call': mock_timeline_item_llm},
        ),
        (
            'tool_batch',
            'components/session_detail_timeline/llm_call.html',
            'tool_batch',
            {'batch': mock_timeline_item_tool},
        ),
        (
            'subagent_block',
            'components/session_detail_timeline/subagent.html',
            'subagent_block',
            {'block': mock_timeline_item_sub},
        ),
        (
            'subagent_block (nested llm_call/tool_batch)',
            'components/session_detail_timeline/subagent.html',
            'subagent_block',
            {'block': mock_timeline_item_sub_with_nested},
        ),
    ]
