...
CSS_DIR = Path(__file__).resolve().parent.parent.parent / "src" / "session_browser" / "web" / "static" / "css"
...
def extract_css_rules(text: str) -> list[tuple[int, str, str]]:
    ...
    处理 @media / @supports / @keyframes 等嵌套。
    """
...
def calculate_selector_depth(selector: str) -> int:
    ...
    使用组合符拆分，统计非空部分数量。
    特殊处理：
    - :root { ... } 中的选择器不算深度（它是变量声明容器）
    - @media/@keyframes 等 at-rule 已在外层过滤
    - 伪类/伪元素（:hover、::before）依附于前面的元素，不单独计数
    """
    ...
    for lineno, selector_str, _body in rules:
        ...
        for sel in selectors:
            ...
            if depth > BLOCK_THRESHOLD:
                report.blocks.append(DepthViolation(
                    severity="BLOCK",
                    file=css_path.name,
                    selector=sel,
                    depth=depth,
                ...
                report.warnings.append(DepthViolation(
                    severity="WARN",
                    file=css_path.name,
                    selector=sel,
                    depth=depth,
...
def print_report(report: DepthReport) -> None:
    ...
    if not report.blocks and not report.warnings:
        print("无违规。所有选择器深度均 <= {}。".format(WARN_THRESHOLD))
        print()
