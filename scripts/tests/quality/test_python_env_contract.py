...
def _write_project(root: Path, *, dev_extra: str = '') -> None:
    root = python_env.tool_project_dir(root)
...
def test_repository_pyproject_is_virtual_dev_tools_project():
    ...
    config = tomllib.loads((python_env.tool_config_path(REPO_ROOT)).read_text(encoding='utf-8'))
    ...
    assert config['tool']['pytest']['ini_options']['testpaths'] == ['scripts/tests']
    assert (python_env.tool_project_dir(REPO_ROOT) / 'uv.lock').is_file()
...
def test_official_uv_sync_entries_pin_the_local_environment() -> None:
    ...
    assert (
        'UV_PROJECT_ENVIRONMENT="$repo_root/.local/python/venv" uv sync --project scripts --frozen'
        in codex_setup
    )
    assert (
        'UV_PROJECT_ENVIRONMENT="$GITHUB_WORKSPACE/.local/python/venv" uv sync --project scripts --frozen --extra dev'
    ) in workflow
...
def test_lock_check_requires_pyproject_and_uv_lock(tmp_path: Path):
    ...
    (python_env.tool_project_dir(tmp_path) / 'uv.lock').unlink()
...
def test_lock_check_requires_python_312_contract(tmp_path: Path):
    _write_project(tmp_path)
    (python_env.tool_project_dir(tmp_path) / '.python-version').write_text(
        '3.13.0\n', encoding='utf-8'
    )
...
def test_lock_check_accepts_uv_minor_lock_equivalent(tmp_path: Path):
    _write_project(tmp_path)
    (python_env.tool_project_dir(tmp_path) / 'uv.lock').write_text(
