...
from scripts.gates.checks.repository import check_file_boundary as policy
...
def test_controlled_gate_config_directory_passes(tmp_path: Path) -> None:
    root = tmp_path / 'repo'
    _init_repo(root)
    config = root / 'scripts/gates/config/technical-terms.json'
    config.parent.mkdir()
    config.write_text('{}', encoding='utf-8')
    _git(root, 'add', 'scripts/gates/config/technical-terms.json')

    assert policy.check(['--root', str(root), '--all-tracked']).passed


def test_gate_config_name_does_not_allow_a_root_file(tmp_path: Path) -> None:
    root = tmp_path / 'repo'
    _init_repo(root)
    (root / 'scripts/gates/config').write_text('{}', encoding='utf-8')

    result = policy.check(['--root', str(root)])

    assert not result.passed
    assert any(
        'Gate 根目录路径未登记: scripts/gates/config' in item.message for item in result.diagnostics
    )
