...
from scripts.gates.checks.repository.check_acceptance_traceability import (
    EXPECTED_FEATURE_TABLES,
    _validate_acceptance_traceability,
)
...
def test_traceability_accepts_defined_case_with_existing_code_binding(tmp_path: Path) -> None:
    tests_dir = tmp_path / 'scripts' / 'tests'
    tests_dir.mkdir(parents=True)
    code_path = 'scripts/tests/test_feature.py'
    ...
    result = _validate_acceptance_traceability(tmp_path)

    assert result.errors == []


def test_traceability_rejects_test_id_missing_from_case_tables(tmp_path: Path) -> None:
    tests_dir = tmp_path / 'scripts' / 'tests'
    tests_dir.mkdir(parents=True)
...
def test_traceability_uses_current_acceptance_cases_directory(tmp_path: Path) -> None:
