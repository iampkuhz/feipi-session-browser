...
TERMS, FORBIDDEN = checker._load_policy(REPO_ROOT / 'scripts/gates/config/technical-terms.json')
...
'''
    ...
    assert 'scripts/gates/config/technical-terms.json' in violation.suggestion
...
def test_public_check_accepts_valid_script(tmp_path: Path) -> None:
    ...
    assert checker.check(
        [str(source), '--policy', str(REPO_ROOT / 'scripts/gates/config/technical-terms.json')]
    ).passed
...
def test_scripts_discovery_preserves_implementation_only_scope(tmp_path: Path) -> None:
    """测试下沉后仍扫描脚本实现；仅显式选择时扫描测试文件。"""
    scripts = tmp_path / 'tmp' / 'scripts'
    tests = scripts / 'tests'
    tests.mkdir(parents=True)
    implementation = scripts / 'tool.py'
    test_file = tests / 'test_tool.py'
    implementation.write_text('def run(): pass\n', encoding='utf-8')
    test_file.write_text('def test_run(): pass\n', encoding='utf-8')
    for excluded in ('tmp', '__pycache__', 'node_modules', 'build'):
        nested = scripts / 'nested' / excluded / 'cached.py'
        nested.parent.mkdir(parents=True)
        nested.write_text('def cached(): pass\n', encoding='utf-8')

    assert checker._discover([str(scripts)]) == [implementation]
    assert checker._discover([str(test_file)]) == [test_file]


def test_default_policy_uses_gate_owned_configuration(tmp_path: Path, monkeypatch) -> None:
    policy = tmp_path / 'scripts/gates/config/technical-terms.json'
    policy.parent.mkdir(parents=True)
    policy.write_text(
        '{"canonical_terms": ["Gate"], "forbidden_translations": []}', encoding='utf-8'
    )
    monkeypatch.setattr(checker, 'REPO_ROOT', tmp_path)

    assert checker._load_policy() == ({'Gate'}, ())
    source = tmp_path / 'tool.py'
    source.write_text('"""负责读取检查输入；不负责写入数据，由 Gate 调用。"""\n', encoding='utf-8')
    assert checker.check([str(source)]).passed
