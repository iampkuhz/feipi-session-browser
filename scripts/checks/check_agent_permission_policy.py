...
def main() -> int:
    ...
    if default_mode == "bypassPermissions":
        ...
        if not PRE_WRITE_HOOK.is_file():
            errors.append("defaultMode 为 bypassPermissions 但 .claude/hooks/pre-write.sh 不存在")
        ...
        if manifest_data is None:
            ...
            platforms = manifest_data.get("platforms", {}) if manifest_data else {}
            claude_cfg = platforms.get("claude", {}) if isinstance(platforms, dict) else {}
            claude_hooks = claude_cfg.get("hooks", {}) if isinstance(claude_cfg, dict) else {}
