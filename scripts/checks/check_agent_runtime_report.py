...
def get_protected_roots() -> list[str]:
    """解析 `get_protected_roots` 对应的输入并返回规范化结果；不修改调用方数据。"""
...
def is_protected(path: str, roots: list[str] | None = None) -> bool:
    """判断 `is_protected` 对应的约束是否成立；不修改输入状态。"""
...
def validate_report_content(
    ...
    if not isinstance(outcomes, list):
        ...
        outcomes = []
    outcome_ids = {
        str(item.get("id", "")) for item in outcomes if isinstance(item, dict) and item.get("id")
    }
    ...
    if not isinstance(escape_rate, (int, float)) or not isinstance(threshold, (int, float)):
        ...
        errors.append(f"gate_escape_rate.escape_rate({escape_rate}) 超过 threshold({threshold})")
    ...
    for row in concurrency:
        if _status(row) != "PASS":
            ...
            errors.append(f"concurrency_matrix {rid} 状态必须为 PASS，实际为 {_status(row) or '?'}")
...
def main() -> int:
    ...
    if args.run_id or args.report_path:
        if not all(
            ...
            return fail(
                "run-scoped runtime report requires --run-id --client --session-id --change-id --worktree-root --changed-files --report-path"
            )
        try:
    ...
    print(
        f"[{GATE_NAME}] legacy warning: legacy runtime report mode; not multi-primary safe",
        file=sys.stderr,
    )
