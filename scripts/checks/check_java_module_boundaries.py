#!/usr/bin/env python3
"""本模块负责执行 `check_java_module_boundaries` 对应的确定性仓库检查。
...
class Finding:
    """保存 `Finding` 的结构化契约数据；字段由所属运行阶段构造并由后续报告读取。"""
    ...
    for module in sorted(actual - configured):
        findings.append(
            Finding(
                ...
                'unregistered-module',
                'settings.gradle.kts',
                f'{module} 未登记到 java-modules.yaml',
            )
        )
        ...
        if (mdir / 'build.gradle.kts').exists():
            findings.append(
                Finding(
                    'ERROR',
                    'missing-include',
                    rel(mdir / 'build.gradle.kts'),
                    f'{module} 有 build.gradle.kts 但未 include',
                )
            )
    return findings
            ...
            if dep in transition:
                ...
                findings.append(
                    Finding(
                        ...
                        rel(build_file),
                        f'{module} 仍依赖过渡模块 {dep}',
                    )
                )
                continue
            findings.append(
                Finding(
                    'ERROR', 'forbidden-dependency', rel(build_file), f'{module} 不允许依赖 {dep}'
                )
            )
    return findings
    ...
    for module, meta in config['modules'].items():
        ...
        for java_file in java_files(root_path):
            ...
            if pkg is None:
                findings.append(
                    Finding(
                        'ERROR', 'missing-package', rel(java_file), 'Java 文件缺少 package 声明'
                    )
                )
                ...
                findings.append(
                    Finding(
                        ...
                        'package-root',
                        rel(java_file),
                        f'package {pkg} 不属于 {module} 的 roots {roots}',
                    )
                )
    return findings
        ...
        if not matches:
            findings.append(
                Finding(
                    ...
                    'class-missing',
                    class_name,
                    f'{class_name} 未找到，期望位于 {required_module}/{required_package}',
                )
            )
            continue
            ...
            if module != required_module or pkg != required_package:
                findings.append(
                    Finding(
                        ...
                        'class-placement',
                        rel(path),
                        f'{class_name} 应位于 {required_module} 的 package {required_package}，当前 {module}/{pkg}',
                    )
                )
    return findings
...
def main() -> int:
    ...
    parser.add_argument(
        '--fail-transition', action='store_true', help='将 transition dependency 也视为失败'
    )
