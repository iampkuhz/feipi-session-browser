...
本检查用于防止已清理内容回流，包含四条规则：源码不得记录历史版本标记；Harness 只描述
当前状态；前端只支持桌面视口；静态资源不得保留空文件或无效兼容垫片。唯一公开入口是
``check(arguments)``；返回诊断表示源码违反当前状态政策。
"""
...
def _check_current_source_policy(repo_root: Path) -> tuple[list[str], list[str]]:
    """运行四类当前源码政策检查并分别汇总错误与告警。"""
    errors: list[str] = []
    warnings: list[str] = []
    ...
    excluded_files = {
        'check_current_source_policy.py',
        'test_current_source_policy.py',
    }
    filtered_files = []
    for f in all_text_files:
        ...
        if len(relative.parts) >= 2 and relative.parts[:2] == ('openspec', 'changes'):
            # 变更工作区记录迁移过程，不是仓库当前态规范；长期真相位于 openspec/specs。
            continue
        if (
