#!/usr/bin/env python3
"""本模块负责执行 `check_agent_hook_parity` 对应的确定性仓库检查。
...
def main() -> int:
        ...
        for key in REQUIRED_HOOK_KEYS:
            if key not in hooks:
                ...
                if reason_key in plat_cfg or reason_key in hooks:
                    continue
                errors.append(f"platforms.{plat}.hooks 缺少 {key}（或 {reason_key}）")
    ...
    for plat in PLATFORMS:
        ...
        for key in REQUIRED_HOOK_KEYS:
            ...
            if val is None:
                ...
                if reason_key in plat_cfg or reason_key in hooks:
                    continue
                errors.append(f"platforms.{plat}.hooks.{key} 为空或 null，且无 {reason_key}")
                continue
            if isinstance(val, list) and len(val) == 0:
                reason_key = f"{key}_unsupported_reason"
                if reason_key in plat_cfg or reason_key in hooks:
                    continue
                errors.append(f"platforms.{plat}.hooks.{key} 是空 list，且无 {reason_key}")
                continue
                ...
                for hf in val:
                    if isinstance(hf, str):
                        ...
                        if not full_path.is_file():
                            errors.append(f"platforms.{plat}.hooks.{key} 中文件不存在: {hf}")
    ...
    for plat in ["claude", "codex"]:
        ...
        if not isinstance(pre_write, list) or len(pre_write) == 0:
            errors.append(f"platforms.{plat}.hooks.pre_write 必须非空")
    ...
    if isinstance(qoder_cfg, dict):
        ...
        if isinstance(qoder_hooks, dict):
            ...
            if not isinstance(qoder_pre_write, list) or len(qoder_pre_write) == 0:
                reason = qoder_cfg.get("pre_write_unsupported_reason") or qoder_hooks.get(
                    "pre_write_unsupported_reason"
                )
                if not reason:
                    errors.append(
                ...
                for hf in qoder_pre_write:
                    if isinstance(hf, str) and not (ROOT / hf).is_file():
                        errors.append(f"platforms.qoder.hooks.pre_write 中文件不存在: {hf}")
        ...
        if hooks_data is not None:
            for (event, matcher), expected in CODEX_REQUIRED_BINDINGS.items():
                ...
                if not any(
                    ...
                    errors.append(
                        f".codex/hooks.json {event}{suffix} 未绑定 Git-root stable command for {expected}"
                    )
            ...
            for command in stop_commands:
                if ".codex/hooks/stop_check.sh" in command:
                    ...
                    if int(stop_hook.get("timeout") or 0) < 1230:
                        errors.append(
                            ".codex/hooks.json Stop timeout must exceed 1200s target timeout plus cleanup buffer"
                        )
        ...
        if qoder_settings is not None:
            for (event, matcher), expected in QODER_REQUIRED_BINDINGS.items():
                ...
                if not any(
                    ...
                    suffix = f" matcher={matcher}" if matcher else ""
                    errors.append(
                        f".qoder/settings.json {event}{suffix} 未绑定 Git-root stable command for {expected}"
                    )
            stop_commands = _commands_for(qoder_settings, "Stop", "")
            for command in stop_commands:
                if ".qoder/hooks/stop_check.sh" in command:
                    stop_hook = (
                        qoder_settings.get("hooks", {}).get("Stop", [])[0].get("hooks", [])[0]
                    )
                    if int(stop_hook.get("timeout") or 0) < 1230:
                        errors.append(
                            ".qoder/settings.json Stop timeout must exceed 1200s target timeout plus cleanup buffer"
                        )
    ...
    local_only = (
        data.get("permission_policy", {}) if isinstance(data.get("permission_policy"), dict) else {}
    )
