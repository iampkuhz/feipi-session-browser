...
def run_checks() -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-runtime-worktree-") as tmp:
        ...
        _expect(
            mismatch_result.returncode == 2 and "WORKTREE_BASE_MISMATCH" in mismatch_result.stderr,
        ...
        with ThreadPoolExecutor(max_workers=len(sessions)) as executor:
            bootstrap_results = {
                name: executor.submit(_bootstrap, repo, runtime_root, client, session_id, checkout)
        ...
        _expect(
            bootstrapped["primary-a"].get("worktreeId") != bootstrapped["linked"].get("worktreeId"),
        ...
        primary_successes = [
            name for name in ("primary-a", "primary-b") if acquired[name].returncode == 0
        ]
        primary_conflicts = [
            name for name in ("primary-a", "primary-b") if acquired[name].returncode != 0
        ]
        ...
        for name, record in active.items():
            ...
            _expect(
                released.get("status") == "READ_ONLY_READY" and not released.get("writerLease"),
