...
BLOCK_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"(^|[;&|]\s*)rm\s+-[^\n]*r[^\n]*f\s+(/|['\"]?/['\"]?)\s*($|[;&|])", re.I), "禁止 rm -rf /。"),
    (re.compile(r"(^|[;&|]\s*)rm\s+-[^\n]*r[^\n]*f\s+(~|\$HOME)(/|\s|$)", re.I), "禁止 rm -rf ~ 或 $HOME。"),
    (re.compile(r"\bgit\s+reset\s+--hard\b", re.I), "禁止 git reset --hard 破坏未提交改动。"),
    (re.compile(r"\bgit\s+clean\s+-[^\n]*f[^\n]*d[^\n]*x\b|\bgit\s+clean\s+-[^\n]*x[^\n]*d[^\n]*f\b", re.I), "禁止 git clean -fdx。"),
    (re.compile(r"\bchmod\s+-R\s+777\s+/\b", re.I), "禁止 chmod -R 777 /。"),
    (re.compile(r"\bdd\s+if=.*\s+of=/dev/", re.I), "禁止 dd 写入 /dev 设备。"),
    (re.compile(r"\bcat\s+~/(?:\.claude|\.codex|\.qoder)/.*\.jsonl\b", re.I), "禁止直接 cat 大段真实 session jsonl。请用 head/sed/rg 局部读取。"),
    (re.compile(r"\b(cat|grep|rg)\b.*(~/(?:\.ssh|\.aws)|id_rsa|id_ed25519|aws_access_key|secret_access_key)", re.I), "禁止读取或输出 SSH/AWS 密钥。"),
]

WARN_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"\bcurl\b[^\n|;]*\|\s*(sh|bash)\b", re.I), "curl | sh 安装脚本存在供应链风险；建议先下载并审计。"),
    (re.compile(r"\bwget\b[^\n|;]*\|\s*(sh|bash)\b", re.I), "wget | sh 安装脚本存在供应链风险；建议先下载并审计。"),
]
