...
本模块只保留面向 identity/session 的高层辅助函数。
Git 证据采集实现已移入 `stop_entry_checks/git_evidence.py`。
...
# ── 向后兼容 re-export（Git 证据实现已移入 git_evidence.py）───────
from scripts.harness.stop_entry_checks.git_evidence import (  # noqa: E402, F401
    GitEvidenceError,
    _hash_untracked_contents,
