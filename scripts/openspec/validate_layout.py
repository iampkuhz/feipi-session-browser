...
errors = []
if not (root/'specs').is_dir(): errors.append('openspec/specs missing')
if not (root/'changes').is_dir(): errors.append('openspec/changes missing')
for change in (root/'changes').glob('*') if (root/'changes').exists() else []:
    if change.name == 'archive' or not change.is_dir(): continue
