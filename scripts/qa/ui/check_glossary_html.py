...
def main() -> int:  # noqa: PLR2004 - thresholds are static DOM contract counts.
    ...
    checks: list[tuple[str, callable]] = [
        ...
        (
            ...
            lambda: (
                "{% set active_page = 'glossary' %}" in html,
                "active_page='glossary' found"
                if "{% set active_page = 'glossary' %}" in html
                else 'MISSING',
            ),
        ),
        (
        ...
        (
            ...
            lambda: ('<h1>' in html, 'h1 found' if '<h1>' in html else 'MISSING'),
        ),
        (
            'T165-H07 .subtitle present',
            lambda: (
                'class="subtitle"' in html,
        ...
        ('T165-H12 glossary.js exists and syntax valid', lambda: _check_js_syntax(js)),
        (
            'T165-H13 glossary.css linked in head_extra',
            lambda: (
                '/static/css/glossary.css' in html,
        ...
        (
            ...
            lambda: (
                html.count('class="card metric-card"') >= 4,
                "{} card.metric-card(s) found".format(html.count('class="card metric-card"'))
                if html.count('class="card metric-card"') >= 4
                else "ONLY {} card.metric-card(s)".format(html.count('class="card metric-card"')),
            ),
        ),
        (
        ...
        (
            ...
            lambda: (
                'card section' in html and html.count('class="card section') >= 7,
                "{} card.section(s) found".format(html.count('class="card section'))
                if html.count('class="card section') >= 7
                else "ONLY {} card.section(s)".format(html.count('class="card section')),
            ),
        ),
        (
            'T165-H28 .section-head in each section',
            lambda: (
                'class="section-head"' in html,
        ...
        (
            ...
            lambda: (
                'data-table' in html and html.count('class="data-table') >= 5,
                "{} data-table(s) found".format(html.count('class="data-table'))
                if html.count('class="data-table') >= 5
                else "ONLY {} data-table(s)".format(html.count('class="data-table')),
            ),
        ),
        (
            'T165-H32 data-table-enhanced on all tables',
            lambda: (
                html.count('data-table-enhanced') >= 5,
        ...
        (
            ...
            lambda: (
                'id="provider-mapping"' in html and 'class="section-title">Provider 映射' in html,
        ...
        (
            ...
            lambda: (
                'class="section-title">已知限制' in html,
                'Known limitations section found'
                if 'class="section-title">已知限制' in html
                else 'MISSING',
            ),
        ),
        (
            ...
            lambda: (
                'class="section-title">Session Anomalies' in html,
                'Session Anomalies section found'
                if 'class="section-title">Session Anomalies' in html
                else 'MISSING',
            ),
        ),
        (
            ...
            lambda: (
                'class="section-title">Round Signals' in html,
                'Round Signals section found'
                if 'class="section-title">Round Signals' in html
                else 'MISSING',
            ),
        ),
        ...
        (
            ...
            lambda: (
                'data-search="glossary-term"' in html,
                'glossary-term search found'
                if 'data-search="glossary-term"' in html
                else 'MISSING',
            ),
        ),
        # ── Accessibility ──────────────────────────────────────────
        ...
        (
            ...
            lambda: (
                'aria-label="Search glossary terms"' in html,
                'aria-label=Search glossary terms found'
                if 'aria-label="Search glossary terms"' in html
                else 'MISSING',
            ),
        ),
        (
            ...
            lambda: (
                'aria-label="术语页摘要指标"' in html,
                'aria-label on metric grid found'
                if 'aria-label="术语页摘要指标"' in html
                else 'MISSING',
            ),
        ),
        # ── Breadcrumb ─────────────────────────────────────────────
        (
            ...
            lambda: (
                'href="/dashboard"' in html,
                'breadcrumb with dashboard link found'
                if 'href="/dashboard"' in html
                else 'MISSING',
            ),
        ),
        # ── Token formatting ───────────────────────────────────────
        (
            ...
            lambda: (
                'format_compact_token' not in html,
                'clean — no format_compact_token in static glossary'
                if 'format_compact_token' not in html
        ...
        (
            ...
            lambda: (
                'TODO' not in html and 'FIXME' not in html and 'HACK' not in html,
                'clean'
                if ('TODO' not in html and 'FIXME' not in html and 'HACK' not in html)
...
def _check_js_syntax(js: str) -> tuple[bool, str]:  # noqa: PLR0912 - mirrors JS parser diagnostics.
    ...
    return (
        True,
        f'balanced braces/parens/brackets, {len(js)} bytes, all structural markers present',
    )
