#!/usr/bin/env python3
"""SessionStart/SubagentStart hook: inject concise workflow context into the
agent session so that every session (main or subagent) knows the current
OpenSpec change status, protected-edit rule, and evidence path.
...
def _count_total_tasks(change_id: str) -> int:
    ...
    for line in content.splitlines():
        stripped = line.strip()
        if stripped.startswith("- [x]") or stripped.startswith("- [ ]") or \
           stripped.startswith("* [x]") or stripped.startswith("* [ ]"):
            count += 1
    return count
...
def _run_self_test() -> int:
    """Validate the injector in a temporary git repo.
    ...
    with tempfile.TemporaryDirectory(prefix="inject_ctx_test_") as tmpdir:
        ...
        check(f"line count <= 20 (got {line_count})", line_count <= 20,
              f"lines={line_count}")
