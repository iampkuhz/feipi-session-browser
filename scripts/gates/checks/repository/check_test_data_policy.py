...
唯一入口 ``check(arguments)`` 检查两件事：仓库内 fixture/resource 数据文件必须被 Git 跟踪；
测试源码、维护说明与这些数据不得包含真实 Session、个人用户名或固定个人设备路径。README
只解释如何维护相邻数据，不是测试输入，因此不按数据文件要求其在检查前已经进入 Git 索引。
诊断只报告相对路径、行号和规则名，不复述命中的敏感内容。
"""
...
def _is_fixture_path(path: Path, root: Path) -> bool:
    """判断文件是否位于 Python/Playwright 共用的 fixture 目录。"""
    try:
...
def _is_fixture_documentation(path: Path, root: Path) -> bool:
    """判断文件是否只是 fixture 目录的维护说明，而不是测试输入数据。"""
    return path.name.lower() == 'readme.md' and _is_fixture_path(path, root)
...
def _is_managed_data(path: Path, root: Path) -> bool:
    ...
    try:
        ...
        return False
    is_fixture = _is_fixture_path(path, root)
...
def _validate_test_data_policy(root: Path) -> list[str]:
    ...
    errors = [
        ...
        and not _is_fixture_documentation(path, root)
    ]
