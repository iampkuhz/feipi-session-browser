...
| `javaCheck` | 运行 Java 编译、测试和标准源码规则。 | 自动：改动 Java/Kotlin 源码或测试、Gradle 与模块构建配置、技术词表或锁文件。<br>人工：`--gate javaCheck`；Target `java-src`、`java-build`。<br>执行：无 `-x` 排除的 Gradle root `check`；原生任务图统一负责编译、测试、Checkstyle、Javadoc、零 skipped/aborted、中文注释、record Javadoc、PMD suppression 与 PMD。 | `≤240s` | `≤600s` |
